...
class TestWorktreeDirtyReasonByPath:
    """Unit tests for _worktree_dirty_reason_by_path (#2118)."""

    def test_returns_reason_string_when_dirty(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Returns the exact reason string unsaved_work_reason produces."""
        from cw.reconcile import _worktree_dirty_reason_by_path
        ...
        result = _worktree_dirty_reason_by_path("client-a", tmp_path / "wt")
        assert result == "2 uncommitted path(s)"

    def test_returns_none_when_clean(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Returns None when unsaved_work_reason reports a clean worktree."""
        from cw.reconcile import _worktree_dirty_reason_by_path
    ...
    def test_returns_none_when_checked_out_branch_raises(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Fail-safe: returns None (not dirty) when the internal call raises."""
        from cw.reconcile import _worktree_dirty_reason_by_path
