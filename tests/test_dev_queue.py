...
class TestMigrateDevQueue:
    ...
    def test_migrate_dev_queue_fills_pending_fix_dispatch_default(self) -> None:
        """migrate_dev_queue fills pending_fix_dispatch=None (v34, #2017)."""
        raw: dict[str, object] = {
            "schema_version": 33,
            "tasks": [
                {
    ...
    def test_migrate_dev_queue_fills_fix_dispatch_session_id_default(self) -> None:
        ...
        migrated = migrate_dev_queue(raw)
        assert migrated["tasks"][0]["fix_dispatch_session_id"] is None

    def test_v34_pending_fix_dispatch_preserved_idempotently(self) -> None:
        """A recorded handoff survives a second migration pass -- the filler is
        additive, never an overwrite back to None. A dropped payload here would
        silently lose the fix loop's entire action list (#2017 R21.4)."""
        pending = {
        ...
        twice = migrate_dev_queue(migrate_dev_queue(raw))
        assert twice["tasks"][0]["pending_fix_dispatch"] == pending
        assert twice["tasks"][0]["fix_dispatch_session_id"] == "fix-sess"
...
_APPROVED_HEAD = "0123456789abcdef0123456789abcdef01234567"


class TestApproveScopeDrift:
    """#2337: the operator grants plan_scope_drift growth on the row, bound to
    the branch head, and the ticket re-runs IMPL via a same-stage requeue."""
    ...
    def _seed(
        ...
        *,
        ...
        """Persist one parked row and stub the gh HEAD lookup; return its calls."""
        _write_client_yaml(tmp_config_dir, tmp_path)
        task = _make_blocked_task(
            status=status, stage=stage, blocked_reason=blocked_reason
        )
    ...
    def test_happy_path_stamps_and_requeues_at_impl(
        ...
        result = approve_scope_drift_ticket(
            "GEN-500", "genhealth", ["src/b.py", "src/a.py"]
        )
        ...
        assert result == {
            ...
            "ticket_id": "GEN-500",
            "client": "genhealth",
            "extra_files": ["src/a.py", "src/b.py"],
            "approved_head": _APPROVED_HEAD,
        }
        assert calls == [("dev/GEN-500", tmp_path / "ws")]
        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.status == QueueItemStatus.PENDING
        assert t.stage == Stage.IMPL
        assert t.session_id is None
        assert t.stage_base_ref is None
        assert t.scope_drift_approved_extra_files == ["src/a.py", "src/b.py"]
        assert t.scope_drift_approved_head == _APPROVED_HEAD

    def test_extra_files_are_deduped_and_sorted(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_scope_drift_ticket
        ...
        result = approve_scope_drift_ticket(
            "GEN-500", "genhealth", ["z.py", "a.py", "z.py", "m.py", "a.py"]
        )

        assert result["extra_files"] == ["a.py", "m.py", "z.py"]
        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.scope_drift_approved_extra_files == ["a.py", "m.py", "z.py"]

    @pytest.mark.parametrize(
        ("status", "stage", "blocked_reason", "match"),
        [
            (
            ...
            (
                ...
                Stage.IMPL,
                "impl_failed",
                "blocked_reason is 'impl_failed'",
            ),
        ],
        ids=["not_blocked", "not_impl_stage", "wrong_blocked_reason"],
    )
    def test_rejects_a_row_not_parked_for_plan_scope_drift(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ...
    def test_rejects_empty_extra_files(
        ...
        with pytest.raises(ApproveGateError, match="no extra files"):
            approve_scope_drift_ticket("GEN-500", "genhealth", [])
    ...
    @pytest.mark.parametrize(
        ...
        ids=["gh_unavailable", "gh_no_sha"],
    )
    def test_rejects_when_branch_head_unresolvable_without_mutation(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        head: tuple[str | None, bool],
    ) -> None:
        from cw.dev_queue import approve_scope_drift_ticket
        from cw.exceptions import ApproveGateError

        self._seed(tmp_config_dir, tmp_path, monkeypatch, head=head)
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="could not resolve"):
            approve_scope_drift_ticket("GEN-500", "genhealth", ["a.py"])
...
# ---------------------------------------------------------------------------
# TestApproveMustFixOverride — approve_must_fix_override_ticket (#2205)
# ---------------------------------------------------------------------------

_REVIEWED_SHA = "fedcba9876543210fedcba9876543210fedcba98"


def _must_fix_override() -> MustFixOverride:
...
class TestApproveMustFixOverride:
    """#2205: the operator records a durable, audited override of a codex
    MUST_FIX park, bound to the verdict's reviewed_sha and finding identities.
    The row is stamped only -- never moved."""
    ...
    def _seed(
        ...
        *,
        status: QueueItemStatus = QueueItemStatus.BLOCKED_ON_USER,
        blocked_reason: str | None = "codex_must_fix_findings",
        must_fix: tuple[tuple[str, str], ...] = (
            ("src/b.py", "Leak of 3 handles at line 40"),
            ("src/a.py", "Bug  here"),
        ),
        verdict_ticket_id: str = "GEN-500",
        write_verdict: bool = True,
        login: str | None = "octocat",
    ) -> tuple[Path, list[CapturedEvent]]:
        """Persist one parked row with a verdict in its worktree; capture events."""
        from tests.conftest import _make_finding, write_review_verdict_envelope

        _write_client_yaml(tmp_config_dir, tmp_path)
        worktree = tmp_path / "wt-must-fix"
        worktree.mkdir()
        task = _make_blocked_task(
            status=status, stage=Stage.REVIEW, blocked_reason=blocked_reason
        )
        task.worktree_path = worktree
        save_dev_queue(DevQueueStore(tasks=[task]))
        verdict_path = worktree / ".claude" / "review-verdict.json"
        if write_verdict:
            verdict_path = write_review_verdict_envelope(
                worktree,
                ticket_id=verdict_ticket_id,
                reviewed_sha=_REVIEWED_SHA,
                must_fix=[
                    _make_finding(file=file, summary=summary)
                    for file, summary in must_fix
                ],
            )
        ...
        def _capture(
            ...
            *,
            ...
            events.append((event_type, dict(payload), correlation_id))

        monkeypatch.setattr("cw.dev_queue.must_fix_override.record_event", _capture)
        return verdict_path, events

    @freeze_time("2026-09-25 12:00:00")
    def test_happy_path_stamps_override_and_leaves_row_parked(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_must_fix_override_ticket

        _, events = self._seed(tmp_config_dir, tmp_path, monkeypatch)

        result = approve_must_fix_override_ticket(
            "GEN-500", "genhealth", "  shipping; follow-up #9999  "
        )

        expected_ids = [("src/a.py", "bug here"), ("src/b.py", "leak of N handles")]
        assert result == {
            "ticket_id": "GEN-500",
            "client": "genhealth",
            "stage": "review",
        ...
        assert t.status == QueueItemStatus.BLOCKED_ON_USER
        assert t.stage == Stage.REVIEW
        assert t.blocked_reason == "codex_must_fix_findings"
        assert t.must_fix_override == MustFixOverride(
            actor="octocat",
            reason="shipping; follow-up #9999",
            reviewed_sha=_REVIEWED_SHA,
            finding_ids=expected_ids,
        ...
        assert events == [
            (
                OrchestratorEventType.TICKET_APPROVED,
                {
                    ...
                    "reason": "shipping; follow-up #9999",
                    "reviewed_sha": _REVIEWED_SHA,
                    "finding_ids": [list(pair) for pair in expected_ids],
                },
                "GEN-500",
            )
        ]

    def test_finding_ids_are_deduped(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_must_fix_override_ticket
        ...
        self._seed(
            ...
            tmp_path,
            monkeypatch,
            must_fix=(("src/a.py", "Bug here"), ("src/a.py", "bug   HERE")),
        )

        result = approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert result["finding_ids"] == [("src/a.py", "bug here")]

    def test_unresolvable_operator_login_records_empty_actor(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ...
        self._seed(tmp_config_dir, tmp_path, monkeypatch, login=None)
        ...
        assert result["actor"] == ""
        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.must_fix_override is not None
        assert t.must_fix_override.actor == ""
        assert any("operator login" in r.message for r in caplog.records)

    def test_rerun_overwrites_the_prior_record(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Newest wins: a second override on the same park replaces the first
        rather than stacking."""
        from cw.dev_queue import approve_must_fix_override_ticket

        self._seed(tmp_config_dir, tmp_path, monkeypatch)
        approve_must_fix_override_ticket("GEN-500", "genhealth", "first")
        approve_must_fix_override_ticket("GEN-500", "genhealth", "second")

        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.must_fix_override is not None
        assert t.must_fix_override.reason == "second"

    @pytest.mark.parametrize(
        ("status", "blocked_reason", "match"),
        [
            (
                QueueItemStatus.PENDING,
            ...
            (
                QueueItemStatus.BLOCKED_ON_USER,
                "plan_scope_drift",
                "blocked_reason is 'plan_scope_drift'",
            ),
            (
                QueueItemStatus.BLOCKED_ON_USER,
                None,
        ...
        ids=["not_blocked", "scope_drift_park", "no_blocked_reason"],
    )
    def test_rejects_a_row_not_parked_for_codex_must_fix(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ...
        with pytest.raises(ApproveGateError, match=match):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")
    ...
    def test_rejects_blank_reason(
        ...
        with pytest.raises(ApproveGateError, match="--reason"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", reason)

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_rejects_missing_verdict_file(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_must_fix_override_ticket
        from cw.exceptions import ApproveGateError

        verdict_path, events = self._seed(
            tmp_config_dir, tmp_path, monkeypatch, write_verdict=False
        )
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="could not read") as excinfo:
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert str(verdict_path) in str(excinfo.value)
        assert events == []
        assert load_dev_queue().model_dump() == before

    @pytest.mark.parametrize(
        "body",
        ["not json", '{"ticket_id": "GEN-500"}', '{"verdict": {}}'],
        ids=["not_json", "no_verdict", "no_ticket_id"],
    )
    def test_rejects_unparseable_verdict(
        ...
        verdict_path, events = self._seed(tmp_config_dir, tmp_path, monkeypatch)
        verdict_path.write_text(body, encoding="utf-8")
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="not a valid review verdict"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_rejects_foreign_verdict(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A verdict owned by another ticket is not this park's verdict."""
        from cw.dev_queue import approve_must_fix_override_ticket
        from cw.exceptions import ApproveGateError

        _, events = self._seed(
            tmp_config_dir, tmp_path, monkeypatch, verdict_ticket_id="GEN-999"
        )
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="belongs to ticket 'GEN-999'"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_rejects_non_blocking_verdict(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_must_fix_override_ticket
        from cw.exceptions import ApproveGateError

        _, events = self._seed(tmp_config_dir, tmp_path, monkeypatch, must_fix=())
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="not blocking"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_rejects_unfingerprintable_finding(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Fail closed: a no-diff-anchor MUST_FIX has no identity to bind, so
        the override refuses rather than silently leaving it uncovered."""
        from cw.dev_queue import approve_must_fix_override_ticket
        from cw.exceptions import ApproveGateError
        ...
        _, events = self._seed(
            ...
            tmp_path,
            monkeypatch,
            must_fix=(("src/a.py", "Bug here"), ("N/A", "Missing follow-up")),
        )
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="Missing follow-up"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_rejects_when_worktree_unresolvable(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_must_fix_override_ticket
        from cw.exceptions import ApproveGateError
        ...
        _, events = self._seed(tmp_config_dir, tmp_path, monkeypatch)
        store = load_dev_queue()
        store.tasks[0].worktree_path = None
        save_dev_queue(store)
        before = load_dev_queue().model_dump()

        with pytest.raises(ApproveGateError, match="worktree"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert events == []
        assert load_dev_queue().model_dump() == before

    def test_event_failure_leaves_row_unstamped(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Event-first ordering: an audit write that fails means no override
        was recorded, so FINALIZE keeps blocking."""
        from cw.dev_queue import approve_must_fix_override_ticket
        ...
        monkeypatch.setattr("cw.dev_queue.must_fix_override.record_event", _raise)
    ...
    def test_save_failure_after_event_is_logged_and_raised(
        self,
        tmp_config_dir: Path,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ...
        monkeypatch.setattr("cw.dev_queue.must_fix_override.save_dev_queue", _fail_save)

        with caplog.at_level("ERROR"), pytest.raises(OSError, match="disk full"):
            approve_must_fix_override_ticket("GEN-500", "genhealth", "why")

        assert len(events) == 1
        assert any("GEN-500" in r.message for r in caplog.records)
        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.must_fix_override is None
...
class TestTransitionTaskStatus:
    ...
    def test_clears_usage_limit_act_unconditionally(self) -> None:
        """Any transition clears the mid-turn usage-limit intent (#2324).

        The act's own final transition clears it in the same row write; any
        other transition dispositions the row out from under the act, which
        ends it.
        """
        ...
        for status in QueueItemStatus:
            ...
            transition_task_status(task, status, unproductive=False)
...
class TestStageRegress:
    ...
    def test_regress_into_review_clears_must_fix_override(self) -> None:
        """#2205: a regress into REVIEW means a re-review, which supersedes the
        verdict the operator's MUST_FIX override was bound to."""
        from cw.dev_queue import _stage_regress

        task = _make_stage_task(stage=Stage.FINALIZE)
        task.must_fix_override = _must_fix_override()
        _stage_regress(task, Stage.REVIEW)
        assert task.must_fix_override is None

    def test_regress_into_non_review_stage_keeps_must_fix_override(self) -> None:
        """Only a regress INTO REVIEW clears it; a FINALIZE->IMPL self-heal
        moves HEAD, which check_must_fix_override.py already refuses."""
        from cw.dev_queue import _stage_regress

        task = _make_stage_task(stage=Stage.FINALIZE)
        override = _must_fix_override()
        task.must_fix_override = override
        _stage_regress(task, Stage.IMPL)
        assert task.must_fix_override == override

    def test_must_fix_override_survives_pending_reset(self) -> None:
        """#2205: `requeue --stage finalize` resets the row to PENDING, which
        clears blocked_reason -- the override must outlive that reset."""
        from cw.dev_queue import transition_task_status

        task = _make_blocked_task(
            stage=Stage.REVIEW, blocked_reason="codex_must_fix_findings"
        )
        override = _must_fix_override()
        task.must_fix_override = override
        transition_task_status(task, QueueItemStatus.PENDING)
...
# ---------------------------------------------------------------------------
# Tracker-neutral plan approval + Linear-safe plan-of-record resolution
# (dev-queue schema v35; the #968/#1906 follow-up).
# ---------------------------------------------------------------------------


_LINEAR_PROJECT_CONFIG = "tracking:\n  primary:\n    system: linear\n"


def _assert_fetch_not_called(_ticket_id: str, **_kwargs: object) -> str | None:
    msg = (
        "fetch_approved_plan_comment must not be called for a known non-GitHub tracker"
    )
    raise AssertionError(msg)
...
class TestPlanApprovedAtStamp:
    ...
    def test_migrate_fills_plan_approved_at_default(self) -> None:
        """migrate_dev_queue fills plan_approved_at=None (v35)."""
        raw: dict[str, object] = {
            "schema_version": 34,
            "tasks": [
                {
            ...
            "tasks": [
                {
                    ...
                    "priority": 0,
                    "status": "pending",
                    "plan_approved_at": "2026-09-04T00:00:00+00:00",
                }
            ],
        }
        twice = migrate_dev_queue(migrate_dev_queue(raw))
        assert twice["tasks"][0]["plan_approved_at"] == "2026-09-04T00:00:00+00:00"
    ...
    def test_model_default_is_none_and_round_trips(self) -> None:
        ...
        assert restored.plan_approved_at == stamped

    @freeze_time("2026-09-04 12:00:00")
    def test_approve_requeue_branch_stamps_plan_approved_at(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """The #968 same-stage re-park still records the operator's approval:
        that is exactly the round the re-dispatched plan stage must see it."""
        from cw.dev_queue import approve_ticket
        ...
        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-stamp1")
        ...
        t = next(t for t in load_dev_queue().tasks if t.ticket_id == "GEN-500")
        assert t.status == QueueItemStatus.PENDING
        assert t.plan_approved_at == datetime(2026, 9, 4, 12, 0, 0, tzinfo=UTC)

    def test_approve_advance_branch_stamps_plan_approved_at(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_ticket
        ...
        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-stamp2")
    ...
    def test_approve_review_stage_does_not_stamp(
        self, tmp_config_dir: Path, tmp_path: Path
    ) -> None:
        """Only the PLAN gate is a plan approval; a REVIEW approve leaves the
        field alone."""
        from cw.config import save_state
        from cw.dev_queue import approve_ticket
        from cw.models import CwState
        ...
        _write_client_yaml(tmp_config_dir, tmp_path)
        task = _make_blocked_task(stage=Stage.REVIEW, session_id="sess-stamp3")
        save_dev_queue(DevQueueStore(tasks=[task]))
        session = _make_session(
            session_id="sess-stamp3",
            last_result={"status": "review_pending_approval"},
        )
        save_state(CwState(sessions=[session]))
...
class TestAdvisoryNoteMigration:
    """`advisory_note` is a persisted TicketTask field, so schema v37 carries
    a backfill for rows written before it existed (#1762)."""

    def test_migrate_fills_advisory_note_default(self) -> None:
        """migrate_dev_queue fills advisory_note=None on a v36 row (v37)."""
        raw: dict[str, object] = {
            "schema_version": 36,
            "tasks": [
                {
    ...
    def test_migrate_preserves_advisory_note_idempotently(self) -> None:
        ...
        twice = migrate_dev_queue(migrate_dev_queue(raw))
        assert twice["tasks"][0]["advisory_note"] == "?session_mismatch"

    def test_load_dev_queue_migrates_v36_file_without_advisory_note(
        self, tmp_config_dir: Path
    ) -> None:
        ...
        assert "advisory_note" not in v36_data["tasks"][0]
        dev_queue_file().parent.mkdir(parents=True, exist_ok=True)
        dev_queue_file().write_text(json.dumps(v36_data))
...
class TestUsageLimitActMigration:
    """`usage_limit_act` is the mid-turn usage-limit act's persisted intent, so
    schema v41 carries a backfill for rows written before it existed (#2324)."""

    _INTENT: ClassVar[dict[str, object]] = {
        "session_id": "sess-1",
        "branch": "auto",
        "started_at": "2026-09-24T01:00:00+00:00",
        "reset_at": None,
        "until": "2026-09-24T01:30:00+00:00",
    }

    def test_migrate_fills_usage_limit_act_default(self) -> None:
        """migrate_dev_queue fills usage_limit_act=None on a v39 row (v41)."""
        raw: dict[str, object] = {
            "schema_version": 39,
            "tasks": [
                {
            ...
            "tasks": [
                {
                    ...
                    "priority": 0,
                    "status": "running",
                    "usage_limit_act": dict(self._INTENT),
                }
            ],
        }
        twice = migrate_dev_queue(migrate_dev_queue(raw))
        assert twice["tasks"][0]["usage_limit_act"] == self._INTENT

    def test_load_dev_queue_migrates_v39_file_without_usage_limit_act(
        self, tmp_config_dir: Path
    ) -> None:
        """A v39 store on disk without the key loads as v41 with None."""
        from cw.config import dev_queue_file
    ...
    def test_v39_row_with_codex_orphan_link_migrates_to_v41(
        ...
        usage_limit_act key, gains usage_limit_act=None at v41 and keeps the
    ...
    def test_usage_limit_act_round_trips_through_the_store(
        ...
        intent = UsageLimitAct.model_validate(self._INTENT)
        task = TicketTask(
            ticket_id="GEN-39",
            client="test-client",
            status=QueueItemStatus.RUNNING,
        ...
        loaded = load_dev_queue().tasks[0].usage_limit_act
...
class TestPlanIsReviewedTrackerAware:
    """`_plan_is_reviewed` never asks GitHub about a Linear-tracked ticket, and
    resolves the real branch-derived worktree for dispatch-driven rows whose
    ``worktree_path`` is never stamped."""

    def _write_wt_plan(self, wt: Path, body: str) -> None:
        (wt / ".cw").mkdir(parents=True, exist_ok=True)
        (wt / ".cw" / "plan.md").write_text(body, encoding="utf-8")

    def _point_lifecycle_at(
        self,
        monkeypatch: pytest.MonkeyPatch,
        wt: Path,
        *,
        ...
        patch_worktree(monkeypatch, "worktree_path_for", lambda _client, _branch: wt)
        patch_worktree(monkeypatch, "_checked_out_branch", lambda _wt: checked_out)

    def test_linear_tracker_skips_fetch_and_reads_branch_worktree(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Linear client, reviewed plan on disk in the branch worktree, no
        worktree_path on the row: approve advances without a gh call."""
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-lin1")
        _write_project_config_yaml(tmp_path / "ws", _LINEAR_PROJECT_CONFIG)
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment",
            _assert_fetch_not_called,
        )
        wt = tmp_path / "wt-lin1"
        self._write_wt_plan(wt, plan_body())
        self._point_lifecycle_at(monkeypatch, wt)
    ...
    def test_linear_tracker_unreviewed_worktree_plan_requeues(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """The markers Step 1f writes after the gate are still required: a
        marker-less plan.md re-queues at plan stage (#968), gh untouched."""
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-lin2")
        _write_project_config_yaml(tmp_path / "ws", _LINEAR_PROJECT_CONFIG)
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment",
            _assert_fetch_not_called,
        ...
        wt = tmp_path / "wt-lin2"
        self._write_wt_plan(wt, plan_body(soundness=False))
        self._point_lifecycle_at(monkeypatch, wt)
    ...
    def test_linear_tracker_foreign_branch_worktree_is_not_trusted(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A worktree checked out on another branch must not lend its plan."""
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-lin3")
        _write_project_config_yaml(tmp_path / "ws", _LINEAR_PROJECT_CONFIG)
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment",
            _assert_fetch_not_called,
        ...
        wt = tmp_path / "wt-lin3"
        self._write_wt_plan(wt, plan_body())
        self._point_lifecycle_at(monkeypatch, wt, checked_out="dev/OTHER-1")

        result = approve_ticket("GEN-500", "genhealth")

        assert result["plan_requeued"] is True

    def test_linear_tracker_missing_worktree_requeues(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-lin4")
        _write_project_config_yaml(tmp_path / "ws", _LINEAR_PROJECT_CONFIG)
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment",
            _assert_fetch_not_called,
        )
        self._point_lifecycle_at(monkeypatch, tmp_path / "no-such-wt")

        result = approve_ticket("GEN-500", "genhealth")

        assert result["plan_requeued"] is True

    def test_github_tracker_still_fetches_first(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Regression: a positively-GitHub tracker keeps the tracker-first
        order -- the fetch is consulted and wins over a stale local file."""
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-gh1")
        _write_project_config_yaml(
        ...
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment", _fetch
        )
        wt = tmp_path / "wt-gh1"
        self._write_wt_plan(wt, plan_body(spec=False))
        self._point_lifecycle_at(monkeypatch, wt)

        result = approve_ticket("GEN-500", "genhealth")

        assert calls == ["GEN-500"]
        assert result["to_stage"] == "impl"

    def test_explicit_worktree_path_wins_over_branch_derivation(
        self, tmp_config_dir: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A stamped task.worktree_path is honored without touching the
        branch-derived lookup (USER-origin rows, existing fixtures)."""
        from cw.dev_queue import approve_ticket

        _seed_plan_pending(tmp_config_dir, tmp_path, session_id="sess-wt1")
        _write_project_config_yaml(tmp_path / "ws", _LINEAR_PROJECT_CONFIG)
        monkeypatch.setattr(
            "cw.dev_queue.lifecycle.fetch_approved_plan_comment",
            _assert_fetch_not_called,
        ...
        def _boom(_client: object, _branch: object) -> Path:
            msg = "worktree_path_for must not run when worktree_path is stamped"
            raise AssertionError(msg)

        patch_worktree(monkeypatch, "worktree_path_for", _boom)
        store = load_dev_queue()
    ...
    def test_local_plan_body_without_client_cfg_needs_worktree_path(
        self, tmp_path: Path
    ) -> None:
        """No client config and no stamped worktree_path -> no local plan;
        with worktree_path stamped the file is read directly."""
        from cw.dev_queue import _local_plan_body

        task = _make_blocked_task(stage=Stage.PLAN)
        assert _local_plan_body(task, None) is None

        task.worktree_path = tmp_path / "wt-nocfg"
        self._write_wt_plan(task.worktree_path, "# plan\n")
        assert _local_plan_body(task, None) == "# plan\n"

    def test_local_plan_body_read_error_degrades_to_none(self, tmp_path: Path) -> None:
        """`.cw/plan.md` being a directory (IsADirectoryError) -> None."""
        from cw.dev_queue import _local_plan_body

        task = _make_blocked_task(stage=Stage.PLAN)
        task.worktree_path = tmp_path / "wt-dir"
        (task.worktree_path / ".cw" / "plan.md").mkdir(parents=True)
        assert _local_plan_body(task, None) is None

    def test_tracker_allows_github_fetch_polarity(self, tmp_path: Path) -> None:
        from cw.dev_queue import _tracker_allows_github_fetch

        ws = tmp_path / "ws-polarity"
        ws.mkdir()
        cfg = ClientConfig(name="c", workspace_path=ws)
        assert _tracker_allows_github_fetch(None) is True
        assert _tracker_allows_github_fetch(cfg) is True  # unresolvable
        _write_project_config_yaml(ws, _LINEAR_PROJECT_CONFIG)
        assert _tracker_allows_github_fetch(cfg) is False
        _write_project_config_yaml(
            ws, "tracking:\n  primary:\n    system: github-issues\n"
        )
