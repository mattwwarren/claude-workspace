...
class TestUsageLimitActMigration:
    ...
    schema v39 carries a backfill for rows written before it existed (#2324)."""
    ...
    def test_migrate_fills_usage_limit_act_default(self) -> None:
        """migrate_dev_queue fills usage_limit_act=None on a v38 row (v39)."""
        raw: dict[str, object] = {
            "schema_version": 38,
            "tasks": [
                {
    ...
    def test_load_dev_queue_migrates_v38_file_without_usage_limit_act(
        self, tmp_config_dir: Path
    ) -> None:
        """A v38 store on disk without the key loads as v39 with None."""
        from cw.config import dev_queue_file
