...
class TestCheckConfigFileLoaderFailure:
    ...
    def test_yaml_error_from_load_clients_is_reported_as_fail(
        ...
        monkeypatch.setattr("cw.doctor._deps.load_clients", fake_load_clients)
...
class TestCheckDevQueueLoaderFailure:
    ...
    def test_json_decode_error_from_load_dev_queue_is_reported_as_fail(
        ...
        monkeypatch.setattr("cw.doctor._deps.load_dev_queue", fake_load_dev_queue)
...
class TestCheckWorkspacePaths:
    ...
    def test_load_clients_exception_returns_empty(
        ...
        monkeypatch.setattr("cw.doctor._deps.load_clients", boom)
        results = _check_workspace_paths()
        assert results == []
...
class TestWedgeReapRecipes:
    ...
    def test_reap_false_no_mutations(
        ...
        monkeypatch.setattr(
            "cw.doctor.core._check_wedge_task_running_no_session",
            lambda *_a, **_kw: [],
        )
        monkeypatch.setattr(
            "cw.doctor.core._check_wedge_task_running_completed_session",
            lambda *_a, **_kw: [],
        )
        monkeypatch.setattr(
            "cw.doctor.core._check_wedge_repo_ahead",
            lambda *_a, **_kw: [],
        )
...
class TestCheckProjectConfigs:
    ...
    def test_github_issues_with_gh_present_is_ok(
        ...
        monkeypatch.setattr("cw.doctor.config_checks._gh_on_path", lambda: True)
    ...
    def test_github_issues_without_gh_warns(
        ...
        monkeypatch.setattr("cw.doctor.config_checks._gh_on_path", lambda: False)
        self._write_config(tmp_path, "github-issues")
        results = _check_project_configs({"client-a": self._client(tmp_path)})
        assert results[0].ok is True
    ...
    def test_gh_on_path_true_when_which_resolves(
        ...
        monkeypatch.setattr(
            "cw.doctor.config_checks.shutil.which", lambda _n: "/usr/bin/gh"
        )
    ...
    def test_gh_on_path_false_when_which_none(
        ...
        monkeypatch.setattr("cw.doctor.config_checks.shutil.which", lambda _n: None)
...
class TestWedgeDeadSessionBlockedOnUser:
    ...
    def test_blocked_on_user_duplicate_ticket_collapse(
        ...
        """3 tasks same ticket; oldest → PENDING, newer duplicates → CANCELLED."""
        from cw.config import save_state
        from cw.dev_queue import load_dev_queue, save_dev_queue
        from cw.models import CwState, DevQueueStore, QueueItemStatus
        from cw.native_daemon import FakeNativeDaemonClient
        ...
        save_dev_queue(DevQueueStore(tasks=[t1, t2, t3]))
    ...
    def test_blocked_on_user_reap_session_by_selector_extends_to_blocked(
        ...
        """_reap_session_by_selector reverts BLOCKED_ON_USER when no RUNNING task."""
        from cw.config import load_state, save_state
        from cw.dev_queue import load_dev_queue, save_dev_queue
        from cw.doctor import _reap_session_by_selector
        from cw.models import (
        ...
        sess = Session(
            id="sel-sess-1",
            name="client-a/auto-dev/sel-ticket-1",
            client="client-a",
            purpose=SessionPurpose.IMPL,
            status=SessionStatus.ACTIVE,
            workspace_path=Path("/tmp/ws"),
    ...
    def test_blocked_on_user_duplicate_via_reap_session_collapse(
        ...
        """_reap_session_by_selector collapses duplicate BLOCKED_ON_USER tasks."""
        from cw.config import save_state
        from cw.dev_queue import load_dev_queue, save_dev_queue
        from cw.doctor import _reap_session_by_selector
        from cw.models import (
        ...
        sess = Session(
            id="dup-sess-1",
            name="client-a/auto-dev/dup-ticket-1",
            client="client-a",
            purpose=SessionPurpose.IMPL,
            status=SessionStatus.ACTIVE,
            workspace_path=Path("/tmp/ws"),
        ...
        save_dev_queue(DevQueueStore(tasks=[t1, t2]))
    ...
    def test_blocked_on_user_dead_session_via_run_doctor_reap(
        ...
        """Integration: state + queue + daemon → run_doctor(reap=True) → PENDING."""
        from cw.config import save_state
        from cw.dev_queue import load_dev_queue, save_dev_queue
        from cw.models import CwState, DevQueueStore, QueueItemStatus
        from cw.native_daemon import FakeNativeDaemonClient
...
class TestCheckDispatchRepoHead:
    ...
    def test_on_default_branch_returns_empty(
        ...
        monkeypatch.setattr("cw.doctor.linkage.get_head_branch", lambda _: "main")
    ...
    def test_off_default_branch_returns_warn(
        ...
        monkeypatch.setattr("cw.doctor.linkage.get_head_branch", lambda _: "feature-x")
    ...
    def test_detached_head_returns_warn(
        ...
        workspace = self._write_client(tmp_config_dir, tmp_path)
        monkeypatch.setattr("cw.doctor.linkage.get_head_branch", lambda _: None)
    ...
    def test_oserror_returns_warn_no_crash(
        ...
        monkeypatch.setattr("cw.doctor.linkage.get_head_branch", _raise)
    ...
    def test_run_doctor_wires_check(
        ...
        _stub_claude_version_ok(monkeypatch)
        self._write_client(tmp_config_dir, tmp_path)
        monkeypatch.setattr("cw.doctor.linkage.get_head_branch", lambda _: "hotfix")
...
class TestCheckCrossRepoRows:
    ...
    def test_queue_load_failure_returns_empty(
        ...
        monkeypatch.setattr("cw.doctor._deps.load_dev_queue", _boom)
...
def _patch_config_enabled(monkeypatch: pytest.MonkeyPatch, **kwargs: Any) -> None:
    """Point cw.doctor.config_checks.load_orchestrator_config at an enabled config."""
    from cw.models import OrchestratorConfig
...
class TestCheckReviewRecipeLiveness:
    ...
    def test_orchestrator_config_load_failure_degrades_to_default(
        ...
        monkeypatch.setattr("cw.doctor.config_checks.load_orchestrator_config", _boom)
        save_dev_queue(DevQueueStore(tasks=[_cr_liveness_task()]))
