...
def _captured_windows(kwargs: dict[str, object]) -> Mapping[str, datetime]:
    """Read the per-client back-off mapping out of a spied dispatch_tick call.

    ``usage_limited_until`` is a ``Mapping[str, datetime]`` since #1409 and the
    loop always passes one (empty when nothing is limited), so an absent kwarg
    is a spy-wiring bug rather than a state to assert on -- hence the raise
    instead of a ``.get`` default that would quietly read as "no windows".
    """
    windows = kwargs.get("usage_limited_until")
    if not isinstance(windows, dict):
        msg = f"dispatch_tick was called without a window mapping: {windows!r}"
        raise TypeError(msg)
    return windows
...
# ---------------------------------------------------------------------------
# TestDispatchTickAutoBypassesApprovedPlan -- #1286
# ---------------------------------------------------------------------------
...
class TestDispatchTickAutoBypassesApprovedPlan:
    ...
    ``Stage.PLAN`` -- a fresh ``cw dev-queue add`` included -- re-ran a full
    Stage 1 pass from scratch even when a valid, approved plan was already
    sitting in the ticket's reused worktree.
    """
    ...
    def test_plan_stage_claim_bypasses_to_impl_when_plan_already_signed_off(
        ...
        """A reused worktree carrying a signed-off .cw/plan.md makes the very
        next dispatch spawn /auto-dev-impl, not /auto-dev-plan -- and the
        advance is persisted to the stored row, not just the in-memory task.

        A second tick against the now-IMPL row does not re-run the bypass
        check or re-emit a second TASK_STAGE_CHANGED event for this ticket.
        """
        from cw.worktree import create_worktree
        from tests.conftest import plan_body
        ...
        cw_dir.mkdir(parents=True, exist_ok=True)
        (cw_dir / "plan.md").write_text(plan_body(), encoding="utf-8")
        add_ticket(TicketTask(ticket_id="GEN-BYPASS", client="test-client"))
        ...
        assert spawned == 1
        assert daemon.spawn_calls[0][1] == "/auto-dev-impl GEN-BYPASS --headless"

        running = load_dev_queue().running()
        assert len(running) == 1
        assert running[0].stage == Stage.IMPL

        # Persisted row, reloaded fresh -- proves the advance reached
        # dev_queue.json, not just the in-memory task object handed to the
        # executor.
        reloaded = load_dev_queue()
        stored = next(t for t in reloaded.tasks if t.ticket_id == "GEN-BYPASS")
        assert stored.stage == Stage.IMPL

        bypass_events = [p for _, p, cid in stage_changed if cid == "GEN-BYPASS"]
        assert len(bypass_events) == 1
        assert bypass_events[0]["old_stage"] == Stage.PLAN
        assert bypass_events[0]["new_stage"] == Stage.IMPL
        assert bypass_events[0]["direction"] == "advance"
    ...
    def test_plan_stage_claim_no_bypass_when_no_plan_md(
        ...
        """Ordinary fresh-ticket path: no .cw/plan.md anywhere -> no bypass,
        and no network call is added to the common path.

        ``fetch_approved_plan_comment`` is stubbed to raise if called at all:
        the automatic dispatch call site passes ``allow_tracker_fallback=
        ...
        add_ticket(TicketTask(ticket_id="GEN-FRESH", client="test-client"))

        daemon = FakeNativeDaemonClient()
        spawned = dispatch_tick(simple_config, native_daemon=daemon).spawned

        assert spawned == 1
        assert daemon.spawn_calls[0][1] == "/auto-dev-plan GEN-FRESH --headless"
    ...
    def test_plan_stage_claim_no_bypass_when_plan_unsigned(
        ...
        """A .cw/plan.md is present but missing the soundness signoff marker
        -> not a bypass; the worker still spawns at PLAN."""
        from cw.worktree import create_worktree
        from tests.conftest import plan_body
        ...
        assert spawned == 1
        assert daemon.spawn_calls[0][1] == "/auto-dev-plan GEN-UNSIGNED --headless"
    ...
    def test_task_stage_changed_event_emitted_on_auto_bypass(
        ...
        """A TASK_STAGE_CHANGED(old=PLAN, new=IMPL, direction=advance) event
        is recorded on the stored row's auto-bypass advance."""
        from cw.worktree import create_worktree
        from tests.conftest import plan_body
        ...
        cw_dir.mkdir(parents=True, exist_ok=True)
        (cw_dir / "plan.md").write_text(plan_body(), encoding="utf-8")
        add_ticket(TicketTask(ticket_id="GEN-EVT", client="test-client"))
        ...
        events = [
            (etype, payload)
            for etype, payload, cid in stage_changed
            if cid == "GEN-EVT"
        ]
        assert len(events) == 1
    ...
    def test_plan_stage_bypass_advances_only_the_row_matching_created_at(
        ...
        """#1286 round 4: the bypass's locked re-find must disambiguate
        duplicate RUNNING rows for one (ticket_id, client) by ``created_at``.

        Row A (earlier, REVIEW) precedes row B (later, PLAN) in the store. The
        bypass is invoked for B; a first-match (ticket_id, client, RUNNING)
        predicate would advance A -- stamping IMPL and raising the high-water
        mark on a row doing unrelated work -- and leave B at PLAN.
        """
        import cw.dispatch.claim as claim_mod
        ...
        task = row_b.model_copy()
        claim_mod._apply_plan_bypass_if_available(task, sample_client_config, worktree)

        assert task.stage == Stage.IMPL
        stored = {t.created_at: t for t in load_dev_queue().tasks}
        assert stored[row_a.created_at].model_dump() == row_a_before
        assert stored[row_b.created_at].stage == Stage.IMPL
        assert stored[row_b.created_at].stage_high_water == Stage.IMPL

        events = [p for _, p, cid in stage_changed if cid == "GEN-DUP"]
        assert len(events) == 1
        assert events[0]["old_stage"] == Stage.PLAN
        assert events[0]["new_stage"] == Stage.IMPL
        assert events[0]["direction"] == "advance"
    ...
    def test_plan_stage_bypass_no_advance_when_no_row_matches_created_at(
        ...
        """#1286 round 4: a RUNNING row for the same (ticket_id, client) whose
        ``created_at`` differs from the task's is NOT the task's row -- the
        ...
        task = row_b.model_copy(update={"created_at": row_b.created_at + timedelta(1)})
        claim_mod._apply_plan_bypass_if_available(task, sample_client_config, worktree)

        assert task.stage == Stage.PLAN
        assert [t.model_dump() for t in load_dev_queue().tasks] == before
        assert [p for _, p, cid in stage_changed if cid == "GEN-DUP"] == []
...
def _seed_occupied_ticket_worktree(
    client: ClientConfig,
    monkeypatch: pytest.MonkeyPatch,
    source: str,
    *,
    ...
    """Provision *ticket_id*'s per-ticket worktree, then occupy it (#2213,
    promoted to module scope for #2077 -- both TestClaimRefusesOccupiedWorktree
    (post-claim path) and TestClaimScreensOccupiedWorktreeBeforeClaiming
    (pre-claim screen) seed occupancy identically; only which mechanism
    catches it differs.
...
class TestClaimRefusesOccupiedWorktree:
    ...
    Since #2077 a ``dispatch_tick`` never even claims such a row: its pre-claim
    occupancy screen catches it first, so the ``dispatch_tick``-based tests here
    now exercise that screen. ``test_outcome_flags_the_skip_without_a_spawn_error``,
    ``test_occupied_after_fast_forward_leaves_head_at_the_new_sha`` and
    ``test_a_deferred_head_of_line_ticket_does_not_starve_the_next_one``
    deliberately call ``_claim_next_pending``/``_spawn_claimed_task`` directly
    (bypassing ``dispatch_tick``'s precompute) to keep exercising the post-claim
    ``_defer_occupied_claim`` safety net in isolation.
    """
    ...
    def test_occupied_worktree_is_not_spawned_into(
        ...
        # charged, no spawn error stamped. Since #2077 the pre-claim occupancy
        # screen catches it before any claim is attempted, so no release hold
        # is stamped either -- the next tick's fresh screen reconsiders it.
        task = load_dev_queue().tasks[0]
        assert task.status == QueueItemStatus.PENDING
        assert task.session_id is None
        assert task.attempts == 0
    ...
    def test_the_skip_never_trips_the_lane_circuit_breaker(
        ...
        # The breaker threshold is 2; go well past it. Since #2077 the
        # pre-claim occupancy screen refuses the row before any claim, so no
        # release hold is stamped and every tick genuinely re-screens it.
        for _ in range(4):
    ...
    def test_next_stage_is_refused_while_the_prior_stage_is_live_then_spawns(
        ...
        assert len(daemon.spawn_calls) == 1
        # Refused BY the occupancy check (not by a capacity gate): since #2077
        # the pre-claim occupancy screen catches it before any claim, so the
        # row stays PENDING with no attempt charged and no release hold.
        assert load_dev_queue().tasks[0].next_eligible_at is None
...
# ---------------------------------------------------------------------------
# TestClaimScreensOccupiedWorktreeBeforeClaiming (#2077)
# ---------------------------------------------------------------------------


class TestClaimScreensOccupiedWorktreeBeforeClaiming:
    """#2077: the claim path must check worktree availability BEFORE
    claiming a ticket and transitioning it to RUNNING, not discover the
    conflict only after the claim (WorktreeOccupiedError,
    TestClaimRefusesOccupiedWorktree) or during the executor's own
    hook-context write (HookContextConflictError). Uses the SAME
    live_home_reason predicate as both of those post-claim mechanisms,
    precomputed once per client per tick (mirrors the #1862 stale-PR-
    ticket-ids precompute) and threaded into _claim_next_pending -- so a
    row whose worktree a live session or daemon worker already holds is
    never even transitioned to RUNNING: no attempt charged, no spawn
    attempted, no defer/backoff stamp needed (a fresh precompute runs
    every tick, so the row is simply reconsidered next tick without a
    next_eligible_at hold)."""
    ...
    def test_occupied_worktree_is_never_claimed(
        ...
        assert any(
            self._TICKET in m
            and str(worktree) in m
            and _OCCUPANT_REASON[source] in m
            and "not claimed" in m
            for m in messages
        ), messages

        events = read_events(
            consumer="test-2077-occupied-screen",
            event_types=[OrchestratorEventType.DISPATCH_TICK],
        )
        occupied_events = [
            e
        ...
        assert occupied_events[0].payload["ticket_id"] == self._TICKET
        assert occupied_events[0].payload["client"] == "test-client"
    ...
    def test_the_screen_does_not_starve_a_free_sibling_in_the_same_tick(
        ...
        """Unlike the post-claim defer (#2213), no next_eligible_at hold is
        needed to protect a free sibling: the occupied ticket is simply
        never claimed in the first place, so the SAME
        _claim_next_pending call's internal loop moves past it to the
        ...
        assert tasks[self._TICKET].status == QueueItemStatus.PENDING
        assert tasks[self._TICKET].attempts == 0
...
class TestStaleWorktreeYieldsToLiveOccupant:
    ...
    def _assert_released_not_charged(self, stale_tree: Path) -> None:
        """The row stays PENDING, never claimed at all (#2077's pre-claim
        occupancy screen) -- not a failure or a park."""
...
class TestDispatchUsageLimitBackoff:
    ...
    def test_usage_limited_until_future_skips_all_clients(
        ...
        """A client whose own window is in the future is skipped (#1409).

        The single client here IS the whole fleet, so this also pins the
        every-client-limited early return: ``usage_limit_detected`` stays
        False rather than being re-armed.
        """
        ...
        result = dispatch_tick(
            simple_config,
            native_daemon=daemon,
            usage_limited_until={"test-client": future},
        )
    ...
    def test_usage_limited_until_elapsed_spawns_normally(
        ...
        result = dispatch_tick(
            simple_config,
            native_daemon=daemon,
            usage_limited_until={"test-client": past},
        )
    ...
    def test_run_dispatch_loop_persists_usage_limited_until(
        ...
        assert len(saved) >= 1
        assert set(saved[0]) == {"test-client"}
        assert saved[0]["test-client"] > datetime.now(UTC)
    ...
    def test_run_dispatch_loop_observes_backoff_written_by_another_process_mid_loop(
        ...
        diverges from what it loaded at startup (empty here), so it would keep
        ...
        def observing_tick(*args: object, **kwargs: object) -> DispatchTickResult:
            nonlocal call_count
            call_count += 1
            captured.append(_captured_windows(kwargs))
            if call_count == 3:
                raise KeyboardInterrupt
        ...
        assert captured[0] == {}
        assert len(daemon.spawn_calls) == 1
        # Tick 2: the merge picked up the other process's write for this
        # client -- still-future, and no additional spawn occurred.
        assert captured[1]["test-client"] > datetime.now(UTC)
        assert len(daemon.spawn_calls) == 1
    ...
    def test_run_dispatch_loop_corrupt_sidecar_mid_backoff_does_not_shorten_window(
        ...
        load_usage_limited_until() returns an EMPTY mapping for a file that is
        absent, unreadable, OR malformed (dispatch_state.py) -- a bare
        assignment in the per-tick merge would let a transient disk-read
        failure silently resurrect spawning during an active window. The merge
        must be miss-safe per client: only a later PERSISTED value can extend
        a client's window, never shorten or clear an active in-memory one
        (#1346; per-client since #1409).
        ...
        def corrupting_tick(*args: object, **kwargs: object) -> DispatchTickResult:
            ...
            if call_count == 1:
                ...
                # load_usage_limited_until() degrades to an empty mapping.
        ...
        assert captured[1]["test-client"] > datetime.now(UTC)
        # Tick 3: disk read degrades to an empty mapping (corrupt), but the
        # merge must NOT drop the still-active in-memory window built on
        # tick 2 -- still present, still future, still no new spawn.
        assert captured[2]["test-client"] > datetime.now(UTC)
        assert len(daemon.spawn_calls) == 1
    ...
    def test_run_dispatch_loop_expired_disk_window_does_not_resurrect_backoff(
        ...
        in-memory backoff (#1346). load_usage_limited_until() already omits
        a past timestamp (dispatch_state.py contract), so the merge's
        take-the-later logic never treats an expired persisted value as
        eligible to extend anything -- the entry keeps its lapsed value, and
        the gate falls through to its own now-vs-deadline check, which lets
        tick 2 spawn normally."""
        from freezegun import freeze_time
        ...
        assert captured[0] == {"test-client": expiring}
    ...
    def test_usage_limit_cleared_not_emitted_when_window_never_active(
        ...
        now_active = cw.dispatch.loop._handle_usage_limit_window_transition(
            False, usage_limited_until={}, armed_at=None
        )
...
class TestUsageLimitResetThreading:
    ...
    def test_reset_at_threads_from_spawn_to_tick_result(
        ...
        assert result.usage_limit_detected is True
        assert result.usage_limit_clients == {"test-client": reset_at}
    ...
    def test_reset_at_defaults_to_none_on_tick_result(
        ...
        assert result.usage_limit_detected is True
        assert result.usage_limit_clients == {"test-client": None}
    ...
    def test_reconcile_path_keeps_flat_backoff(
        ...
        It names no client, so every KNOWN client is armed on the flat
    ...
    def test_loop_persists_resolved_window(
        ...
        assert saved[0] == {"test-client": expected}
