"""Tests for the flat ``cw watch`` board TUI.
...
-- client names, ticket IDs, worktree columns, and so on. The
orchestrator-dashboard render stack moved to ``cw board`` (issue #986);
only the ``cw watch`` flat-board surface remains here.
"""
