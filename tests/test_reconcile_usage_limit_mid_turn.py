...
def test_act_auto_reverts_row_completes_session_and_arms_lockout(
    ...
    def _recording_record_event(
        etype: OrchestratorEventType,
        payload: dict[str, Any] | None = None,
        *,
        correlation_id: str | None = None,
    ) -> None:
        order.append(etype.value)
    ...
    monkeypatch.setattr(
        "cw.reconcile.usage_limit_mid_turn.record_event", _recording_record_event
    )
    ...
    assert order.index(OrchestratorEventType.SESSION_COMPLETED.value) < order.index(
        "stop"
    )
...
def test_act_arms_through_shared_helper_and_audits_before_persisting(
    tmp_config_dir: Path,
    tmp_path: Path,
    home: Path,
    daemon: FakeNativeDaemonClient,
    ...
    with pytest.raises(OSError, match="inbox write failed"):
        _act(state, candidates, OrchestratorConfig())
