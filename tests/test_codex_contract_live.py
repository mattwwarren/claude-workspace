...
Snap confinement (R12): a snap-installed ``codex`` cannot read ``/tmp`` (snap
...
Diagnostics (R9) here are a structured-log + job-summary stopgap; re-wire into
...
class _RecordingCodexRunner(RealCodexRunner):
    ...
    def assert_clean(self) -> None:
        """Scan the last captured subprocess output for leaked secrets (R10)."""
...
def live_base() -> Iterator[Callable[[], Path]]:
    ...
    Snap-confined codex cannot reach ``/tmp`` (R12), so fixture repos and
...
class TestCodexContractSchemaEnforcement:
    """Even a prompt that asks codex to violate the schema still round-trips (R11).
...
class TestCodexContractMissingOutput:
    """A non-writable ``-o`` target yields exactly one failure, no crash (R13)."""
    ...
    def test_missing_output_file(
        ...
        diff, _sha, _files = _capture_diff(repo, "main")
        # Point the scratch dir at a non-existent subtree so the -o write path
        # cannot resolve; _run_codex_role must degrade, not raise.
        missing_scratch = base / "does-not-exist" / uuid.uuid4().hex
        doc, failure = _run_codex_role(
            runner=_RecordingCodexRunner(),
            worktree=repo,
            role=_ROLE,
            ...
            model=None,
            timeout_seconds=120,
            scratch_dir=missing_scratch,
        )
        # Tolerant per R13: npm-installed codex versions differ on exit code.
        assert doc is None
...
class TestCodexContractTimeout:
    """A ~2s timeout against a real invocation drives the subprocess-kill path (R8)."""
...
class TestCodexContractDiagnostics:
    """Record the installed codex version + capability outcome via a log (R9)."""
