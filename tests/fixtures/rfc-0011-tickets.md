...
- **Sprint:** 0
- **Depends on:** none
- **Context:** A single axis read by both epics — self vs external — derived at `pr_hydrate` from PR authorship rather than a stored field, so it never drifts; a hold with no PR is always self, and external cannot arise without a review request.
- **Scope:** D-S1
- **Acceptance:**
  - A hold with no PR resolves to `counterparty=self`.
  - Auto-dev's own PRs resolve to `counterparty=self` by construction, since `_is_candidate` already fences the auto-dev PR set.
  - The operator's GitHub login is resolved at runtime, cached, and overridable via `clients.yaml` for the multi-account case.

### S2 — native review-request register

- **Epic:** none
- **Wave:** 0
- **Sprint:** 0
- **Depends on:** S1
- **Context:** The narrowest possible slice of the deferred `register` store — a new `review_requested` webhook type (plus a `cw review register <pr>` CLI entry) that fires only when the requested reviewer is the operator's own resolved identity, with no `discover` scan and no ported thread-state machine.
- **Scope:** D-S2a, D-S2b
- **Acceptance:**
  - The `review_requested` trigger (webhook or CLI) fires only when the requested reviewer is the operator's own resolved identity.
  - A team-review request the operator merely belongs to is not registered.
  - A registered PR becomes a sibling watched-PR record, not a `TicketTask` extension, and consumes no dispatch slot.
...
- **Sprint:** 1
- **Depends on:** S1
- **Context:** A hold caused by operator/dependency unavailability carries a distinct disposition from a generic blocked park, still routing to BLOCKED_ON_USER but tagged so the attention layer and resume path can treat it as waiting-on-you rather than broken.
- **Scope:** D-A1
- **Acceptance:**
  - A held task routes to BLOCKED_ON_USER with a distinct disposition tag — no new QueueItemStatus, no parser bump.
  - The new `blocker.reason` value stays out of `FINALIZE_REGRESS_BLOCKER_REASONS`.
  - `push_auth_failed` is retro-classified as the first instance of this class.

### A2 — unavailability detector

- **Epic:** I
- **Wave:** 1
- **Sprint:** 1
- **Depends on:** A1
- **Context:** Generalize the fixed 4-signature push-auth detector into a shared classifier over the full unavailability failure family, applied wherever a leg touches the remote or the gh/MCP-github surface.
- **Scope:** D-A2
- **Acceptance:**
  - The shared classifier covers network-unreachable, GitHub 5xx/secondary-rate-limit, auth-failure, MCP-github-unreachable, and classifier-deny (`tool_denied`) failures.
  - Rollout is staged surface-by-surface: push sites first, then intake / hygiene / tracker / CI-poll.
...
- **Sprint:** 1
- **Depends on:** A1
- **Context:** A cheap probe at dispatch/tick time so a session never spins up just to die against a down remote or expired auth; the queue holds instead.
- **Scope:** D-A5
- **Acceptance:**
  - The probe runs at dispatch time and again immediately before finalize, not on every reconcile tick.
  - A down remote/auth holds at the queue level instead of spinning up a session that will just die.
  - The probe result is cached with a short (~1 minute) TTL.

### A3 — stop-before-finalize hold

- **Epic:** I
- **Wave:** 2
- **Sprint:** 2
- **Depends on:** none
- **Context:** A per-run flag or per-client config that, for an operator-away wave, runs intake through review to completion and holds every leg at the Stage 3→4 boundary as awaiting_operator, emitting one digest instead of N noisy per-leg auth walls.
- **Scope:** D-A3
- **Acceptance:**
  - `--hold-finalize` and `finalize_gate: manual` both force a hold at the Stage 3→4 boundary even when auth is live at dispatch.
  - A force-hold also holds Small-scope tickets, which never park today.
  - One digest is emitted instead of N noisy per-leg auth walls.
...
- **Sprint:** 2
- **Depends on:** none
- **Context:** A drain command that re-fires every awaiting_operator ticket through the existing --resume <ticket> path, closing the loop from overnight hold to a one-command morning resume.
- **Scope:** none
- **Acceptance:**
  - `cw dev-queue resume --held` re-fires every `awaiting_operator` ticket through the existing `--resume <ticket>` path.
  - The resume path jumps to `detect_current_stage()` with no session file required.

### A6 — digest / batch on attention channel

- **Epic:** I
- **Wave:** 2
- **Sprint:** 2
- **Depends on:** none
- **Context:** Coalesce N awaiting_operator parks into one operator signal rather than N session.needs_attention pushes at 3am, and double as the delivery vehicle for B5's rejection flags.
- **Scope:** D-A6
- **Acceptance:**
  - N `awaiting_operator` parks coalesce into one operator signal instead of N separate pushes.
  - Genuine `blocked`/broken parks stay immediate and are never delayed by the coalescer.
  - The digest is also the delivery vehicle for B5's rejection flags.
...
- **Sprint:** 1
- **Depends on:** S1
- **Context:** A watched-PR session whose counterparty is external is exempt from the flat idle-reap floor, since reaping it would silently block a teammate who is waiting on our review.
- **Scope:** none
- **Acceptance:**
  - A watched-PR session whose counterparty is `external` is exempt from silent idle-reap.
  - A quiet review-of-a-teammate's-PR session surfaces an escalation instead of being reaped.

### B2 — two-party consent gate

- **Epic:** II
- **Wave:** 1
- **Sprint:** 1
- **Depends on:** S1
- **Context:** Consent to act toward a teammate's PR is operator config-enablement crossed with the target's own explicit in-band GitHub action, never a registry or opt-in a teammate must adopt.
- **Scope:** none
- **Acceptance:**
  - Outbound acting toward a teammate's PR requires both the operator's cw-settings enablement and the target's own explicit in-band GitHub action.
  - The operator's own authored PRs remain unrestricted by this gate.

### B3 — individual re-request gate

- **Epic:** II
- **Wave:** 2
- **Sprint:** 2
- **Depends on:** none
- **Context:** A re-review fires only on an explicit individual re-request; new commits pushed to an already-reviewed PR do not auto-retrigger, and a team re-request is never our action point.
- **Scope:** none
- **Acceptance:**
  - A re-review fires only on an explicit individual re-request, never on new commits alone.
  - A team re-request is never an action point.
...
- **Sprint:** 2
- **Depends on:** none
- **Context:** The review we submit is succinct, answer-first, and self-fact-checked before posting — each claim verified against the diff/CI/source, with an unverifiable claim dropped or flagged rather than holding the whole review.
- **Scope:** D-B4
- **Acceptance:**
  - Each posted review is self-fact-checked against diff/CI/source before it ships.
  - An unverifiable claim is dropped or flagged rather than holding the whole review on it.
...
- **Sprint:** 2
- **Depends on:** none
- **Context:** If the target rejects or dismisses a section of our review, the default is to accept it with no push-back reply, and instead emit an inbound operator signal so follow-up work is captured.
- **Scope:** none
- **Acceptance:**
  - A rejected or dismissed review section is accepted with no push-back reply.
  - Rejection instead emits an inbound operator signal so follow-up work is captured.
