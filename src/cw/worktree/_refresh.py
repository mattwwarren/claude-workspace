...
class ReuseRefreshReport:
    ...
    - ``notes``: one single-line entry per refresh FAILURE the caller cannot
      ...
      absent from origin with no commits of its own, already equal or ahead,
      or a worktree that is occupied. One exception (#2328): a branch absent
      from origin that DOES have commits of its own also gets a note, even
      though leaving it untouched is the designed action -- the reader (a
      fix_agent prompt, a later pipeline stage) needs to know it may be
