...
_GC_VERDICT_REASON: dict[GcVerdict, str] = {
    ...
    GcVerdict.SKIP_MAIN: "main checkout",
}
...
@handle_errors
def worktree_gc(apply: bool, client_name: str | None, timeout: int) -> None:
    """GC worktrees for squash-merged or closed branches.

    Checks each worktree branch's PR state via the gh CLI and removes
    worktrees where the PR is MERGED or CLOSED. Dry-run by default;
    pass --apply to act. Locked worktrees are always skipped.
    """
    ...
    report = run_worktree_gc(git_cwd, apply=apply, timeout=timeout)
