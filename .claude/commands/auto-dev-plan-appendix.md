...
**Bookkeeping-line grammar.** `plan-stage-resolutions-applied` is a leading bookkeeping line in `.cw/plan-draft.md`, recording the identity of the authoritative resolutions source last folded into the draft. Step 1a.0b also uses two leading bookkeeping lines for its durable attempt and approval-invalidation state:
...
<!-- plan-stage-resolutions-attempted: source=comment:<id>; outcome=started|failed|succeeded -->
<!-- plan-stage-resolutions-attempted: source=body:<sha>; outcome=started|failed|succeeded -->
<!-- plan-stage-approval-revoked: at=<UTC>; fingerprint=<sha> -->
```

Ordinal position: the `plan-stage-resolutions-attempted` line, when present, follows every `plan-stage-settled` marker; the `plan-stage-approval-revoked` line follows it; and the `plan-stage-resolutions-applied` line remains always last, per the Bookkeeping-line order rule (`.claude/commands/auto-dev-plan.md`, Settlement marker grammar section). If the applied line is absent, the other two lines remain at the tail in that order.
