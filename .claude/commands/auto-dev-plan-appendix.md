...
**Step 1c.0 — Round-cap read + settlement folding (resumed rounds only).** Fires only when Step 1a.0's resume branch fired this dispatch; on a fresh (non-resumed) dispatch skip straight to step 1 below.
   ...
   7. **Body-consistency fold (resumed rounds only, #2154).** Fires when (a) step 5 above settled ≥1 item in this round's own transcription pass, OR (b) the newest `## Pending Verification Scan` park comment's `### Advisory plan-review findings (address in the same round)` sub-section carries a persisting MUST_FIX from either station. Re-spawns the **Plan** agent (`model: "sonnet"`), scoped to mechanically rewrite `## Files Modified`, Phase 1, Phase 2, and `## Touch-point Contract` for consistency with `## Decisions` / `## Settled Plan Items` / `## Adopted Assumptions` / `## Self-Verified Premises` / `## Deferred Premises` — forbidding new scope and forbidding any edit to `## Ambiguities` or the premises sections (those are Step 1c's own territory, not this fold's). Tracked on an **independent axis** from, and does **not** decrement, Step 1f.4's standard 1-cycle revision budget — mirrors Step 1f.4's Format-only revision shape. Capped at **1 attempt** per round. Write the returned plan text back to `.cw/plan-draft.md`, preserving the leading bookkeeping lines (round counter, fingerprint, settlement markers) untouched. Runs BEFORE step 1 above (source the ambiguity list) — i.e., before this round's fresh scan, so the scan evaluates a body already consistent with what was settled.
