...
Either source is sufficient **on its own terms** — the comment path is gated by the token, no-double-duty, and staleness rules below, the row path by the fingerprint match above; neither is a mere presence check. Absent both, EXIT `plan_pending_approval` again (persist the draft, reference the existing park comment rather than re-posting the full draft — do not re-run the advisory stations just to re-ask). A resumed draft that was parked for approval must not slip through approval by being resumed.

**Row-path alternate sufficient condition (operator-authority delta, #2433).** Independent of the fingerprint-equality clause above: a non-null `plan_approved_at` AND no operator-authority delta since `plan_approved_at` — per the *Operator-authority delta* rule in `.claude/commands/auto-dev.md`, applying its two-branch test — is independently sufficient without requiring `plan_approved_fingerprint` to equal `draft_fp`. The row-path revocation-marker check above already gates this alternate condition too, exactly as it gates the equality one — a durable `<!-- plan-stage-approval-revoked: ... -->` marker treats the entire row-path evidence pair (`plan_approved_at`/`plan_approved_fingerprint`) as revoked and absent, so this alternate condition cannot fire on a just-revoked, stale-cached `plan_approved_at` either.
