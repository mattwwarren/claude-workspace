...
**Staleness guard (#2074).** The token-bearing comment must be the LAST comment, in live-fetched chronological order, that carries either a pipeline fixed header (`## Pending Verification Scan`, `## Blocking Review Findings`) or operator authorship — no `## Pending Verification Scan` or `## Blocking Review Findings` comment this pipeline posted may appear after it. Either header appearing later means the draft moved on since the operator approved it; treat the token as stale (absent evidence) and re-park.
