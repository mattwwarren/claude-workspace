...
0. **Resume check (runs before the tracker scan):** if `.cw/plan-draft.md` exists in the worktree AND `.cw/plan.md` does NOT exist, treat it exactly like "Plan found (sufficient)" below — extract its content, skip Step 1b's generation entirely, log "Found persisted plan draft from prior blocked/parked attempt — resuming, not regenerating," append a `friction_highlights` entry noting the resume (e.g. `resumed plan from .cw/plan-draft.md — prior blocked/parked attempt`, since `plan_source` stays the existing `"generated"` literal and carries no signal of its own that this was a resume rather than a fresh generation), and proceed to Step 1c (the ambiguity scan still runs unconditionally). **Supersession/ordering guard:** if `.cw/plan.md` already exists, ignore `.cw/plan-draft.md` regardless of either file's timestamp — an approved `.cw/plan.md` always wins over a stale draft. **Checkpoint-origin note (#1778):** `.cw/plan-draft.md` may also originate from a mid-flow checkpoint — the Step 1b post-generation checkpoint or the Step 1f.4 post-revision checkpoint — rather than only from a blocked/parked EXIT. A checkpoint-origin draft is never treated as final or pre-approved: the resume check above is presence-based (keyed only on whether `.cw/plan.md` exists) and routes ANY resumed draft, checkpoint-origin or exit-origin alike, through Step 1c's ambiguity scan and Step 1f's review stations unconditionally — so no separate checkpoint-specific guard is needed.
...
- **File enumeration under an explicit `## Files Modified` heading (required, #1779).** List every file the implementation will modify or create as **one bullet per file**, in the form `- <repo-relative path> (~<N> lines)` — one path per bullet, path first, nothing else before it. Mirrors the `## Patterns Found` / `## Touch-point Contract` producer-section convention above. This heading is not cosmetic: `.claude/scripts/check_plan_scope_conformance.py` parses this section (by heading prefix, and paths from either bullets or a table) at Step 2.5 gate 2 to measure the delivered diff against the plan, and a plan that buries its file list in prose gives that gate no anchor (it fails open and the run ships unmeasured); the gate also tolerates a markdown table as a fallback, but bullets remain the required format here so plans stay consistent with the other producer sections. Estimated line counts stay in the bullet text as before.
...
- The following health check block verbatim:
   ...
   **Headless only — checkpoint the draft before review runs (#1778).** Immediately after the Plan agent returns (before the `stage.entered` telemetry below and before Step 1c's ambiguity scan or Step 1f's review stations run), write the plan's current text to `.cw/plan-draft.md` in the worktree — best-effort: if this write fails, log and continue; do not treat it as blocking. This draft is unreviewed at capture time: neither Step 1c's ambiguity scan nor Step 1f's review stations have run yet, so it must never be treated as final or pre-approved, and it does not weaken, replace, or race the existing Step 1f.3 blocked-exit draft writes — those remain the authoritative writes for their own exit paths. No new resume machinery is needed: Step 1a.0's resume check, the supersession guard, Step 1e, and Step 1g best-effort deletions already handle a checkpoint-origin draft exactly like any other `.cw/plan-draft.md`. Skip this checkpoint when Step 1b did not run this invocation (e.g. an extracted-from-Linear plan that never reached the Plan agent).
...
4. **Headless mode:**
   ...
   **Draft-persistence rule (every Stage-1 human-gated headless exit with a plan in hand).** Before ANY headless EXIT that leaves Stage 1 while a draft plan exists — generated by Step 1b or extracted in Step 1a this invocation — write the plan's current text to `.cw/plan-draft.md` in the worktree. This covers the Step 4c `ambiguities_pending_resolution` and `premises_pending_verification` exits (including the combined premises+ambiguities exit) and Checkpoint 1's headless `plan_pending_approval` exit, in addition to the two Step 1f.3 `plan_unreviewable`/`plan_unsound` writes that already exist — those two are instances of this same rule. The write captures the plan **as it stands at the moment of exit**, including every section inserted so far (`## Adopted Assumptions`, `## Self-Verified Premises`, merged resolutions, any already-appended signoff marker), so the next dispatch's Step 1a.0 resume check picks up exactly where this round left off instead of regenerating from scratch — plan identity across rounds is what makes an answered question stay answered. Nothing else changes: the Step 1a supersession guard, the Step 1e `no_op` clearing, and the Step 1g post-approval clearing already handle staleness and cleanup. Worktree durability note: queue re-dispatch reuses the ticket's worktree, so the draft survives between rounds; if a dispatch path ever builds a fresh worktree without carrying the draft along, the resume check simply finds nothing and Step 1b regenerates — the current behavior, degraded gracefully.

   **Consolidated park (single-exit rule, #1650).** When Step 4c below (or Checkpoint 1's headless large-scope clause) decides to exit for a human AND a draft plan exists in hand (generated or extracted this invocation, or resumed from `.cw/plan-draft.md`), do NOT exit carrying only that gate's findings — each serial gate costs one operator round (mean park latency 9–13h), and a ticket that trips them one at a time pays 2–4 rounds discovering what one round could have surfaced together. Finish ALL remaining plan-phase analysis first:
   ...
   The exit **status** is unchanged by consolidation — Step 4c's precedence still picks `premises_pending_verification` over `ambiguities_pending_resolution`, and a park with nothing parked/unverified but Large scope still exits `plan_pending_approval` via Checkpoint 1 — only the comment gets richer. Sentinel: append `consolidated park: <a> ambiguities, <p> premises, <f> advisory findings, scope <tier>` to `friction_highlights` (`<a>`/`<p>`/`<f>`/`<tier>` are literal placeholders — substitute the computed values before emitting; no schema change). Guard: the advisory station run happens only when a draft plan exists in hand — an exit with no plan keeps its existing comment shape. Result-payload rules are untouched: `ambiguities`/`premises` arrays carry the same parked/unverified-only subsets as before; advisory findings travel in the comment and `friction_highlights` only.
   ...
   **Same partition, mirrored for premises — three ways (#1651).** Before any exit/continue decision, if a `PREMISES TO VERIFY — N items` block is present, split its items by each item's `Verified:` sub-bullet into `self_verified` (the Verified field's leading token is exactly `YES`, case-insensitive, backed by a quoted citation), `deferred` (the leading token is exactly `DEFER`, case-insensitive, AND both required sub-bullets — `In-implementation check:` and `On mismatch:` — are present and non-empty), and `unverified` (everything else — explicit `NO`, a missing `Verified:` sub-bullet, a `DEFER` missing either required sub-bullet, or any unparseable/malformed value all default-safe to unverified, no exceptions). `deferred` never parks — a runtime-only premise meeting the DEFER bar is verified at implementation start, not by a human at a desk who cannot check it statically either. If no `PREMISES TO VERIFY` block is present, all three buckets are empty. Scoped to this headless branch only: the interactive branch above (Step 1c.2, the premises `AskUserQuestion` bullet) is untouched — it already has its own self-verification-adjacent proceed path (the existing "confirmed true — proceed" option).
   ...
   **Step 4b — Plan-body disposition.**
   ...
   - If `deferred` is non-empty, insert a `## Deferred Premises` section into the plan body — immediately after `## Self-Verified Premises` if that section is present, else at the same anchor chain that section uses — one entry per deferred premise: the claim, the exact `In-implementation check:`, the `On mismatch:` halt condition, and what the plan depends on it for. ALSO insert an explicit leading step into the plan's implementation phase (Phase 2's first step, before any dependent work): run every `## Deferred Premises` check first; on any mismatch, halt via the impl stage's existing `blocked` exit with the premise named in the blocker message — never a silent fallback. A deferred check that cannot even be run (endpoint unreachable, fixture absent) is a mismatch for this purpose, not a pass.
   - For each `deferred` premise, also append one line to `friction_highlights` in the form `deferred premise: <claim> — checked at implementation start; on mismatch: <halt condition>`, so the operator can audit after the fact every premise the run scheduled for in-implementation verification (no schema change — same `list[str]` mechanism as the self-verified line above).

   **Step 4c — Exit/continue decision.** Keys on `parked` and `unverified` ONLY — `deferred` never gates (#1651) — not the raw `AMBIGUITIES`/`PREMISES TO VERIFY` block presence: an all-adopt ambiguity scan (`parked` empty) or a premises scan fully absorbed by `self_verified` + `deferred` (`unverified` empty) is functionally `NO_AMBIGUITIES`/`no premises pending` even though the raw scans returned items:
...
**Blocking-findings comment rule (#1815).** When a headless EXIT below leaves Stage 1 carrying a persisting MUST_FIX (`blocker.reason: "plan_unreviewable"` or `"plan_unsound"`), post the persisting station's MUST_FIX finding(s) — verbatim, reproduced per the Step 1f.4 convention for reproducing a station's findings unmodified — as a tracker comment under the fixed header `## Blocking Review Findings`, one `### <Station Name> — MUST_FIX` sub-section per persisting station. This header is distinct from `## Pending Verification Scan` (Step 1c's park header) and `## Multi-Marker Gate Blocked` (the historical multi-marker diagnostic) — this is a hard block, not a park, and this comment must never post the plan text itself. `<Station>` is a literal placeholder — substitute `Plan Reviewer` for a `plan_unreviewable` exit or `Plan Soundness Reviewer` for a `plan_unsound` exit before emitting. Sentinel: append `blocking findings posted: <station>` to `friction_highlights`, mirroring the #1650 `consolidated park: ...` sentinel idiom above.
...
**Headless only — checkpoint the revised draft (#1778).** Immediately after the plan-revision agent returns a new plan text — whether from the standard MUST_FIX revision cycle or the format-only revision cycle — write the plan's current text to `.cw/plan-draft.md` in the worktree — best-effort: if this write fails, log and continue; do not treat it as blocking. This checkpoint is not the fully-re-reviewed, authoritative version: Step 1f.2 still re-reviews the revised plan, and the existing Step 1f.3 blocked-exit writes (`plan_unreviewable` / `plan_unsound`) remain the authoritative draft writes if the revision cycle is ultimately exhausted — this checkpoint only ensures a revised draft survives an unexpected interruption between the revision agent's return and that exit.
