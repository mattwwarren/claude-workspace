...
A line whose content is anything other than the exact grammar above (trailing text, a label, operator prose) is non-conforming and MUST NOT be written — the transcription step treats an item it cannot express in this grammar as unmappable, never as an approximation. The grammar has no optional position, no label position, and no trailing-text position. The `plan-stage-resolutions-applied` line, when persisted, is always last — appended after every `plan-stage-settled` marker line, or after the fingerprint line if no settlement markers exist yet, or after the round-counter line if neither exists yet (#2376). Step 1a.0b's `plan-stage-resolutions-attempted` and `plan-stage-approval-revoked` lines, when present, precede that always-last line and are themselves leading bookkeeping lines.
