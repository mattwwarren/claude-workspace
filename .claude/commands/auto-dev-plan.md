...
After a plan is in hand (extracted or generated), run an ambiguity scan against the ticket BEFORE scope classification. This runs in every case, including the auto-skip path — a pre-approved plan is not a plan free of ambiguity — except when Step 1a.0b's approved-fingerprint fast path applies on a resumed round, which skips this scan entirely (see `.claude/commands/auto-dev-plan-appendix.md`, section "Step 1a.0b").
...
**Settlement marker grammar (`plan-stage-settled`, #1683).** A settled plan item is recorded as one HTML-comment line appended to `.cw/plan-draft.md`. **Bookkeeping-line order (#2154):** the round-counter line is always first; the `plan-stage-last-evaluated` fingerprint line, when one has been persisted, is always second; every `plan-stage-settled` marker line is appended after both — never before, and never onto the fingerprint's own line. (A draft that has never yet reached a cap check carries no fingerprint line yet, so its settlement markers begin at line 2 instead; once first persisted, the fingerprint line is never removed.) Settlement markers live only in `.cw/plan-draft.md`: no tracker comment this pipeline writes — `## Pending Verification Scan` included — may carry a settlement marker of any kind. The grammar below is closed and exhaustive for the payload after `plan-stage-settled:`; these three forms are the ENTIRE grammar, and nothing else is valid; the other leading bookkeeping lines are defined by the broader bookkeeping-line grammar immediately below.
...
A line whose content is anything other than the exact grammar above (trailing text, a label, operator prose) is non-conforming and MUST NOT be written. The grammar has no optional position, no label position, and no trailing-text position. The `plan-stage-resolutions-applied` line, when persisted, is always last. The transcription step treats an item it cannot express in this grammar as unmappable, never as an approximation. The settlement grammar has no optional position, no label position, and no trailing-text position.

**Leading bookkeeping-line grammar and order (#2154, #2376).** These are the complete leading HTML-comment lines permitted in `.cw/plan-draft.md`; their relative order is fixed and every rewrite preserves them. After the three existing positions documented above, `plan-stage-resolutions-attempted` comes when present; then `plan-stage-approval-revoked` when present; and `plan-stage-resolutions-applied`, when present, is always last — the `plan-stage-resolutions-applied` line, when persisted, is always last. If a later line is absent, the remaining lines close up without changing order.
...
**Step 1c.0 — Round-cap read + settlement folding (resumed rounds only).** Fires only when Step 1a.0's resume branch fired this dispatch; on a fresh (non-resumed) dispatch — the common path — skip straight to step 1 below. When it does fire, the full procedure (the tracker-state-delta fingerprint read, round-counter read, locating the open park comment and the operator's reply, the closed-vocabulary transcription rule, marker/`## Settled Plan Items` writing, `DEFERRED` stub wiring, the `resolution_evidence` candidate, and the full-stream guarantee) lives in `.claude/commands/auto-dev-plan-appendix.md`, section
"Step 1c.0: round-cap read and settlement folding (resumed rounds only)". Read it now if this dispatch resumed; do not transcribe an operator reply, write a settlement marker, or quote operator prose from this summary alone. Step 1a.0b's fast path may have already routed this round past Step 1c entirely — when it did, this section is not reached at all this round.
...
4. **Headless mode:**
   ...
   **Draft-persistence rule (every Stage-1 human-gated headless exit with a plan in hand).** Before ANY headless EXIT that leaves Stage 1 while a draft plan exists — generated by Step 1b or extracted in Step 1a this invocation — write the plan's current text to `.cw/plan-draft.md`. This covers the Step 4c `ambiguities_pending_resolution` and `premises_pending_verification` exits (including the combined exit) and Checkpoint 1's headless `plan_pending_approval` exit, in addition to the three Step 1f.3 headless `blocked` exit writes — `plan_unreviewable`, `plan_unsound` on its 1st MUST_FIX cycle, and `plan_unsound` when MUST_FIX persists after that cycle — which are instances of the same rule. The write captures the plan **as it stands at the moment of exit**, including every section inserted so far (`## Adopted Assumptions`, `## Self-Verified Premises`, `## Deferred Premises`, `## Settled Plan Items`, merged resolutions, any already-appended signoff marker), so the next dispatch's Step 1a.0 resume check picks up where this round left off. The draft's own leading bookkeeping lines are part of the same capture (#1683) and are carried forward per the Draft-rewrite rule below — never dropped on a rewrite. Queue re-dispatch reuses the ticket's worktree, so the draft survives; a path that builds a fresh worktree finds nothing and Step 1b regenerates.

   **Draft-rewrite rule (every write to `.cw/plan-draft.md`, #2154).** Any step that rewrites `.cw/plan-draft.md` — not only a headless EXIT — must carry forward its leading bookkeeping lines unchanged: the `<!-- plan-stage-scan-round: N -->` round-counter line, the `<!-- plan-stage-last-evaluated: ... -->` fingerprint line when one has been persisted, every `<!-- plan-stage-settled: ... -->` marker line, the `<!-- plan-stage-resolutions-attempted: ... -->` line when one has been persisted, the `<!-- plan-stage-approval-revoked: ... -->` line when one has been persisted, and the `<!-- plan-stage-resolutions-applied: ... -->` line when one has been persisted (always last — see Bookkeeping-line order above). The draft-persistence rule above is one instance of this rule, scoped to headless-EXIT writes; the other rewrite sites this rule binds are the Step 1b checkpoint above, the Step 1f.4 post-revision checkpoint below, and Step 1a.0b's resolutions-revision checkpoint (`.claude/commands/auto-dev-plan-appendix.md`) — each cites this rule rather than restating it.
...
"Consolidated park (single-exit rule, #1650)". Read it now if any gate is about to exit for a human with a draft in hand; do not assemble the park comment from this summary alone. The post is followed by a `cw signal-park` stamp (step 3a there); see the *Park-comment stamp rule* in `.claude/commands/auto-dev.md`.
...
**Step 4a — Partition (adopt-assumption fast path, headless only).** Before any exit/continue decision, if an `AMBIGUITIES — N items` block is present, split its items by each item's `Recommendation:` sub-bullet into `adopted` (leading token exactly `ADOPT`, case-insensitive) and `parked` (everything else — explicit `PARK`, a missing `Recommendation:` sub-bullet, or any unparseable/malformed value all default-safe to parked, no exceptions). If no `AMBIGUITIES` block is present, both are empty — the unchanged `NO_AMBIGUITIES` path. Scoped to this headless branch only; the interactive branch (Step 1c.2) is untouched.
   ...
   **Pre-branch integrity checks (single seam, #1683).** Fires exactly ONCE, immediately after Step 4b's disposition pass (stub-identity match included) completes and before Step 4c's branch selection begins — covering all four Step 4c outcomes, AUTO-CONTINUE included. Both checks evaluate here, in this fixed order — stub check first, then cap check — instead of being duplicated at the three Step 4c EXIT bullets, which carry no inline check text of their own.
   ...
   >
   > **Tracker-state-delta reset (#2154).** Before reading the round-counter line, compare the ticket's current tracker state against the fingerprint persisted at the last evaluation — `.cw/plan-draft.md`'s second line, `<!-- plan-stage-last-evaluated: operator_comment=<v>|body_sha=<v> -->` (absent ⇒ no prior fingerprint; treat as no delta). The fingerprint is **exactly two components**, freshly live-fetched each round: `operator_comment` — the `id` of the newest live-fetched comment that survives the *Comment provenance rule* in `.claude/commands/auto-dev.md`, or `none` when no such comment exists (the id is enough; no timestamp is needed); and `body_sha` — a SHA-256 of the live-fetched issue body text (github: `gh issue view <n> --json body`; linear: `get_issue`), computed over the raw string. Neither component is derived from the issue's raw `updated_at`/`updatedAt`: that field changes on every comment, including this pipeline's own park comment, so a fingerprint built on it would reset the cap every round. Because `operator_comment` is gated by the *Comment provenance rule*, an agent-authored comment never becomes `operator_comment`, however recently it was posted, including one posted after the operator's own comment — so posting one changes neither component and can never read as a tracker-state delta. A tracker-state change exists only when either component differs from the persisted value — a new (non-agent-authored) operator comment, or a body edit; when it does, reset `N` to 0 before the branch below; otherwise `N` is unchanged. Persist the freshly-read fingerprint to the same second line regardless of outcome, alongside whatever value of `N` this check ultimately writes.
   >
   > Read `.cw/plan-draft.md`'s round-counter line (`<!-- plan-stage-scan-round: N -->`, default 0 if absent, and reset per the delta check above). If `N < 2`: increment it to `N+1`, persist the incremented line to `.cw/plan-draft.md`, and proceed with whichever EXIT bullet Step 4c selects, as written below. If `N >= 2` (cap reached): headless mode does NOT proceed with that EXIT — instead hard-EXIT **through the consolidated park above** with `status: "blocked"`, `blocker.reason: "ambiguity_scan_unconverged"`, `blocker.stage: "stage1_plan"`, `blocker.details` naming the still-open item(s) verbatim, the round count (`N` at exhaustion — always 2, the fixed cap value from R6), and the fingerprint evaluated — the `operator_comment` id (or `none`) and the `body_sha` (closes #2160) — and `retry_eligible: true`; the same single-comment bundling the three Step 4c EXIT bullets and the stub check already use — any parked ambiguities, unverified premises, or advisory Step 1f findings surfaced this same round are folded into the one park comment, never dropped behind a cap-only block. Emit the `stage.errored` block below before the `blocked` sentinel. Interactive mode does NOT proceed with that EXIT either — instead **AskUserQuestion:** "This ticket's ambiguity scan has parked for 2 consecutive rounds without converging. Continue for one more round / accept the current state (still-open items move to a new `## Open Questions` section in the plan body, unresolved but non-blocking) / abandon the ticket?"
   >
   > ```bash
   > cw event record stage.errored \
   >   --correlation-id "$TICKET" \
...
### Plan-draft fingerprint rule

The single definition of the plan-draft content fingerprint (#2102). Every producer and consumer site below cites this rule **by name** — none of them restates the steps, so there is exactly one place the computation can be changed and exactly one place it can be wrong.

**Computation.** Take the current text of `.cw/plan-draft.md`, **strip** its leading bookkeeping lines — the `<!-- plan-stage-scan-round: N -->` round-counter line, the `<!-- plan-stage-last-evaluated: ... -->` fingerprint line, every `<!-- plan-stage-settled: ... -->` marker line, the `<!-- plan-stage-resolutions-attempted: ... -->` line, the `<!-- plan-stage-approval-revoked: ... -->` line, and the `<!-- plan-stage-resolutions-applied: ... -->` line — and hash what remains with **SHA-256**, rendered as full lowercase hex (`sha256sum`-style, as the `plan-stage-last-evaluated` tracker-state fingerprint is already computed). The stripped lines are bookkeeping about the draft, not the draft: a round-counter increment or a newly-appended settlement marker must never read as a changed plan, or every resumed round would invalidate its own approval.

**Where it is computed.** Fresh, on demand, every time a site needs it — never persisted as an additional bookkeeping line in `.cw/plan-draft.md` (the bookkeeping-line grammar above is closed, #2154).
...
**Headless** (clauses evaluated in order; first match wins): If plan agent reported `pre-flight: already satisfied` → EXIT `no_op` (see Step 1e) — this preempts every clause below, including scope and forbidden-area rejections, since there is nothing to implement. Otherwise: if plan in Linear or resumed from `.cw/plan-draft.md` → AUTO-SKIP plan-approval question (Step 1c's ambiguity scan still runs and may exit `ambiguities_pending_resolution`). **Large-scope carve-out on the resumed-draft path (#1650, #2102):** when the plan was resumed from `.cw/plan-draft.md` AND Step 1d classifies it Large, the AUTO-SKIP additionally requires approval evidence. **Before evaluating the clauses below, compute the resumed draft's fingerprint per the *Plan-draft fingerprint rule* above** — call it `draft_fp`. Two independent sources can supply the evidence:

  - **Comment path:** an operator reply carrying the **comment-path approval token** (grammar and rules below), posted after the prior round's `## Pending Verification Scan` comment (the `### Approval requested` ask), where that reply must itself carry operator authority under the *Comment provenance rule* in `.claude/commands/auto-dev.md` (#2097) — an agent-authored comment is never approval evidence. Prose alone is never sufficient — "approved", "LGTM", "go ahead" are not evidence; fail closed on anything short of the exact token.
  - **Row path:** non-null `queue_metadata.plan_approved_at` **AND** non-null `queue_metadata.plan_approved_fingerprint` in `.claude/cw-context.json`, **AND** either (a) the fingerprint is **equal to `draft_fp`**, or (b) no operator-authority delta since `plan_approved_at`, per the *Operator-authority delta* rule in `.claude/commands/auto-dev.md` — the latter branch does not require fingerprint equality: it tolerates a non-equal fingerprint but still requires both durable approval fields. Both keys are stamped together by `src/cw/dev_queue/approval.py` when the operator runs `cw dev-queue approve`, and threaded in at spawn by `src/cw/spawn.py`. For the fingerprint-equality branch, a non-null timestamp is **not** sufficient by itself: it records that an approval happened, never which text the operator read, so on its own it lets a draft edited after approval resume straight past this gate — the exact vulnerability #2102 closes. The no-delta branch is the separate alternate described above and composes with Step 1a.0b's fast path. The row path is the only evidence a Linear-tracked ticket can ever produce, since `cw dev-queue approve --post-marker` posts through `gh` and cannot reach Linear. **Before accepting either row-path sub-condition, honor any durable `<!-- plan-stage-approval-revoked: at=<UTC>; fingerprint=<sha> -->` marker in the draft:** if its fingerprint equals `plan_approved_fingerprint` and `plan_approved_at` is at or before the marker's `at`, treat the entire row-path evidence pair and both sub-conditions as revoked and absent; a later `plan_approved_at` is a fresh approval and supersedes that marker. Step 1a.0b revokes the row durably with `cw dev-queue revoke-plan-approval <ticket> -c <client>`, which atomically clears **both** row fields under the queue lock; the draft marker is audit/recovery evidence only. The checker treats the row evidence as absent for that dispatch after the mutation, and a later spawn consumes the cleared `queue_metadata` values.

**Operator-authority-delta alternate (#2433).** When the row has both durable approval fields — `plan_approved_at` and `plan_approved_fingerprint` — and the *Operator-authority delta* rule finds no newer operator-authority comment, the approval transfers through this alternate row-path condition independently of fingerprint equality. A newer operator-authority comment disqualifies the alternate and forces the full re-scan; the fast-path instructions in Step 1a.0b apply the same two-branch test.

Either source is sufficient **on its own terms** — the comment path is gated by the token, no-double-duty, and staleness rules below, and the row path by exact fingerprint equality or the Operator-authority delta alternate; neither is a mere presence check. Absent both, EXIT `plan_pending_approval` again (persist the draft, reference the existing park comment rather than re-posting the full draft — do not re-run the advisory stations just to re-ask). A resumed draft that was parked for approval must not slip through approval by being resumed.

**Comment-path approval token grammar (#2074), closed and exhaustive:**

```
<!-- auto-dev-comment-approval -->
```

This token is distinct from, and never satisfied by, the CLI's write-only, audit-only post-marker comment that `cw dev-queue approve --post-marker` posts (#2194) — that marker records a row-path approval already granted through the CLI and stays audit-only; it is never comment-path evidence.

**No double duty (#2074).** The token-bearing comment MUST NOT be the comment Step 1c.0 step 3 (`.claude/commands/auto-dev-plan-appendix.md`) located as this round's settlement-source comment, if that comment settled ≥1 item at step 5. Resolving a parked ambiguity or premise is not approving the plan's scope, and this pipeline never lets one comment do both jobs — however many tokens it carries. Approval must arrive in its own, separate comment.

**Staleness guard (#2074).** The token-bearing comment must be the LAST comment, in live-fetched chronological order, that carries either operator authorship or any pipeline fixed header named by the *Comment provenance rule* in `.claude/commands/auto-dev.md` — `## Pending Verification Scan`, `## Multi-Marker Gate Blocked`, `## Blocking Review Findings`, or `## Operator-Actionable Review Findings`. If any such fixed-header comment appears later, the draft moved on since the operator approved it; treat the token as stale (absent evidence) and re-park.

**Fingerprint mismatch sub-case (#2102).** A non-null `plan_approved_fingerprint` that does **not** equal `draft_fp` is a distinct outcome from absent evidence. First evaluate the *Operator-authority delta* alternate: if a durable `plan_approved_at` exists and no operator-authority comment postdates it, the approval transfers through that alternate despite the mismatch, and Step 1a.0b's fast path applies; the mismatch is tolerated as advisory-only drift with no operator dissent. If that no-delta condition does not hold, then the fingerprint mismatch fallback applies to a resumed draft classified **Large**: EXIT `plan_pending_approval`, and the re-park comment must say the approval was revoked by a **fingerprint mismatch** and must **quote both fingerprints** — the approved one (`plan_approved_fingerprint`) and the current one (`draft_fp`) — so the operator can see that what they approved is not what is about to be built. Do not silently re-ask as though no approval had ever been given. A resumed draft classified **Small** follows the ordinary AUTO-SKIP path; a plan generated in this dispatch follows the ordinary scope behavior: small → AUTO-APPROVE and proceed (Step 1c still gates on ambiguities), large → EXIT `plan_pending_approval` **through the Step 1c consolidated park** (post the single enriched `## Pending Verification Scan` comment — advisory station findings, `### Approval requested`, and the full draft — no branch; ambiguity scan results ride along in the same comment when present; persist the draft to `.cw/plan-draft.md` per the Step 1c draft-persistence rule before exiting). If `--scope-limit small` rejects → EXIT `scope_exceeded`. If `--forbidden` rejects → EXIT `forbidden_area`.
...
**THEN** post the same plan as a comment on the Linear issue (skip for free-text tickets — but never skip the `.cw/plan.md` write). The tracker comment is the audit copy, the file is the pipeline artifact. **Provenance marker (#2097):** end the comment body with the line `<!-- cw-agent-authored -->` on its own line after a blank line, per the *Comment provenance rule* in `.claude/commands/auto-dev.md` — it is what stops a later stage reading this pipeline's own analysis as an operator decision.

**Never background this post (#2157).** Wrap the tracker-post call in a sized `timeout` and run it foreground — never `run_in_background`, never accept a background continuation. A timeout or failure here is `blocked`/`agent_block`, not a silent turn-end: see the *Worker Execution Discipline* section in `.claude/commands/auto-dev.md` for the no-backgrounding rule and why it applies to orchestrator-run calls like this one.
...
**`plan_draft_fingerprint` emission rule (#2102).** Unlike the two keys below, this one is **always present** on a plan-stage sentinel — at the completion sentinel above and at **every park exit** (`plan_pending_approval`, `ambiguities_pending_resolution`, `premises_pending_verification`, and the round-cap / stub / `blocked` hard-exits). Whenever a draft plan is in hand, compute the value per the *Plan-draft fingerprint rule* (Step 1c/1d boundary above) and place it here; when no draft exists, emit `null` explicitly rather than omitting the key. Do not restate the stripping or hashing steps here — the named rule is their only definition. This is what `cw dev-queue approve` reads to bind the approval it records to a specific draft (the dev-queue row's `plan_approved_fingerprint`); a round that omits the key on a park exit hands the next round an approval bound to nothing, which is the #2102 bug reproducing itself.

**`resolution_consumed`/`resolution_evidence` emission rule (#1896).** These two keys must include both keys only if this round settled ≥1 item via Step 1c.0 step 5 (the round's `resolution_evidence` candidate — see that step) AND the round still exits paused through one of the three Step 4c EXIT bullets; must not include them otherwise. The Pre-branch integrity checks' cap-check (`ambiguity_scan_unconverged`) and stub-check (`deferred_stub_unresolved`) hard-exits are never eligible carriers for these two keys, whatever this round settled (#2154) — widening this to the cap/stub exits would let a round the dispatch productivity ceiling (`cw.dispatch.productivity.is_unproductive`, #1750) OR's in `resolution_consumed` as one of its "productive" signals read as productive for a plan-stage round that reset its own cap, defeating that ceiling for exactly the rounds it needs to bound. This is what makes the anti-gaming guarantee hold: only step 5's own current-round transcription can produce a candidate, so a round with nothing settled this pass — including a round that merely finds a prior round's marker already present — never emits `resolution_consumed: true`. **Scoped to Step 1c.0 settlement only (#2098).** A Step 1b `## Binding Pre-flight Resolutions` merge (Step 1b setup above) is NOT a settlement and never emits these keys, even on a round that merged one — that merge happens before the plan is even generated, has no step 5 transcription, and is not gated by a Step 4c EXIT. Its trace is the plan's own `## Pre-flight Resolution Conformance` section and `friction_highlights`; it does not additionally set `resolution_consumed`. Step 1a.0b's resolutions-delta revision (`.claude/commands/auto-dev-plan-appendix.md`) is excluded for the identical reason: it too runs before Step 1c.0's step 5 transcription and is not gated by a Step 4c EXIT bullet, so it never emits `resolution_consumed`/`resolution_evidence` either — its trace is the plan's own `## Pre-flight Resolution Conformance` section and `friction_highlights`, exactly like the Step 1b merge above.
