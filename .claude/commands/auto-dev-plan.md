...
4. **Headless mode:**
   ...
   **Draft-rewrite rule (every write to `.cw/plan-draft.md`, #2154).** Any step that rewrites `.cw/plan-draft.md` — not only a headless EXIT — must carry forward its leading bookkeeping lines unchanged: the `<!-- plan-stage-scan-round: N -->` round-counter line, the `<!-- plan-stage-last-evaluated: ... -->` fingerprint line when one has been persisted, every `<!-- plan-stage-settled: ... -->` marker line, and the `<!-- plan-stage-resolutions-applied: ... -->` line when one has been persisted (always last — see Bookkeeping-line order above). The draft-persistence rule above is one instance of this rule, scoped to headless-EXIT writes; the other rewrite sites this rule binds are the Step 1b checkpoint above, the Step 1f.4 post-revision checkpoint below, and Step 1a.0b's resolutions-revision checkpoint (`.claude/commands/auto-dev-plan-appendix.md`) — each cites this rule rather than restating it.
...
**Computation.** Take the current text of `.cw/plan-draft.md`, **strip** its leading bookkeeping lines — the `<!-- plan-stage-scan-round: N -->` round-counter line, the `<!-- plan-stage-last-evaluated: ... -->` fingerprint line, every `<!-- plan-stage-settled: ... -->` marker line, and the `<!-- plan-stage-resolutions-applied: ... -->` line — and hash what remains with **SHA-256**, rendered as full lowercase hex (`sha256sum`-style, as the `plan-stage-last-evaluated` tracker-state fingerprint is already computed). The stripped lines are bookkeeping about the draft, not the draft: a round-counter increment or a newly-appended settlement marker must never read as a changed plan, or every resumed round would invalidate its own approval.
