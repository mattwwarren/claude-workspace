...
**Headless** (clauses evaluated in order; first match wins): If plan agent reported `pre-flight: already satisfied` → EXIT `no_op` (see Step 1e) — this preempts every clause below, including scope and forbidden-area rejections, since there is nothing to implement. Otherwise: if plan in Linear or resumed from `.cw/plan-draft.md` → AUTO-SKIP plan-approval question (Step 1c's ambiguity scan still runs and may exit `ambiguities_pending_resolution`). **Large-scope carve-out on the resumed-draft path (#1650, #2102):** when the plan was resumed from `.cw/plan-draft.md` AND Step 1d classifies it Large, the AUTO-SKIP additionally requires approval evidence. **Before evaluating the clauses below, compute the resumed draft's fingerprint per the *Plan-draft fingerprint rule* above** — call it `draft_fp`. Two independent sources can supply the evidence:
  ...
  - **Row path:** a non-null `queue_metadata.plan_approved_at` in `.claude/cw-context.json` **AND** a `queue_metadata.plan_approved_fingerprint` that is **equal to `draft_fp`**. Both keys are stamped together by `src/cw/dev_queue/approval.py` when the operator runs `cw dev-queue approve`, and threaded in at spawn by `src/cw/spawn.py`. A non-null timestamp is **not** sufficient by itself: it records that an approval happened, never which text the operator read, so on its own it lets a draft edited after approval resume straight past this gate — the exact vulnerability #2102 closes. The row path is the only evidence a Linear-tracked ticket can ever produce, since `cw dev-queue approve --post-marker` posts through `gh` and cannot reach Linear. **Before accepting this pair — or the alternate sufficient condition below — honor any durable `<!-- plan-stage-approval-revoked: at=<UTC>; fingerprint=<sha> -->` marker in the draft:** if its fingerprint equals `plan_approved_fingerprint` and `plan_approved_at` is at or before the marker's `at`, treat the row evidence as revoked and absent for both sub-conditions, not just the equality one; a later `plan_approved_at` is a fresh approval and supersedes that marker for both. Step 1a.0b revokes the row durably with `cw dev-queue revoke-plan-approval <ticket> -c <client>`, which atomically clears **both** row fields under the queue lock; the draft marker is audit/recovery evidence only. The checker treats the row evidence as absent for that dispatch after the mutation, and a later spawn consumes the cleared `queue_metadata` values.

**Row-path alternate sufficient condition (operator-authority delta, #2433).** The row-path revocation-marker check above already gates this alternate condition too: a durable marker revokes both row fields, so this alternate condition cannot fire on a just-revoked, stale-cached `plan_approved_at` either. On mismatch, both durable row fields (`plan_approved_at` and `plan_approved_fingerprint`) must be non-null (`plan_approved_fingerprint` must not be `None`), no operator-authority delta may have arrived since `plan_approved_at` under the *Operator-authority delta* rule, and the changed portion must be machine-verified as advisory-only; verification excludes substantive or destructive changes, and silence is not verification. This is independently sufficient without requiring `plan_approved_fingerprint` to equal `draft_fp`.

Either source is sufficient **on its own terms** — the comment path is gated by the token, no-double-duty, and staleness rules below, the row path by either row-path condition above; neither is a mere presence check. Absent both, EXIT `plan_pending_approval` again (persist the draft, reference the existing park comment rather than re-posting the full draft — do not re-run the advisory stations just to re-ask). A resumed draft that was parked for approval must not slip through approval by being resumed.
...
**Fingerprint mismatch sub-case (#2102).** A non-null `plan_approved_fingerprint` that does **not** equal `draft_fp` is a distinct outcome from absent evidence, and reports itself as one: the draft changed after the operator approved it, so the approval does not transfer by fingerprint equality alone. **Check the operator-authority-delta branch first (#2433):** if the row-path's alternate sufficient condition above holds — both durable row fields exist, no operator-authority delta since `plan_approved_at` per the *Operator-authority delta* rule, **and the changed portion has been machine-verified as advisory-only** — the approval transfers via that branch instead, tolerating only advisory-only drift with no operator dissent, and Step 1a.0b's fast path applies as normal (no re-park). Comment silence alone is not that verification, and substantive or destructive changes still require re-approval. **Only when that condition also fails** does the following unconditional fallback apply: Still EXIT `plan_pending_approval`, but the re-park comment must say the approval was revoked by a **fingerprint mismatch** and must **quote both fingerprints** — the approved one (`plan_approved_fingerprint`) and the current one (`draft_fp`) — so the operator can see that what they approved is not what is about to be built. Do not silently re-ask as though no approval had ever been given. If plan generated + small → AUTO-APPROVE and proceed (Step 1c still gates on ambiguities). If plan generated + large → EXIT `plan_pending_approval` **through the Step 1c consolidated park** (post the single enriched `## Pending Verification Scan` comment — advisory station findings, `### Approval requested`, and the full draft — no branch; ambiguity scan results ride along in the same comment when present; persist the draft to `.cw/plan-draft.md` per the Step 1c draft-persistence rule before exiting). If `--scope-limit small` rejects → EXIT `scope_exceeded`. If `--forbidden` rejects → EXIT `forbidden_area`.
