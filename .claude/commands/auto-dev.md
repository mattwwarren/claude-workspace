...
| `fix_loop_dispatch_unverified` | Step 3b's post-spawn check (`cw agent-spawn-verify`) found no subagent transcript for the fix agent within its poll window — the async dispatch produced nothing, so ending the turn would have waited forever on a completion notification that will never arrive (#2012). `blocker.stage` is `"stage3_review"`; `blocker.details` carries the command's diagnostic verbatim (project dir checked, `--since`, exclusion, effective poll window). `retry_eligible: true` — the likeliest cause is infra slowness (cold model start, contended host, network-mounted worktree), which a re-dispatch clears. Widen `agent_spawn_verify_poll_seconds` in `orchestrator.yaml` if a client sees this repeatedly on healthy dispatches |
