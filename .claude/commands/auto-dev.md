...
**Failure of the command itself falls back to the frame (carve-out from the Tool-Use Denial Exit).** Like `cw signal-park`, this call is a side channel to cw state, so an auto-mode classifier denial of the `cw result emit` call is **not** a `tool_denied` exit, and an unknown-command error (a `cw` that predates `result emit`, i.e. version skew) or any other failure that is not a `field.path: message` validation report is ignored: run `printf '%s' "$SENTINEL_JSON" | cw result validate -` instead if you can, then frame the payload. The Stop hook still parses the transcript when nothing was recorded, exactly as before #2382.
