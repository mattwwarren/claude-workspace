...
**`no_diff_anchor` — the finding with no diff artifact at all (#1817).** Default `false`. Set it `true` **only** when the finding's remedy lies outside this diff entirely: an acceptance criterion demanding a follow-up ticket that was never filed, a required artifact that exists nowhere in the repo. When you set it, emit `"file": "N/A"` — that exact literal string, never a per-reviewer invention and never a plausible-looking fake path, so the field stays queryable — and leave both `line_start` and `line_end` `null`; a claimed line position is meaningless with no diff to anchor it to, and the schema rejects the combination outright. `evidence` is still REQUIRED and still verbatim, but it quotes the *source of the obligation* (the acceptance-criterion sentence, the plan clause) rather than diff text — this marker is the one documented exemption from the verbatim-diff-substring rule stated below. Do NOT reach for it to smuggle through a finding whose evidence you simply could not locate in the diff: that is `status: "degraded"` with a stated reason, not a `no_diff_anchor` finding.
...
**Third trigger (#1817): the `plan_deviation` exit.** The same rule, the same header, and the same structured-finding source also cover Stage 3's `blocker.reason: "plan_deviation"` exit (the 4a Exit rule below). Without it, a NON_DEFERRABLE finding that is not itself diff-anchorable would exit `blocked` with nothing about it ever reaching the tracker — the exact defect this rule exists to prevent, reintroduced through the one blocked-exit door it did not cover. Unlike the two `review_blocked` sites above, which stay MUST_FIX-worded and unchanged (#1714's guarantee is MUST_FIX-specific there), the `plan_deviation` trigger posts whatever NON_DEFERRABLE finding(s) actually caused the exit, **regardless of severity** — 4a's NON_DEFERRABLE test is not itself severity-scoped, so a SHOULD_FIX NON_DEFERRABLE finding must still reach the tracker rather than fall through a MUST_FIX-worded sentence with nothing to point at.
...
Adjudication assigns each finding a disposition. The coordinating session — never a subagent or executor — sorts **every** finding (MUST_FIX *and* SHOULD_FIX) into exactly one of four buckets:
...
4. **OPERATOR ACTIONABLE (#1817).** The session accepts the finding as valid and in-scope, but its remedy lies outside this diff entirely — the finding carries `no_diff_anchor: true` (a follow-up ticket that was never filed, an artifact that exists nowhere). It is **never eligible for FIX NOW**: the fix loop cannot edit a diff that does not exist, so bucket 1 could only burn cycles on it. **Scoped to `severity: MUST_FIX` only** — a SHOULD_FIX `no_diff_anchor` finding the session accepts is *not* eligible here and routes through ordinary DEFER (bucket 3) like any other SHOULD_FIX; `cw review adjudicate` enforces this at the model, so a SHOULD_FIX adjudicated `operator_action` is rejected outright rather than silently accepted. **Excludes NON_DEFERRABLE findings** — a finding that is both NON_DEFERRABLE and `no_diff_anchor` never reaches this bucket at all: it is by definition already "beyond fix-loop scope" under the 4a Exit rule above, and exits `blocked`/`plan_deviation` there instead. A `no_diff_anchor` finding the session *disagrees* with still goes to ordinary REJECT (bucket 2). Record it with `outcome: "operator_action"` and post it per the operator-actionable findings comment rule below.
...
Source the body directly from the already-structured finding data, the same way the blocking-findings comment rule above does. The entry's `rationale` is REQUIRED and must name the concrete action the operator has to take — file ticket X, link the ticket that discharges criterion Y — so a reader of the checklist never has to open the transcript to learn what "actionable" meant. No PR exists yet at this point, so the comment posts to the ticket, not a PR. Sentinel: append `operator actionable findings posted: review_operator_actionable` to `friction_highlights`, mirroring the `blocking findings posted: <reason>` idiom above.

**Its trigger is `ADJUDICATIONS`, not `blocker.reason`.** This rule fires purely on the presence of an `operator_action` entry; it does not check which `blocker.reason` the stage ultimately exits with. That decoupling is what makes the racing-exits case resolve with no new machinery: when one review pass carries both an operator-actionable finding and a *separate*, unrelated NON_DEFERRABLE finding that independently triggers `plan_deviation`, `plan_deviation` wins the recorded `blocker.reason` and this comment still posts anyway. Two comments, one exit reason, no precedence ladder to invent.
...
**Operator-action override (#1817) — the last check before the Stage 3 Completion sentinel.** After the verify-fixes pass above, if `ADJUDICATIONS` contains any `outcome: "operator_action"` entry, Stage 3 EXITS `blocked` with `blocker.reason: "review_operator_actionable"` and `blocker.stage: "stage3_review"` (open enum; routes to BLOCKED_ON_USER via the existing generic dispatch rules, not to finalize — mirrors `plan_deviation`'s row shape verbatim), **instead of** whatever AUTO-CONTINUE / `review_pending_approval` outcome this round's natural conclusion would otherwise have produced. Surface the entries in `blocker.details`. The checklist comment itself has already posted per the operator-actionable findings comment rule at Checkpoint 3a, whose trigger is independent of this exit — do not post it a second time here. Because that bucket is MUST_FIX-scoped at the model (Checkpoint 3a bucket 4), the check is a bare existence test, with no severity branch of its own.

**Which of Step 3b's exits this override governs — and why nothing is lost on the one it does not.** Step 3c is the funnel for two of Step 3b's three exits: fix-loop convergence and the sparse-feedback skip (this section's own opening sentence names exactly those two). The cycle-5 hard exit is neither: that path EXITS `blocked` with `blocker.reason: "review_blocked"` directly at Step 3b.5 and never flows forward into Step 3c, so this override does not apply there — and none is needed. The cycle-5 exit already posts "the still-unresolved MUST_FIX findings," and an operator-actionable finding is exactly such a finding: its `ADJUDICATIONS` entry is recorded at Checkpoint 3a bucket-assignment time, *before* Step 3b runs at all, and that exit's posting is not scoped to bucket-1 findings. The only observable difference on the cycle-5 path is which `blocker.reason` is recorded — a difference, not a loss, since the finding's tracker visibility and adjudication record are unaffected either way.
