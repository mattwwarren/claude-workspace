...
**Comments are live, not cached (#1794).** The cached `comments` array in `.cw/context.json` is a Stage-0 (or earlier Stage-2 re-materialization) snapshot only. Dispatch spawns `/auto-dev-{stage.value} <ticket> --headless` **directly per stage** (`src/cw/executor.py`, RFC 0005 A2) — Stage 0 does NOT re-run between pipeline stages, so on an IMPL-stage re-entry (queue re-dispatch, `--regress --stage impl`, or a resumed `s2_implementing` session) the cached array can be arbitrarily stale and an operator send-back comment would never reach this stage. Regardless of whether `.cw/context.json` already exists, this stage MUST live-fetch the ticket comments on every invocation via the active tracker's fetch op (`list_comments(<id>)` for `linear`; `gh issue view <n> --json comments` for `github-issues`), overwrite `.cw/context.json`'s `comments` array with the fresh result (each entry mapped to `{"author": "<login>", "created_at": "<createdAt>", "body": "<text>"}`), and bump `materialized_by_session` to the current session id. Mirrors `auto-dev-plan.md`'s "Comments and body are live, not cached" paragraph. Also write the fresh array verbatim to `/tmp/impl-comments-$CW_SESSION.json` — the Pre-Stage Detector Guard below consumes it. **On comments-fetch failure, a regressed entry hard-blocks; a fresh one WARNs (#2415).** Before deciding how to handle a failed live fetch, read `queue_metadata.regressed_into_stage` from `.claude/cw-context.json` — the same source the Pre-Stage Detector Guard's `REGRESSED_INTO_STAGE` reads (`.claude/commands/auto-dev-impl-appendix.md`). **If it is non-empty** (this IMPL entry was reached via `_stage_regress`) **and** the live fetch exits non-zero or returns malformed JSON: EXIT `blocked` with `blocker.reason: "impl_comments_unreadable_after_regress"` and `blocker.details` naming the fetch failure. A regress exists specifically to act on newer comments, and continuing on the stale cached array would defeat it, and the worker cannot tell whether the comment that triggered the regress is the very one it can no longer read. **Otherwise** (a fresh, non-regressed entry), **WARN and continue** (mirrors `auto-dev-intake.md`'s Step 0d): emit an attention signal, log `"impl_comments_fetch_failed"` in `friction_highlights`, and leave the existing `comments` array untouched rather than overwriting it with an empty/failed result — a stale-but-real array is better evidence than none. Still write whatever was fetched (possibly `[]`) to `/tmp/impl-comments-$CW_SESSION.json`; the staleness script degrades gracefully on a bad or empty comments file without masking an independently-sourced `regressed_into_stage` signal.
...
- **Type check gate (do NOT declare complete until clean):**
  ...
  - **Strict override check (run first).** If the operator's global instructions (the user-level `~/.claude/CLAUDE.md` the harness loads into every session's context — check what is already in your context; do not go read the file) or the repo's own `CLAUDE.md` demand that pre-existing mypy errors be fixed (e.g. "cannot accept any pre-existing mypy issues"), the baseline branch below does NOT apply — use the no-baseline branch. If fixing the pre-existing errors would require editing files outside the plan's `## Files Modified`, do NOT fix them: STOP and report a BLOCK in friction with the full error set and a proposed split (this change vs. the inherited debt). Never absorb an unplanned legacy refactor.
  - **If the repo maintains a mypy baseline** and no strict override applies, only errors the repo's own baseline-filtered mypy command reports block this gate; pre-existing baselined errors in a file you merely touched do NOT block. Baseline detection, the exact comparison against `$FORK_POINT`, and the rule that the baseline file may never gain entries in this diff live in `.claude/commands/auto-dev-impl-appendix.md`, section "Type check gate: mypy baseline detection and comparison". Read it now before claiming a baseline applies; do not improvise the detection or comparison from this summary. Record `MYPY_BASELINE_FILE`, `MYPY_BASELINE_CMD`, and the baselined pre-existing errors in touched files (`file:line [code]`) in the friction report — the orchestrator re-verifies them and routes them to deferred tickets.
  ...
  - **If the repo has no baseline** (or a strict override applies), fix every mypy error in touched files, pre-existing ones included — treat those as active bugs, not noise to inherit.
...
- **The following Completion Artifacts block verbatim** (Subagent Reliability Mitigation 2 — orchestrator gates on these, NOT on the prose claim of "done"):
  ...
  ```
  Hallucinating or paraphrasing these is a discipline failure — the orchestrator parses them. Pasted output contradicting the "done" claim (FAILED in test tail, blocking mypy errors per the type-check gate, empty diff) results in `impl_failed`, and a report missing a row for any gate named in the session's quality-gate sentence draws the same `impl_failed` disposition.
