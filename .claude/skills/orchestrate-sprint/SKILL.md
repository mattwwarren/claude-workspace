...
Arm the attention monitor (`scripts/attention_monitor.sh <client>`) — a
persistent watch on the `session.needs_attention` / `timed_out` /
`reap_proposed` event bus for your client, deduped so a parked session doesn't
spam. Events arrive to *you*; you triage; you push the operator only what
changes what they'd do next. Use `/cw-queue-peek` for the WAIT/PEEK/STOP verdict
on any session running long. **Silence means healthy** — the monitor covers the
failure signatures, so no news genuinely is good news (don't poll on top of it).
