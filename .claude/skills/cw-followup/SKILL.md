...
**It refuses to run inside a dispatch worker** (it checks the nearest `.claude/cw-context.json` for `headless`, exits non-zero and writes nothing), because a worker settling findings raised by its own reviewer is the pipeline adjudicating itself. There is no bypass flag and you must not look for one. If you hit that refusal, stop: report the exact payload and the reason the user gave, and say plainly that `cw review settle` has to be run on the operator's own machine.
