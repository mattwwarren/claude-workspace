...
- **`create_worktree` can now fetch and fast-forward a behind, unoccupied worktree on the reuse path, closing the reuse-path fetch asymmetry (#2213):** the first-time path fetched `origin/<branch>` before resolving the branch, but reuse returned the existing worktree untouched, so a per-ticket worktree reused across pipeline stages could sit on a stale HEAD while `origin/<branch>` had moved on. The refresh is opt-in via a new keyword-only `refresh_on_reuse` (default `False`, today's path-only behavior); the dispatch claim path and `dispatch_fix_agent` pass `True`, interactive `cw start` does not. With it set, an occupancy check with local reads only runs first: an unexpected checked-out branch, unsaved work (uncommitted, untracked or unpushed, per `unsaved_work_reason`), a live session homed on the path (the shared `live_session_worktree_paths`, extracted from the worktree GC, narrowed to catch only `OSError`/`ValueError`, and reporting `None` for "indeterminate" so each caller decides what that means), or a live worker in the daemon roster homed on it (new fail-closed `NativeDaemonClient.list_live_worker_cwds`, sharing one roster parser with `list_live_session_short_ids`; a roster that is not valid UTF-8 is indeterminate too, not a crash) all mean the worktree is used as-is, with a DEBUG log naming the path and reason and no network call. Paths are compared after normalization on both sides (`Path.resolve()`, plus an explicit `stat()` because Python 3.13's non-strict `resolve()` silently swallows a symlink loop), and unreadable session state, an unreadable roster or ANY path that cannot be normalized (`ELOOP`, `EACCES`, `ENOTDIR`, `ENAMETOOLONG`, every other `OSError`) fail closed (occupied); the one exception is a path that does not exist (`FileNotFoundError`), which is a fact rather than a failure to look, so a stale record of a deleted worktree cannot veto every refresh. The reuse refresh fails closed while the worktree GC, which shares `live_session_worktree_paths`, deliberately keeps failing open: do not "fix" one into consistency with the other. Otherwise it best-effort fetches `origin/<branch>` (a network call that can be slow). `fetch_feature_branch` now returns a three-state `FetchOutcome` instead of a bool, because the two non-success cases need opposite handling: only `FETCHED` fast-forwards; `BRANCH_ABSENT` (the branch is not on origin yet, an expected state for a fresh ticket) proceeds without a refresh and is not reported as friction; `FAILED` skips the fast-forward entirely rather than moving HEAD to the stale tracking ref it leaves behind, and is reported. The full occupancy check is re-run immediately before `git merge --ff-only` to `refs/remotes/origin/<branch>`, which happens only when HEAD is a strict ancestor; the re-check narrows but does not eliminate the window in which a session can start, which is accepted because holding a lock across a network fetch is worse. Diverged and `--ff-only`-refused worktrees are left as they were with a WARNING. Submodule synchronization after a fast-forward is deliberately NOT part of this change (it is tracked separately and will be built on these refusal semantics). `create_worktree` takes an optional `refresh_report` (`ReuseRefreshReport`) out-parameter: its `notes` receive exactly one single-line entry, naming the worktree and the reason, for each refresh failure the caller cannot otherwise see (fetch failed, fast-forward refused by git, diverged, an OS error), and its `live_occupant` is set whenever a live session or worker may be homed on the worktree (including when that could not be read). `dispatch_fix_agent` prepends the notes to the fix agent's prompt and, on a `live_occupant`, raises the existing transient `HookContextConflictError` before ANY later mutation of that worktree (the `git fetch`, the merge of `origin/<default>` into it, the hook-context write and the spawn), because refusing only the fast-forward and then merging into and spawning onto a tree a live worker is using defeats the guard; unsaved work alone does not stop that dispatch. The refresh itself never resets, force-checks-out, deletes or raises. A branch that is not on origin yet is now logged at DEBUG instead of WARNING (`fetch_feature_branch`'s `quiet_missing_ref`), which also quiets the first-time path. This closes the asymmetry only and does not claim to fix the byte-identical review verdicts that prompted the ticket.
