...
- **A resumed plan draft's stale round cap now resets on a genuine tracker-state change, and a plan-stage cap/stub hard-exit that settled an item this round now credits `resolution_consumed` like the other three park exits (#2154, closes #2160):** the plan-stage round cap (`<!-- plan-stage-scan-round: N -->`, cap 2) persisted across dispatches with no way to tell "still stuck" from "operator answered, round N should reset" apart, so a ticket that converged could still hard-park `ambiguity_scan_unconverged` on the next round purely from stale bookkeeping. The cap check now persists a `<!-- plan-stage-last-evaluated: ... -->` fingerprint (`updated_at`, the newest comment's id/timestamp, `queue_metadata.plan_approved_at`) alongside the round counter and resets `N` to 0 whenever a fresh live-fetch of those values diverges from what was last persisted — a new operator comment, a body edit, or an approval reply, per the existing #1653 human-facing vocabulary in `orchestrate-sprint/SKILL.md`. The cap check's `blocker.details` also now names the comment id/timestamp the round's fingerprint evaluated, closing #2160's ask. Separately, `resolution_consumed`/`resolution_evidence` (#1896) previously emitted only on the three Step 4c EXIT bullets — a round that settled an item via Step 1c.0 step 5 but then tripped the cap or stub hard-exit (routed through consolidated park, not a Step 4c bullet) was structurally excluded from ever reporting the settlement, even though the settlement genuinely happened. The emission rule now also covers the cap-check (`ambiguity_scan_unconverged`) and stub-check (`deferred_stub_unresolved`) hard-exits, unchanged in every other respect (still scoped to Step 1c.0 settlement only, per #2098). Both fixes are markdown-only — `cw.auto_dev_result.schema.AutoDevResult` and `cw.dispatch.productivity.extract_claim_evidence` were already status-agnostic for these two keys; two new regression-lock tests prove this rather than assert it from reading alone. A resumed draft also now runs a capped, independent-axis "body-consistency fold" (Step 1c.0) that re-spawns the Plan agent to mechanically rewrite `## Files Modified`/Phase 1/Phase 2/`## Touch-point Contract` for consistency with the round's settled decisions, so a resumed plan's own file/phase sections no longer drift out of sync with what was actually settled.
