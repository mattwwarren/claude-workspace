...
- **The codex reviewer no longer reads its own previous verdicts (#2213):** `_load_operator_comments` rendered the entire ticket comment thread into every reviewer role's prompt, eliding only the `### Settle a finding` payload from pipeline-authored comments (#2210). Everything else in every prior "Codex Review Verdict" comment went in — its superseded scope assessment, its stale out-of-scope file list and its MUST_FIX titles — under a header the prompt tells the model to treat as adjudication input. On a ticket eight rounds deep that was ~90 KB of the reviewer's own earlier output per role per round, and the observed result was exactly what #2213 reported and then could not explain from the (verified-correct) diff: byte-identical verdicts across consecutive runs, MUST_FIX findings citing files absent from the branch, and mis-anchored line citations — the model was quoting the thread, not the code. Comments carrying `AGENT_COMMENT_MARKER` are now dropped entirely; the `_elide_settle_section` regex is gone with them. Exclusion stays provenance-keyed: an operator's own comments, a pasted payload included, still render verbatim, and cross-round memory continues to flow through the structured ledger (`adjudicated_findings`, the voided-findings record), which is the channel designed for it. ADR-0016 invariant 7 is restated accordingly.
- **`.claude/review-verdict.md` is git-ignored and stamped with the ticket and sha it is about (#2279, ADR-0016 follow-up F10):** the per-ticket verdict file #2095 introduced was tracked, so each ticket's `git add -A` committed its own verdict, the merge carried it to `main`, and every sibling branch's next base sync delivered some *other* ticket's verdict into its worktree — observed as #2232's verdict sitting in #1409's worktree, and as branches hitting merge conflicts on the file. `/auto-dev-finalize` reads that file as the authority on whether to halt (#2205), so a ticket with real MUST_FIX findings could inherit a sibling's `Non-blocking` and ship, or a clean one could inherit a blocking verdict and park for findings never about it. The file is removed from the index and added to `.gitignore` (a test pins both), and `_persist_review_verdict` now prefixes the durable copy with `<!-- cw-review-verdict ticket=<id> reviewed_sha=<sha> -->` plus a human-readable line, so a finalize step or an operator can verify the verdict names the branch being finalized and refuse a mismatch. The posted tracker comment is unchanged. **Not addressed here:** the finalize-side ownership check itself lives in the skill, not in `cw`; this change gives it the header to check.
