...
- **`cw doctor` no longer flags a `session.spawned` roster orphan and dispatches a fix agent for a row that reverted mid-handoff, and its detection layer no longer risks reaping a live fix agent (#2142):** the review→fix-agent async handoff wrote `task.pending_fix_dispatch` and left the row `RUNNING` by design, but an unrelated non-sentinel `RUNNING→PENDING` revert (crash/phantom/stall/salvage) could fire on that row before the handoff was consumed; the next reconcile tick then dispatched the fix agent unconditionally, with no `task.status` check, correlating the new session to no row at all — it inflated `dispatch/tick.py`'s session-based `running_count` (feeding `cap_full`) while no task-row occupancy metric ever counted it. `_build_dispatch_jobs` now guards on `task.status != RUNNING` before building a dispatch job, dropping a stale handoff with an audit-trail event pair instead of spawning an orphan. Two new `cw doctor` wedge classes catch the residual and complementary shapes: `wedge/orphan-active-pending-row` (class-9) flags a `PENDING` row whose `session_id`/`fix_dispatch_session_id` still names a live daemon session — scoped to `PENDING` only, since a `BLOCKED_ON_USER` row can legitimately keep `fix_dispatch_session_id` pointing at a still-working fix agent (dirty-worktree salvage moves a `RUNNING` row to `BLOCKED_ON_USER` without clearing that field) and reaping it would kill the agent mid-commit; and `wedge/fix-dispatch-running-stale` (class-10) flags a `RUNNING` row whose dispatched fix session has gone quiet past the existing `fix_loop_await_deadline_minutes` threshold, catching the case ~15 minutes faster than class-8's general 45-minute bucket. `cw dev-queue status` also now annotates a per-client divergence between the session-based `running` count and the task-row `RUNNING` count, so this class of drift is visible without running `cw doctor` directly.
