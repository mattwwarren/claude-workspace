...
- **A headless Stop hook that fires after a park comment but with no sentinel now parks the row instead of leaving it RUNNING until the liveness ladder finds it (#2135):** a worker that posted its park/blocker comment to the tracker and then stopped without emitting an `AUTO_DEV_RESULT` left its dev-queue row `RUNNING` — holding a lane slot — until the 45-minute staleness bucket paged. `cw signal-stop` now derives the evidence from the session's own transcript: a completed, non-error `gh issue comment <ticket>` under one of the exit-only pipeline headers with the `<!-- cw-agent-authored -->` marker, made in the current run leg (after the last user re-entry record), with no pending background tasks and no sentinel — not even raw `AUTO_DEV_RESULT` framing text — after it. On that conjunction the row moves `RUNNING → BLOCKED_ON_USER` with the new `stopped_without_sentinel` disposition, charging no `unproductive_attempts`, and emits a `task.transition` plus a `session.needs_attention`. Evidence-driven, never a timer (ADR-0014): the session is left ACTIVE and the worker is not stopped, so a late sentinel still rescues the row (#918), and the disposition is deliberately outside concierge's false-park requeue set. The accompanying liveness change is signal-only — the sweep's recurring `session_unresponsive` distress fire is withheld for a row in exactly this state whose `session_id` matches the session, since it already paged once; bucket latching is untouched and a requeue re-enables the signal. Post shapes other than `Write` + a literal `--body-file` (or an inline `--body`) are documented false negatives and defer exactly as before. **Ships dark:** the park is gated by `park_on_abandoned_exit_enabled` in `orchestrator.yaml` (default `false`) plus a per-lane / per-ticket `park_on_abandoned_exit` map whose floor is `false`; with the master switch off a sentinel-less Stop defers exactly as it did before, and the transcript is not even scanned. Arming it is an operator action — see `config/CONFIG_REFERENCE.md`'s *Abandoned-Exit Park Enablement*. The detector additionally requires the `gh issue comment` text to **start a shell command**, so a heredoc, an `echo`, or a `$(...)`/backtick body that merely contains an example post can never be mistaken for one. The body is read from **that invocation's own shell segment only** and parsed argv-style, so a `--body`/`--body-file` from a later or embedded command is never borrowed. The gate is fail-closed and cheap: the Stop hook checks the `RUNNING` dev-queue row before touching config, memoizes the resolved config per process, and treats any config load failure or unknown client as disabled (one WARNING with the client name and error class, nothing raised out of the hook).
