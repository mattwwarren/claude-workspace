...
- **Locked re-finds of a RUNNING dev-queue row now match the caller's own row, not the first duplicate (#2219):** several write paths re-found "their" row under `dev_queue_lock()` by `(ticket_id, client, RUNNING)` alone and mutated the first match, so when two RUNNING rows shared a ticket (reachable via add-after-terminal plus `requeue --from-completed`) the wrong row could be reverted, parked, stamped with a session, or handed a review ledger. A shared `_find_running_row` helper in `src/cw/dispatch/claim.py` now re-finds by exact identity: `created_at` for claim-time callers (spawn-success stamp, spawn-failure reverts, the codex capability gate and dirty-worktree parks) and `session_id` for post-spawn callers. `src/cw/codex_background.py`'s session-id stamp and finding-disposition sync take a required `created_at`, and its failure-path revert matches the review's own session id. `src/cw/doctor/loop_health.py`'s `cw doctor --reap <SESSION>` revert, which had no client filter at all, now matches the reaped session's client and id.
