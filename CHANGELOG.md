...
- **`impl_comments_unreadable_after_regress` is now a recognized operator-unavailable blocker reason, and an unrecognized-reason breadcrumb now carries a slice of `blocker.details` (#2415):** `OPERATOR_UNAVAILABLE_BLOCKER_REASONS` gains the new reason for an IMPL Orientation step whose live tracker-comment fetch fails after reaching the stage via `_stage_regress` — the operator/tracker is unreachable, not the implementation broken. `_breadcrumb_for_reason` (Rule 5's `SESSION_NEEDS_ATTENTION` breadcrumb) now takes the full `blocker` payload and, for any still-unrecognized reason, appends up to 200 chars of `blocker.details` (trailing `…` only when truncated), so an operator can triage an invented reason from the breadcrumb alone. `auto-dev-impl.md`'s Orientation policy changes from warn-and-continue to hard-block when tracker comments are unreadable after a regress.
- **The opencode worker prompt now threads `--session-id` so it can push its own sentinel instead of only validating it (#2430):** `_spawn_fire_and_forget` passes the session id it creates into `preflight_fn` (widened from `Callable[[]]` to `Callable[[str]]`), and `OpencodeExecutor`'s pre-flight builds its PLAN/IMPL/REVIEW prompts with that session id via `build_stage_prompt`'s new optional `session_id` parameter. A known session id switches `_sentinel_rules()` to an emit-first flow — `cw result emit - --session-id <sid>` with a `field.path` fix-and-rerun loop — mirroring the detached Claude daemon's `emit_cli` primary path (#2382); `session_id=None` keeps the prior validate-only text byte-identical. Any other non-zero exit records a `cw_result_emit_fallback: <reason>` friction entry and falls back to validate-only, so a `cw` CLI outage never blocks the sentinel. `LocalExecutor`'s `preflight_fn` is updated mechanically (aider never uses the sid).
- **The crash-only local harvest sweep now has a codex branch with an audited clean-requeue gate (#2387, RFC 0014 A1):** `LocalLivenessHandle.backend` admits `"codex"`, and a codex session whose recorded PID has died is branched out of `_act_on_local_harvest_candidates` before any result synthesis — no sentinel is synthesized or routed, and its ticket is never counted as harvested. The four checks `cw.reconcile.codex_boot` requeues on (`reap_policy: auto`, codex fix loop off, worktree clean apart from the review verdict, HEAD unmoved since the review's baseline) are all evaluated; all passing requeues the task (`TICKET_REQUEUED`, reason `codex_harvest_clean_requeue`), any failing parks it `BLOCKED_ON_USER` (`codex_review_orphaned_at_harvest`). The session closes `COMPLETED`/`CRASHED` only after a `SESSION_COMPLETED` audit event carrying the executor, status transition, every gate check's result, and the dead PID/start-time is recorded; a failed audit write transitions nothing. Dormant until `CodexExecutor.spawn()` stamps a liveness handle (RFC 0014 A2, #2388).
...
- **First-class, auditable operator override for a codex MUST_FIX park (#2205):** `cw dev-queue approve <ticket> --override-must-fix --reason "..."` records a durable `TicketTask.must_fix_override` (dev-queue schema v42) on a row parked for `codex_must_fix_findings`. The record carries actor, reason, the verdict's `reviewed_sha` and the `fingerprint_v1` identity of every MUST_FIX finding, and is audited as a `TICKET_APPROVED` event. It is stamp-only, so the operator still runs `requeue --stage finalize`. Because that requeue clears `blocked_reason`, the override rides to the FINALIZE worker as `queue_metadata.must_fix_override` (cw-context schema v10). A new `auto-dev-finalize.md` "MUST_FIX Override Verification" step runs the stdlib `check_must_fix_override.py` gate before any PR is created or reused. A blocking verdict with no matching override (stale `reviewed_sha`, moved HEAD, a different finding set, or an unreadable verdict) now parks with `codex_must_fix_findings` instead of shipping. A matching override ships, with an `## Operator override` section in the PR body. `REVIEW_VERDICT_JSON_RELATIVE_PATH` moves to `cw.review_findings`.
- **The codex review fix loop now parks early when it is diverging instead of running to `--max-cycles` (#2394):** a new `codex_fix_loop_divergence` module tracks, per cycle, how many originally-found MUST_FIX findings have been resolved and how much cumulative churn (via the shared numstat parser) the loop has produced; `is_diverging()` trips once the loop has resolved no original finding for 2 consecutive cycles and cumulative churn exceeds `max(150, 50% of the pre-loop diff)`. `run_review_with_fix_loop` seeds the tracker from the cycle-0 findings and pre-loop diff, checks convergence first (a cycle that clears every finding still exits clean), and otherwise parks with the new `fix_loop_diverging` blocker reason, a per-cycle breakdown in `blocker.details`, and a new `review.fix_loop_divergence_detected` event.

### Changed

- **Headless workers now record their sentinel with `cw result emit -` instead of only validating it (#2382):** every `/auto-dev*` stage command's pre-emit gate is now the new *Sentinel emit rule* in `auto-dev.md` — run `cw result emit -` from the session worktree root, fix every `field.path: message` error it reports and re-run until it exits 0, then frame the recorded JSON. The recorded copy is what the Stop hook routes on (#536 emit precedence), so the bytes cw validated are the bytes cw acts on; the framed transcript block remains the display copy and the parse fallback when nothing was recorded (denied or unknown command, opencode workers). `cw result emit` validates strictly, without the transcript parser's leniency coercions. A worker that falls back to the frame (denied or unknown command, or any other non-validation failure) records `cw_result_emit_fallback: <reason>` in `friction_highlights` so the fallback rate is visible. RFC 0012's out-of-scope bullet on worker-invoked emit is superseded by a new Amendment there.
- **`RealAiderRunner`/`RealOpencodeRunner` and `LocalExecutor`/`OpencodeExecutor` now share one spawn/launch/harvest implementation instead of two near-identical copies (#2369):** a new `cw.executor_launch._launch_logged_subprocess` replaces the byte-identical `launch` methods on both runners, and a new `cw.executor.core._spawn_fire_and_forget` now owns session create, pre-flight, launch, liveness-handle store, door completion and the crash guard for both `LocalExecutor.spawn` and `OpencodeExecutor.spawn`, with each executor injecting only its own pre-flight and a pre-bound blocked-result constructor. Harvest now dispatches on the new `LocalLivenessHandle.backend` field (state schema bumped to v19, `CW_STATE_SCHEMA_VERSION = 19`) via a backend-keyed synthesizer registry instead of probing for `.cw/opencode.log`, so an opencode run that never wrote its log is still harvested as opencode and a stray log in an aider worktree no longer misroutes it; unknown backends fall back to git synthesis, matching the previous default. A pre-v19 handle is migration-filled with the new `DEFAULT_LOCAL_LIVENESS_BACKEND` ("aider") constant, and the migration now logs a warning naming the session id when it does. `LocalExecutor`/`OpencodeExecutor` constructors are retyped from `AiderRunner`/`OpencodeRunner` to the shared `FireAndForgetRunner` Protocol (no behavior change).
- **`cw.dispatch.claim` is now a package split by concern instead of one 1800-line module (#2378):** `claim.py` becomes `claim/__init__.py` (pure re-export of all 39 names) plus six new submodules — `events.py` (the five per-task `dispatch.tick`/`SESSION_NEEDS_ATTENTION` emitters), `claimed_row.py` (`_find_running_row`, `_revert_claimed_task_to_pending`, `_park_running_task_blocked_on_user`, `_stamp_spawn_success`), `codex_capability.py` (the TTL-cached capability probe/gate, now also home to `_SpawnOutcome`), `lane_stats.py` (`_lane_occupants_for_client`/`_lane_stats_for_client`), `screening.py` (the occupancy/backoff/fix-dispatch/stale-PR/attempt-ceiling screen, `resolve_occupied_ticket_ids`, `_claim_next_pending`), and `spawn.py` (the worktree-provision + spawn path, `_spawn_claimed_task`, the hook-context conflict router, the approved-plan bypass). All moves are verbatim (no behavior change); the package no longer re-exports any imported name, so a stale flat-path test patch now fails loudly instead of silently no-opping.

### Fixed

- **`plan_draft_fingerprint` can no longer reach the queue malformed, and `cw result emit` recomputes it from the draft on disk (#2382):** a 62-character copy of a 64-character digest was stamped by `cw dev-queue approve` and re-opened the approval gate on every round. The `AutoDevResult` schema now rejects any non-null fingerprint that is not a 64-character lowercase hex digest (naming the length), `_stamp_plan_approval` records `None` with a warning for a pre-existing malformed value, and `cw result emit` treats a non-null fingerprint only as a claim that a draft is in hand — the digest it records is computed by cw from the target session's own `<worktree>/.cw/plan-draft.md` (never the invoker's cwd, so `--session-id` from another directory cannot bind a foreign draft; or the new `--plan-draft <path>` option), exiting 1 with no state modified when a claim has no draft file. A session that already carries a terminal result short-circuits to the existing 'already recorded' outcome before binding or validation. An already-persisted `last_result` carrying a malformed fingerprint is reconstructed with the value nulled (warning names the length), so the new validator cannot make a legacy recorded result unreconstructable on the Stop-hook or phantom-sweep path. The computation moved to `cw.plan_fingerprint.compute_plan_draft_fingerprint` (shared with the #2342 promotion gate) and now strips all six bookkeeping-line kinds the named rule lists, where the promotion gate's private copy stripped three.
...
- **A completion-routing race can no longer let the generic `dirty_worktree` backstop clobber a session's own already-classified result (#2355):** the codex fix-loop's completion path writes `Session.status=COMPLETED` and `last_result` synchronously, but the `SESSION_COMPLETED` event that lets `consume_completed_sessions()` route the task with the correct, already-classified reason fires later (after a slow GitHub API call). If `reconcile()` sampled the worktree in that window, a local-commit/push failure looked dirty and got stamped `disposition="dirty_worktree"` over the more specific reason `last_result` already carried. `revert_timed_out_tasks()`/`revert_completed_silent_tasks()` now leave a RUNNING task untouched entirely for 60s after `Session.completed_at`, giving the event-driven router a chance to run first; a missing `completed_at` gets no grace (fail-safe), and behavior past the window is unchanged.
- **`cw review verify-fixes` no longer downgrades a `"fixed"` disposition to `"dropped"` when the fix deleted the whole cited file outright (#2420):** `_fix_is_substantiated` now credits a fix diff carrying a `deleted file mode`/`+++ /dev/null` block for the finding's file as touching every line inside it, for both file-level and line-anchored findings, so no spurious `review.fixed_disposition_downgraded` event or downgraded-count is produced for a correct whole-file fix.
...
- **`occupancy_gate_enabled` operator escape hatch for #2077's pre-claim worktree-occupancy screen (#2396):** new `OrchestratorConfig` field (default `true`, gate stays enforced), plus a nullable per-client `ClientConfig` rollout override, resolved and gated exactly like `pr_gate_enabled` — the global setting is the explicitly audited emergency control, while a client override can stage the bypass before the I/O-bearing `resolve_occupied_ticket_ids` precompute runs at all. `live_home_reason` fails a claim closed on any indeterminate read, so a broad misclassification could otherwise silently stop every PENDING task for a client from being claimed with no `dispatch_tick` failure; either setting skips the precompute and falls back to #2077's post-claim `WorktreeOccupiedError`/`HookContextConflictError` handling, which still refuses a genuinely occupied worktree, just one claim later.
...
- **Aider and opencode workers no longer die when Ctrl-C hits the parent `serve` process (#2367):** their children inherited the parent's foreground process group, so a `SIGINT` delivered to that group killed the fire-and-forget worker along with the serve process it was meant to outlive. `RealAiderRunner` and `RealOpencodeRunner` now launch their children with `start_new_session=True`, putting each worker in its own session so it survives the parent's interrupt.
...
- **A REVIEW session's fix-loop dispatch handoff no longer goes stale and respawns a plain review, looping (#2204):** when a REVIEW session's sentinel reported `blocker.reason=fix_loop_pending_dispatch`, its dev-queue row stayed RUNNING by design, but the session itself went DAEMON COMPLETED — and the generic crash/phantom/stall/timeout backstop reverts (`_revert_running_tasks_for_sessions`, `revert_timed_out_tasks`, `revert_completed_silent_tasks`, `_apply_phantom_queue_mutations`) only exempted a row carrying a mid-turn usage-limit act, not one carrying an unconsumed `pending_fix_dispatch`/`fix_dispatch_session_id`. A same-tick revert reparked the row PENDING before `run_fix_dispatch()` ever saw it, which then classified the stranded handoff as stale, dropped it, and paged `SESSION_NEEDS_ATTENTION`/`STAGE_ERRORED` — leaving the ticket to be reclaimed into a fresh, looping `/auto-dev-review`. A new shared `_is_backstop_exempt` predicate composes the existing `_is_fix_dispatch_held` claim-path exemption with the usage-limit-act check, and all four backstop-revert sites now consult it.

## [1.57.0] - 2026-09-25

### Fixed

- **A blocking fix-loop survivor is no longer rendered as suppressed, and the blocking-branch history note no longer goes negative (#2352):** `_survivors_only_verdict` stamped a still-blocking cap-exit survivor with the same `"deferred"` disposition literal `review_adjudication` uses for a non-blocking, operator-issued decision, so the renderer displayed a blocking MUST_FIX as suppressed. A new `"unresolved"` `Disposition` value is now used exclusively for this producer, and the renderer's no-annotation check treats it like `"fixed"`. Separately, once `_admit_new_must_fix` lets a genuinely-new MUST_FIX finding into the fix loop mid-run, `review.deferred` can exceed `review.must_fix_initial`, and subtracting the two produced a negative "resolved" count; the history note now renders both counts independently when `deferred` exceeds `must_fix_initial` instead of deriving a resolved count by subtraction.
- **The codex review fix loop now pushes every commit it makes, instead of leaving it only in the local worktree (#2354):** `_commit_fix_cycle` committed each fix cycle and returned without pushing, so an unpushed fix commit was either discarded by finalize's reset-to-origin (#2216) or parked the ticket `dirty_worktree` when the session exited (#2324). Each committed cycle now runs `git push origin HEAD:refs/heads/<branch>` and verifies the fetched origin tip matches, and a push failure or mismatch parks through the existing commit-failure path. As a backstop, a clean review exit whose HEAD is still not on `origin/<branch>` gets one self-heal push, and parks as `codex_unpushed_at_exit` if that also fails. Finalize's Step 4c.2 now pushes any local commits origin lacks before `git checkout -B` resets to origin, and BLOCKs naming the SHAs rather than resetting when that push fails.
- **Pipeline PR titles now describe the whole change instead of the branch's first commit (#2362):** with no `--title`, `/ship-it`'s Tier 3 titled a PR after the branch's oldest substantive commit, usually a building block such as a helper or a parser tweak, and squash-merge carried that subject into main. Finalize now composes `<type>(<scope>): <summary> (#<ticket>)` from `.cw/plan.md`'s `## Summary` (falling back to the ticket title) and passes it to `/prep-pr --title`.
...
- **A usage limit hit mid-turn by a running worker is now detected, arms the lockout, and frees the slot without charging an attempt (#2324):** a headless worker whose turn hit the account usage limit never left the daemon roster — its transcript just ended on `You've hit your weekly limit · resets Sep 26, 11pm` with no sentinel — so the phantom sweep never saw it, no `usage_limited_until` lockout was armed, and the RUNNING row held its lane slot for over an hour. A new reconcile sweep, `cw.reconcile.usage_limit_mid_turn`, detects a roster-present session whose last content-bearing transcript record is that message with no sentinel anywhere, arms that client's lockout to the message's parsed reset time (or `usage_limit_backoff_seconds` when it does not parse), and emits `dispatch.usage_limit_armed` plus `session.needs_attention` with `paused_status: "usage_limited_mid_turn"` naming the reset. The row move is gated by the lane's `reap_policy`, like the phantom sweep: under `auto` the row reverts to PENDING with `next_eligible_at` at the reset (so the claim gate releases it then), the session closes COMPLETED with the new `completed_reason: "usage_limited"` (not a crash), and its surface is stopped; under the `signal_only` default the row parks BLOCKED_ON_USER with `disposition: "usage_limited_mid_turn"` and the session is left alone for an operator. Neither charges `unproductive_attempts`. Because that act spans the daemon, `sessions.json`, the dev queue and the event inbox, which cannot commit together, it is decided by one write-ahead intent on the row — the new `TicketTask.usage_limit_act` (dev-queue schema v41; older rows load with it unset) — and every reconcile tick that finds the intent resumes the act, doing each remaining step once: arm the lockout, record the audit events (at-least-once, keyed by a new `act_started_at` payload field), and under `auto` re-check the tail, stop and close the session, then transition the row, which clears the intent. A tail that changed before the stop abandons the act and leaves the session running. While the intent is set, the phantom sweep, the completed/timed-out session backstops and the liveness page leave the row alone, so an act interrupted at any point is finished on a later tick without charging an attempt. `parse_usage_limit_reset` now also reads the month-day form (`resets Sep 26, 11pm`) this message uses — previously it recognised only a bare time or a weekday, so a real weekly-limit reset always fell back to the flat one-hour window. `resolve_usage_limited_until` moved from `cw.dispatch.loop` to `cw.dispatch_state`, next to a new `record_usage_limit_armed` that is now the one `dispatch.usage_limit_armed` audit for both the spawn path and the mid-turn sweep, so reconcile shares them without an import cycle. See `docs/events.md`.
- **`cw doctor` now surfaces a DAEMON session stuck `ACTIVE`/`IDLE` with no daemon surface and no local liveness record (#2237):** such a row was skipped by `compute_drift()` (so neither the reconcile phantom sweep nor `wedge/active-no-daemon-entry` saw it) while dispatch still counted it against the client ceiling. The new advisory wedge class `wedge/active-null-liveness-orphan` reports it past `SPAWN_GRACE_SECONDS` with the exact `cw spawn close <id>` command, and `cw doctor --reap` never mutates it (ADR-0014: absence plus elapsed time is not evidence of death). `CODEX_BACKEND` sessions are excluded, since `codex_boot`/`codex_reparks` own them (#2285/#2307). A session whose backend cannot be resolved (no ticket id in its name, no `clients.yaml` entry, no RUNNING row, or a RUNNING row owned by a different session) still gets a finding, with a "could not resolve its executor backend" recipe.
- **An ambiguous session name no longer resolves to an arbitrary row (#2237):** `find_by_name_or_id` returned the newest row whose name matched, so `cw spawn close <client>/auto-dev/<ticket>` could hit an already-completed sibling and refuse while the live same-named session kept its slot. A name that matches more than one session now raises `AmbiguousSessionIdentifierError`, and every name-accepting command (`bg`, `done`, `resume`, `peek`, `start --parent`, `spawn close`, `spawn complete`, `orchestrate workers`/`parent`, `result emit --session-id`) exits non-zero listing each candidate as `<id>  <status>  started <started_at>` and asking for an id. An id is always unambiguous. `cw bg --all` now backgrounds each session by id, so duplicate names cannot trip it. `tests/test_ambiguous_session_lookup_guard.py` pins every `find_by_name_or_id` call site to a classified allowlist.
...
- **Locked re-finds of a RUNNING dev-queue row now match the caller's own row, not the first duplicate (#2219):** several write paths re-found "their" row under `dev_queue_lock()` by `(ticket_id, client, RUNNING)` alone and mutated the first match, so when two RUNNING rows shared a ticket (reachable via add-after-terminal plus `requeue --from-completed`) the wrong row could be reverted, parked, stamped with a session, or handed a review ledger. A shared `_find_running_row` helper in `src/cw/dispatch/claim.py` now re-finds by exact identity: `created_at` for claim-time callers (spawn-success stamp, spawn-failure reverts, the codex capability gate and dirty-worktree parks) and `session_id` for post-spawn callers. `src/cw/codex_background.py`'s session-id stamp and finding-disposition sync take a required `created_at`, and its failure-path revert matches the review's own session id. `src/cw/doctor/loop_health.py`'s `cw doctor --reap <SESSION>` revert, which had no client filter at all, now matches the reaped session's client and id. The dispatch loop's `SESSION_COMPLETED` consumer (`src/cw/dispatch/loop.py`) now routes through the same helper keyed on the event's own `session_id`; a genuinely legacy event carrying no `session_id` is dropped and logged instead of falling back to a bare `(ticket_id, RUNNING)` match, closing the last such site in `src/cw/`.
...
- **`cw dev-queue requeue` now fails closed when the daemon roster is unreadable, instead of assuming no session is live (#2275):** the requeue path already refused to requeue a ticket with a daemon-live session (`RequeueLiveSessionError`); it now also refuses when the roster itself can't be read or parsed, via a new fail-closed `NativeDaemonClient.list_live_session_short_ids_fail_closed()` (returns `None`, distinct from an absent roster's empty set) and a new `RequeueRosterUnreadableError` subclass carrying the roster path in its message. `cw dev-queue drain` reports this refusal as a distinct `skipped_roster_unreadable` outcome (alongside the existing `skipped_live_session`), via a shared `classify_requeue_live_session_error` helper so the CLI, `drain`, and the `auto-fix-ci` review recipe classify the same error the same way. The `auto-fix-ci` recipe also rolls back its one-shot `auto_fix_ci_fired_at` latch on either refusal (compare-and-clear, so a concurrent change to the row is never clobbered), so a transient live-session or unreadable-roster refusal doesn't permanently block a later, successful redispatch attempt. Documented in `docs/dispatch-runbook.md`.

### Fixed

- **The codex reviewer no longer reads its own previous verdicts (#2283):** `_load_operator_comments` rendered the entire ticket comment thread into every reviewer role's prompt, eliding only the `### Settle a finding` payload from pipeline-authored comments (#2210). Everything else in every prior "Codex Review Verdict" comment went in — its superseded scope assessment, its stale out-of-scope file list and its MUST_FIX titles — under a header the prompt tells the model to treat as adjudication input. On a ticket eight rounds deep that was ~90 KB of the reviewer's own earlier output per role per round, and the observed result was exactly what #2213 reported and then could not explain from the (verified-correct) diff: byte-identical verdicts across consecutive runs, MUST_FIX findings citing files absent from the branch, and mis-anchored line citations — the model was quoting the thread, not the code. Comments carrying `AGENT_COMMENT_MARKER` are now dropped entirely; the `_elide_settle_section` regex is gone with them. Exclusion stays provenance-keyed: an operator's own comments, a pasted payload included, still render verbatim, and cross-round memory continues to flow through the structured ledger (`adjudicated_findings`, the voided-findings record), which is the channel designed for it. ADR-0016 invariant 7 is restated accordingly.
...
- **Codex reviewer roles no longer self-report `status="degraded"` for gates their own rubric never mandated (#2292):** the shared reviewer prompt's output-schema rules only ever discussed degradation and said nothing about a sandbox baseline or where degraded ends and failed begins, so non-exempt roles (e.g. Data Safety Reviewer) reported `"degraded"` for not running the test suite, `uv`, a linter, or a type-checker even when their rubric never asked for it — parking otherwise-clean reviews at the `review_health_gate`. `_OUTPUT_SCHEMA_RULES` now states plainly that a read-only sandbox is the expected baseline, that being unable to run an unmandated gate is not degradation, that a mandated-but-unrunnable gate is still `"degraded"` (never `"failed"`), and reserves `"failed"` for being unable to evaluate the diff itself.
- **A headless `/prep-pr` no longer wedges finalize by backgrounding a quality gate, the liveness sweep names that wedge, and `cw queue peek` shows the PR it already opened (#2251):** `/prep-pr` Step 7 still re-issued a gate that outran its foreground ceiling with `run_in_background: true` and waited for the harness's completion notification, the exact pattern `auto-dev.md`'s Worker Execution Discipline (#2157) forbids for headless Bash calls: a raw Bash background call has no Stop-hook `background_tasks` tracking, so the completion lands in the transcript as an unconsumed `queue-operation` enqueue record and nothing ever starts the turn that would read it. Step 7's background switch is now interactive-only; in headless mode the foreground ceiling is the gate's total budget and a timeout emits the existing `gate_timeout` block immediately. Interactive mode, `prep_pr_state.py` and its ceilings are unchanged. The liveness sweep gains a fourth distress reason, `unconsumed_queue_notification`, for a stale session whose transcript's last record is exactly such an enqueue; it is gated like `dangling_tool_use` (no outstanding subagent spawn), takes priority over it, and carries the redacted, truncated notification text in `breadcrumbs`. It still fires only at the top staleness bucket and remains signal-only (ADR-0014). `cw queue peek` now falls back to the last GitHub PR URL found in a `tool_result` block when no `AUTO_DEV_RESULT` sentinel names a PR, so a session wedged before its finalize sentinel no longer shows `pr -` after `gh pr create` succeeded; URLs in assistant prose are not treated as evidence.
...
- **`create_worktree` can now fetch and fast-forward a behind, unoccupied worktree on the reuse path, closing the reuse-path fetch asymmetry (#2213):** the first-time path fetched `origin/<branch>` before resolving the branch, but reuse returned the existing worktree untouched, so a per-ticket worktree reused across pipeline stages could sit on a stale HEAD while `origin/<branch>` had moved on. The refresh is opt-in via a new keyword-only `refresh_on_reuse` (default `False`, today's path-only behavior); the dispatch claim path and `dispatch_fix_agent` pass `True`, interactive `cw start` does not. With it set, an occupancy check with local reads only runs first: an unexpected checked-out branch, unsaved work (uncommitted, untracked or unpushed, per `unsaved_work_reason`), a live session homed on the path (the shared `live_session_worktree_paths`, extracted from the worktree GC, narrowed to catch only `OSError`/`ValueError`, and reporting `None` for "indeterminate" so each caller decides what that means), or a live worker in the daemon roster homed on it (new fail-closed `NativeDaemonClient.list_live_worker_cwds`, sharing one roster parser with `list_live_session_short_ids`; a roster that is not valid UTF-8 is indeterminate too, not a crash) all stop the refresh before any network call, with a DEBUG log naming the path and reason. Those refusals mean two different things (`RefreshOutcome`), and one flag used to carry both. An unexpected branch or unsaved work is `NOT_REFRESHED` (the tree is the caller's, just not up to date: it is returned and used as-is), and so are a diverged branch, a refused fast-forward, a failed fetch, an absent remote branch and an OS error; only a fast-forward is `REFRESHED`. A live session, a live roster worker, or an indeterminate read of either is `OCCUPIED_BY_LIVE_SESSION` (another worker may be operating in the tree), which `create_worktree` now RAISES as a new `WorktreeOccupiedError` (carrying `path` and `reason`; a `WorktreeError`, but deliberately not a `StaleWorktreeError`, so nothing ever removes an occupied tree) instead of returning a path a spawn could use. Paths are compared after normalization on both sides (`Path.resolve()`, plus an explicit `stat()` because Python 3.13's non-strict `resolve()` silently swallows a symlink loop), and unreadable session state, an unreadable roster or ANY path that cannot be normalized (`ELOOP`, `EACCES`, `ENOTDIR`, `ENAMETOOLONG`, every other `OSError`) fail closed (occupied); the one exception is a path that does not exist (`FileNotFoundError`), which is a fact rather than a failure to look, so a stale record of a deleted worktree cannot veto every refresh. The reuse refresh fails closed while the worktree GC, which shares `live_session_worktree_paths`, deliberately keeps failing open: do not "fix" one into consistency with the other. Otherwise it best-effort fetches `origin/<branch>` (a network call that can be slow). `fetch_feature_branch` now returns a `FetchResult` (a three-state `FetchOutcome` plus git's exit status and first stderr line as `reason`, so a "not refreshed" says whether it was auth, network or a missing remote) instead of a bool, because the two non-success cases need opposite handling: only `FETCHED` fast-forwards; `BRANCH_ABSENT` (the branch is not on origin yet, an expected state for a fresh ticket) proceeds without a refresh and is not reported as friction; `FAILED` skips the fast-forward entirely rather than moving HEAD to the stale tracking ref it leaves behind, and is reported with git's reason. Every dispatch on `FetchOutcome` or `RefreshOutcome` is an exhaustive `match` ending in `assert_never`, so a new member is a type error and an unrecognized one is never read as fetched. The full occupancy check is re-run immediately before `git merge --ff-only` to `refs/remotes/origin/<branch>`, which happens only when HEAD is a strict ancestor; the re-check narrows but does not eliminate the window in which a session can start, which is accepted because holding a lock across a network fetch is worse. Diverged and `--ff-only`-refused worktrees are left as they were with a WARNING. Submodule synchronization after a fast-forward is deliberately NOT part of this change (it is tracked separately and will be built on these refusal semantics). `create_worktree` takes an optional `refresh_report` (`ReuseRefreshReport`) out-parameter: its `notes` receive exactly one single-line entry, naming the worktree and the reason, for each refresh failure the caller cannot otherwise see (fetch failed, fast-forward refused by git, diverged, an OS error), and its `outcome`/`reason` carry the verdict, filled in before `create_worktree` returns or raises (the occupancy refusal does not depend on the report: it is the exception). `dispatch_fix_agent` prepends the notes to the fix agent's prompt and, on `WorktreeOccupiedError`, raises the existing transient `HookContextConflictError` before ANY later mutation of that worktree (the `git fetch`, the merge of `origin/<default>` into it, the hook-context write and the spawn), because refusing only the fast-forward and then merging into and spawning onto a tree a live worker is using defeats the guard; unsaved work alone does not stop that dispatch. The dispatch claim path (`dispatch/claim.py`) previously passed no report and so discarded the refusal and spawned a second worker into the occupied tree; it now handles `WorktreeOccupiedError` with its own narrow `except` (not the stale-worktree branch, which removes a tree, so an occupied tree is never removed): nothing is spawned, and the row returns to PENDING as a release of the claim (`attempts` restored, no `unproductive_attempts` charge, `spawn_error_count` untouched, held off for 30 seconds so the same tick cannot re-claim it and starve the tickets behind it). The outcome sets a new `occupied` flag rather than `spawn_error`, so a long-lived occupant never trips the lane circuit breaker. The stale-worktree branch itself (a wrong-branch tree that `create_worktree` refuses with `StaleWorktreeError`, which the claim path force-removes so the task cannot spin on it) now consults liveness FIRST, with the same predicate, made public as `live_home_reason`: a live session, a live roster worker or an unreadable state or roster (fail closed) defers the claim through the same occupied-claim handling and removes nothing. Only an unoccupied tree reaches the existing dirty check (parked as `dirty_worktree`), and only an unoccupied, clean one is force-removed. Liveness precedes dirtiness because a live occupant is the stronger reason to leave the tree alone and the dirty check shells out to git inside a directory that worker may be mutating. A session record that names the reused worktree is now refused at claim time, before the spawn-time hook-context guard (#1674) is reached; that guard still covers a record that does not name it. The worktree GC is unchanged and keeps failing open. The refresh itself never resets, force-checks-out or deletes, and never raises for a git or OS failure; the only refresh refusal that raises is an occupied worktree. A branch that is not on origin yet is now logged at DEBUG instead of WARNING (`fetch_feature_branch`'s `quiet_missing_ref`), which also quiets the first-time path. A fast-forward that actually moves HEAD now records one audit-only `worktree.fast_forwarded` event (client, ticket, branch, path, full before/after SHAs; `create_worktree` gained a keyword-only `ticket_id` for it, passed by the claim and fix-agent callers), and nothing is recorded on any path that moved nothing; a failed audit write is logged and never undoes the fast-forward. The fetch-failure warning dedup is now keyed on `(client, outcome, reason)` instead of the client alone, so a different failure for the same client (an auth error after a network error) is reported rather than silenced. The same dedup now also covers the workspace-missing and OS-error (for example a missing `git` binary) failures, which previously warned on every tick for the life of the dispatch loop. This closes the asymmetry only and does not claim to fix the byte-identical review verdicts that prompted the ticket.

### Added

- **`cw session send`, an inbound channel for a paused session, and an `AWAITING_OPERATOR` row in `cw queue peek` (#2212):** a worker that paused to ask a question parked its dev-queue row `BLOCKED_ON_USER` and then sat there — `cw queue peek` only ever queried `RUNNING` rows, so the parked row was invisible where an operator was looking, and the only way to answer was lossy: close the session, requeue the ticket, and let a fresh worker rediscover the context. `cw session send <session> --message/--message-file` now appends the answer to a per-session, append-only mailbox at `~/.local/share/cw/inboxes/<session_id>/inbox.jsonl` (paired with a consumed-cursor file, built on the same `flock`/JSONL/cursor primitives as the orchestrator event bus but scoped per session, so one session's appends never serialize behind another's), emits `session.message_sent`, and then attempts to wake the session — preserving the transcript, the worktree, the dev-queue row and its `BLOCKED_ON_USER` status. `cw queue peek` now enumerates `BLOCKED_ON_USER` rows alongside `RUNNING` ones and tags them `AWAITING_OPERATOR`, with the reason read from `disposition` first (the field the primary park path actually stamps), then `blocked_reason`, then `advisory_note`; such a row returns before the age/idle ladder runs — it is supposed to be idle — and is never listed under "Suggested stops." **The wake is a `--resume` continuity respawn, not a live mid-turn injection, and its scope is restricted accordingly:** it fires only for a session whose owning task is `BLOCKED_ON_USER` or whose `SessionStatus` is `IDLE`, and it makes **zero** calls into the daemon client for any other session. There is no way to deliver into a running `claude --bg` turn (#1889's spike established this), and `SessionStatus` carries no signal finer than ACTIVE/IDLE to tell "mid-tool-call" from "paused, safe to interrupt" — inventing that signal, and live-session delivery on top of it, is deferred to **#2255**. **Operator-visible:** sending to a live, mid-task session queues the message and prints `Warning: not delivered yet`, exiting **0** — durable queuing, not delivery, is the command's reliability bar, and until #2255 that is the expected outcome rather than a failure. Exit 1 is reserved for the cases where nothing was queued at all (unknown session, terminal session, bad flags). Sending does **not** transition the dev-queue row; releasing a park stays the separate, explicit action it always was. `--author` is audit-only and defaults to the local OS username. `queue_peek.load_running_tasks` is renamed `load_attention_tasks`; `AWAITING_OPERATOR_SIGNOFF` is deliberately excluded from it, being RFC 0007's distinct ship-signoff park. See ADR-0017 and `docs/dispatch-runbook.md` §6a.
- **`attention_watch.py` replaces `attention_monitor.sh` as the `orchestrate-sprint` wake-on-event watcher, with a client/lane-scoped idle backstop (#2250):** the sprint monitor's polling shell script is retired in favor of a Python watcher that blocks on `cw event tail --follow` for near-instant wake-on-event delivery, while still enforcing a `--max-idle-seconds` backstop so a filtered watch under a steady, non-matching event stream still ends on time instead of running forever. The resume stamp is now written atomically, and `SKILL.md`, `docs/dispatch-runbook.md`, and `docs/headless-contract.md` are updated to point at the new script; `attention_monitor.sh` and its tests are deleted.

## [1.49.0] - 2026-09-22

### Added

- **Per-record rollback and drift surfacing for the review settle ledger, plus `cw review dispositions` and an enforced arming precondition (#2232, closing the gap ADR-0016 named):** #2210 shipped the durable cross-round disposition ledger and its gated fuzzy claim tier, and its own ADR recorded the hole left behind — "entries never expire, and there is no per-record rollback command" — as a **precondition for ever arming the claim tier**. Both halves now exist. **Rollback is a third `Outcome` value, `REVERSED`, produced by `cw review settle` itself:** paste the SAME `file` and `summary` the original settle used with `"outcome": "REVERSED"` and post the marker, and the ledger's existing newest-`recorded_at`-wins merge withdraws the record. No new command and no second write path, so every guard settle already has — the dispatch-worker refusal, the resolved `actor`, the audit-before-effect ordering, the all-events-then-one-write atomicity — applies unchanged, and `merge_finding_dispositions` stays the one chokepoint (ADR-0016 invariant 11). A reversed record matches neither the exact nor the claim tier (both already ignore anything that is not `REJECTED`, so neither matcher changed) and is **not** rendered into the reviewer's binding "previously adjudicated" block — a withdrawal is the *absence* of a decision, so a `REVERSED` line telling the model the finding was decided would assert the opposite of what it means. It emits `review.finding_disposition_reverted` rather than `review.finding_settled`, because the event *type* carries the semantic in this region by convention: an operator asking "what have I withdrawn" runs one `--type` query instead of filtering settles on payload content. **Staleness is surfaced, never silently expired:** the ledger's identity is deliberately not evidence-anchored (an evidence anchor lapses the moment code moves, which is the memory loss #1838 exists to remove), and the accepted cost is a suppression that outlives the code it was granted for. On every review pass a matched record is now checked — did that file change between the sha the record was settled against and the sha being reviewed? — and on drift the record is **not applied**: the finding keeps blocking, the posted comment reports it under "Settled findings re-raised (the code moved)" with both shas, and a `review.finding_disposition_stale` event records it. The ledger entry is left intact and still applies to any pass where its file has not moved; silent expiry is precisely the invisible mechanical act ADR-0016 exists to refuse. The check fails toward surfacing — an unresolvable ref (a rebase or force-push orphaning the settled sha) or an unreadable worktree reads as drift, so you re-settle rather than suppress against code nobody adjudicated. Its event is advisory and warns on a failed write rather than aborting the pass, the mirror of `review.finding_settled`'s refuse-the-whole-settle posture and for the same reason read the other way round: there the audited act *creates* a durable suppression, here it is *declining* to suppress, which is already the safe outcome and already visible twice over. **`cw review dispositions <ticket>` is the missing inspection view:** read-only and lock-free (modelled on `cw dev-queue tasks`), it lists every record bound to a ticket with an explicit OUTCOME column and, given `--worktree`, a STALE column. Every record is shown whatever its outcome — filtering `REVERSED` out would hide a withdrawal's existence entirely, which is a worse safety failure than showing one clearly labelled. It reads the dev-queue row's last-synced ledger, refreshed after each review pass, not a live fetch of the ticket thread. **The arming precondition is enforced, not merely documented:** a new `disposition_drift_check_enabled` gates the automatic check — a default-**on** global in `orchestrator.yaml` with a bidirectional per-lane override in `clients.yaml`, shaped on `busy_wait_guard_enabled` (#1946) rather than on the claim tier's master-switch-then-floor shape, because this is a *check* presumed wanted rather than a *feature* presumed unwanted — and resolving it `false` on a lane whose claim tier resolves `true` now raises `ClaimTierArmingError` instead of running a fuzzy suppression with its drift protection removed. The refusal fires at the one point both gates are resolved for the same task and lane, before the review is entered, and surfaces through the review thread's existing exception path. The gate governs only that automatic call site: `cw review dispositions --worktree` always computes staleness, because an operator who turned the gate off to debug is the one who most needs the diagnostic. **Operator-visible:** a lane running the claim tier must now also run the drift check; and a settle whose file has moved will re-raise once, reported on the comment, until it is re-settled against the current code or reversed. `docs/events.md`, `config/CONFIG_REFERENCE.md`, `docs/dispatch-runbook.md` and ADR-0016 are updated. **Review round 2** widened three of those contracts. `review.finding_disposition_stale` now carries the full provenance set (`outcome`, `reason`, `actor`, `recorded_at`) alongside the two shas, because a consumer triaging a drifted suppression is asking who silenced the finding and why. `review.finding_disposition_reverted` gained a second emitter: the review pass folding the ticket thread's marker into the durable ledger now records a withdrawal arriving that way, which previously reached the ledger with no audit event at all — it fires once, on the state change, and warns rather than aborting the pass on a failed write. And `cw review dispositions` gained REASON and AGE columns, and now renders STALE as `?` for a record carrying no reviewed sha rather than as `no`: "nothing to compare against" is not "verified unchanged". Both new `git` subprocess calls also run with `GIT_*` stripped, so a `cw` process running inside a git hook cannot have its head lookup or drift diff silently redirected to the hook's repository.
- **`cw agent-spawn-pre` now refuses a forked or untyped subagent in a headless worker, and a `Read Only Helper` agent that cannot write at all (#2211, following #2017):** an impl worker forked a harness subagent for a read-only tracker lookup; the fork inherited the parent's tools *and* its implementation mandate, edited a source file and a test, committed, and pushed to the live feature branch before the parent's stop message won the race — and because a harness subagent takes no `cw` roster entry, cw could not see it start, observe it, or stop it. The existing PreToolUse hook on the `^(Agent|Task)$` matcher (no new hook, no extra interpreter start, and already-provisioned worktrees pick it up on upgrade) now also applies a spawn-shape policy from the new leaf module `cw.cli._subagent_policy`: in a worker whose `.claude/cw-context.json` reports `headless: true`, a `subagent_type` of `fork` (any case) or blank exits 2 with the reason on stderr and writes no stamp, and a spawn that names **no** `subagent_type` at all is refused on exactly the same terms — an unnamed spawn is unrostered for the same reason a forked one is, and additionally leaves no record of what it was permitted to do. Denying that second case was gated on a complete spawn-site inventory, because refusing a caller that has nothing correct to retry with is an outage rather than a guard; `review-sweep.md` was the last gap, and it closed by resolution rather than registration — its six role names (`Bug Hunter`, `CLAUDE.md Auditor`, `Context Checker`, `silent-failure-hunter`, `pr-test-analyzer`, `type-design-analyzer`) were never agent types, only prompt-defined roles already running as implicitly-general-purpose agents, so naming `general-purpose` explicitly preserved behavior exactly. Whether any of the six should become real agent definitions is #2253. Interactive sessions are never affected, and any cwd with no ancestor context file is structurally exempt. New `.claude/agents/read-only-helper.md` carries `tools: [Read, Grep, Glob]` — no `Bash`, `Edit` or `Write` — so an extraction helper is incapable of editing, committing or pushing by capability rather than by prompt; it cannot fetch its own input either, so the parent writes the material to a file and hands over the path. The refusal ships behind `subagent_spawn_guard_enabled`, a default-on global in `orchestrator.yaml` with a bidirectional per-lane override in `clients.yaml`, mirroring `busy_wait_guard_enabled` (#1946). Every previously-untyped spawn site in `auto-dev.md`, `auto-dev-impl.md`, `auto-dev-finalize.md`, `prep-pr.md`, `review-monitor.md` and `review-sweep.md` now names `subagent_type: "general-purpose"` explicitly; `review.md` already named a concrete registered type at all nine of its reviewer sites and is unchanged. **Not addressed here:** nothing refuses or records a *push* from an unrostered subagent — `classify_git_mutation`, its enforcement in `cw guard-cwd`, and the durable `guard.subagent_action` event are deferred in full to #2248, which needs its own empirical capture and toggle first. The Stage 2 impl agent and the Stage 4 finalize agents also remain unrostered harness subagents (residual gap R1).
### Fixed

- **The fix-loop dispatcher no longer rebuilds the remote branch from a template, and stops looping when it cannot find one (#2209):** `dispatch_fix_agent` resolved its remote ref with `_resolve_remote_ref`, which returns the first ref that merely EXISTS (the branch's `@{u}`, else `origin/<branch>`), and only then compared that one ref's tip against the worktree's HEAD. In a cw dispatch worktree git's default `branch.autoSetupMerge` sets `@{u}` to `origin/<default_branch>`, which always exists and is never the fix branch — so whenever the impl agent pushed under a slugified name without `-u`, resolution picked `origin/main`, the post-hoc HEAD check failed, and no later candidate was ever tried. Since #2075 removed the attempt charge from that failure path, the resulting review → failed-dispatch → review cycle was unbounded. `_resolve_fix_remote_ref` now folds HEAD-equality into selection: it walks the impl sentinel's reported branch, then `@{u}`, then the templated `origin/<branch>` guess, and returns the first candidate whose **tip equals HEAD**, skipping one that resolves but is stale rather than dying on it. The reported name comes from a new `_reported_branch` scan of the ticket's sessions (newest-first on `started_at`, first non-blank `str` `last_result.branch`) — no persisted-schema change and no `DEV_QUEUE_SCHEMA_VERSION` bump. `branch` remains the local/worktree key, so the fix session still reuses the review session's worktree. **Operator-visible:** when no rung matches, the row now parks `BLOCKED_ON_USER` with `disposition: fix_dispatch_ref_unresolved` and one `session.needs_attention` (no `stage.errored`, no per-tick repeat) instead of clearing the handoff and reverting to PENDING. Uniquely among the park dispositions, `pending_fix_dispatch` is **retained** — as evidence of the action list the REVIEW round derived, not as a resume point: `cw dev-queue requeue` sets the row PENDING, which stops matching the parked-row exemption (it gates on `BLOCKED_ON_USER`), so the #2142 stale-handoff sweep drops the retained handoff and the ticket is claimed into a fresh REVIEW session (whether requeue should resume the handoff instead is #2265). While parked, the row is exempted from the detect filter and the #2142 stale-handoff drop so it neither consumes a per-tick spawn-cap slot nor has its action list cleared. The disposition is escalation-eligible (paging after 45 minutes) but deliberately outside `HOLD_DISPOSITIONS`, `DRAIN_DISPOSITIONS` and concierge's false-park requeue — auto-requeueing a branch cw cannot locate would re-derive the same failure. Every other `CwError` keeps the existing clear-and-revert path, discriminated by the new typed `RemoteRefUnresolvedError`. Two stale invariant claims are corrected alongside: `fix_dispatch.py`'s module docstring and `core.py`'s #2064 reorder-safety comment both asserted the module is escalation-inert, which the new park falsifies; the reorder stays safe because the one-tick observation delay sits inside the 45-minute escalation grace period. **Not addressed here:** `worktree._resolve_remote_ref` has no production caller left after this change (its own unit tests still exercise it standalone); removing it is a follow-up. Branch-naming standardisation across clients is #2247.
...
- **`cw review settle`, a ready-to-paste settle payload in every blocking codex review comment, a `contests_adjudication` escape hatch, and a lane-gated claim-match tier for the cross-round finding ledger (#2210):** the #1838 ledger had no production writer and only matched exact normalised text, so a re-worded re-raise of an operator-settled finding re-parked the ticket. `cw review settle` now renders the ticket-postable `REVIEW-FINDING-DISPOSITIONS` marker, and each blocking comment prints one payload per finding whose verbatim `file`/`summary` are the ledger identity. `Finding.contests_adjudication` lets a reviewer knowingly contest a settled finding (honoured on the codex single-pass lane). A same-file claim/symbol match tier ships **off**: it needs `codex_claim_suppression_enabled: true` in `orchestrator.yaml` AND `codex_review_tiers: {claim_suppression: true}` on the lane, and until armed it records each finding it would have suppressed as a `review.finding_claim_shadowed` event. Minting a settle is an audited operator act: `--reason` is mandatory and non-blank, every record carries its actor, a CLI-stamped UTC timestamp, the verbatim summary and the `reviewed_sha` it was raised against, each settled finding emits a `review.finding_settled` event, and the command refuses to run inside a dispatch worker (no bypass flag) so the pipeline cannot settle its own reviewer's findings. The settle section of a blocking comment is capped at 10 payloads / 12,000 characters, with the remainder listed compactly rather than a JSON block truncated mid-structure. **The reader enforces that contract, not just the writer:** a disposition record is applied only when it carries the full provenance set (finding identity, actor, a UTC timestamp, reviewed sha, non-empty reason), and anything short of that is ignored — by the mechanical backstop *and* by the reviewer prompt's "previously adjudicated" block — logged at WARNING, and reported on the review comment under "Disposition records refused (no provenance)". Hand-authoring a `REVIEW-FINDING-DISPOSITIONS` block is unsupported; `cw review settle` is the only supported producer, because it is the only path that records provenance and refuses inside a worker. A marker or queue row written before this change still loads but is no longer applied, so a finding settled under the old shape will re-appear once, with the refusal named on the comment — re-settle it with `cw review settle`. The audit events are now recorded **before** the marker is written, and a failed emit aborts the settle with no marker and a non-zero exit: an audit record with no effect is noise, a durable suppression with no audit record is invisible. **A record that fails provenance is never written into the ledger and never replaces an entry already in it** — the ledger write itself validates first, so a malformed or pasted marker cannot evict a legitimately settled finding; the refusal is logged once at WARNING (ticket and key) and reported on the review comment while the prior entry keeps applying. **The ledger key now binds the verbatim summary** — `file::normalized summary::<sha256 of the exact summary>` — so a finding whose summary differs can no longer drift onto another finding's record: the exact tier matches a byte-identical summary only, and a reworded re-raise is the (default-off, shadowed) claim tier's job. The reviewed sha is deliberately not part of the key, because the reviewer re-raises on later commits; it stays a required field of the record. **Operator-visible:** a record minted before this change (two-part `file::normalized summary` key) is refused as an identity gap and must be re-settled with `cw review settle`. The reviewer prompt and refusal note now build the marker's sentinel from one shared constant. **Review text is untrusted input, and a record is now made by POSITION as well as shape:** the renderer writes model-authored finding text into the very comments the ledger's reader parses on the next round, so a finding whose summary carried a well-formed, fully-provenanced `REVIEW-FINDING-DISPOSITIONS` block minted a durable suppression no operator authored. Two independent layers now sit in front of the provenance checks: every untrusted span the review comment interpolates (summary, file, evidence, consequence, suggested fix, contest claim, reviewer role/detail, debt and refused-record rows) is escaped through the new leaf module `cw.review_markers`, which neutralises both sentinels and the `<!--`/`-->` delimiters visibly (`REVIEW-FINDING\-DISPOSITIONS`) rather than stripping them; and the parser honours a block only where `render_finding_disposition_block` emits it — opening the comment body, under its `## Review Finding Dispositions` title. **Operator-visible:** post the marker `cw review settle --out` renders as its own comment, unedited; a block pasted below a preamble or quoted in a reply is not a record and is not applied. The `### Settle a finding` payload keeps its `file`/`summary` verbatim (they are the ledger key) and is made inert losslessly instead, by JSON-escaping `<`/`>`. **The dispatch-worker refusal now fails CLOSED:** `cw review settle` proceeds only when the nearest `.claude/cw-context.json` reports `headless: false`; a worker, a missing or unreadable context, a missing `headless` key and a non-boolean `headless` all refuse, non-zero, writing no marker, no `--out` file and no event. **Operator-visible:** a plain checkout has no context file, so settle must be run from an interactive `cw` session worktree (or a directory beneath one). `DISPOSITION_SENTINEL`, `SETTLE_SECTION_HEADING`, `VOIDED_SENTINEL` and `RefusedDisposition` moved to `cw.review_markers`, which imports nothing from `cw`, so `cw.codex_review._context._prompt_text` (static prompt text) and `cw.review_findings._models` (the executor-neutral finding contract) no longer pull the disposition ledger in behind one constant apiece. See ADR-0016.
...
- **`cw doctor` gains a `stop-hook-scope` check for a Stop hook installed user-level (#2226):** a `cw signal-stop` hook in `~/.claude/settings.json` or `~/.claude/settings.local.json` applies to every Claude session on the machine rather than just cw's worktrees. No cw install path writes one — `scripts/install-skills.sh` touches only `~/.claude/{commands,skills,agents,scripts}` and `cw init` writes only the `Bash(cw:*)` allowlist entry — so the check is detection only, naming the offending file, the `hooks.Stop` coordinates and the exact line to delete. Advisory severity (WARN), so it never changes `cw doctor`'s exit code. A settings file that exists but cannot be read or parsed (invalid UTF-8, malformed JSON, valid JSON that is not an object, an unreadable path) is itself a WARN finding naming the file and the failure class — never a crash out of `cw doctor` — and the other file is still scanned; a missing file is silent.

### Fixed

- **`/prep-pr` `detect-gates` now returns exactly the gates a `CLAUDE.md` bash block defines, not prose bullets and ecosystem defaults on top (#2187, #1867):** run against this repo it returned 11 gates instead of the 9 `## Quality Gates` lists CI enforces — the prose bullet ``No suppressions (`# noqa`, `# type: ignore`) without explicit user approval`` parsed as a gate named ``No suppressions (`# noqa`, `# type``, and the ecosystem default `ruff-format` (`uv run ruff format --check .`, the #1867 gate) was never overridden because the block's own gate was named `ruff`. Three changes in `.claude/scripts/prep_pr_state.py`: a bullet is a gate only when the text before the first `": "` is a single identifier-like token with a non-empty command (a command wrapped in one backtick pair is unwrapped), so prose bullets are ignored; a bash block that yields at least one gate is now authoritative and drops the ecosystem defaults instead of name-matching them (`detected_from` is then `["CLAUDE.md"]`; an empty or unclosed block still falls back to the defaults, and a bullet-only `CLAUDE.md` still overrides defaults by name); and gates that share an executable are named `<tool>-<qualifier>` from the subcommand or `-m` marker — `ruff-check`, `ruff-format`, `pytest-not-integration`, `pytest-integration` — so the two pytest gates and the two ruff gates no longer collide (non-colliding gates such as `mypy`, `pre-commit` and `diff-cover` keep their names). `gate-timeout <name>` now resolves by longest hyphen-delimited prefix, so `pytest-integration` still gets the `pytest` ceiling. A new test pins the repo's own nine-gate list. **Behavior change:** `/prep-pr` no longer auto-fixes format failures for repos whose gates come from a `CLAUDE.md` bash block — the bash block has no autofix syntax and the ecosystem `ruff-format` default (which carried `uv run ruff format .`) is now dropped. Autofix is only available for bullet-declared gates whose name is not also a bash-block gate; block gates such as `ruff-format` are fixed manually or by the `/prep-pr` Step 7 fix loop. `.claude/commands/prep-pr.md` documents the new rules and drops the stale "8 quality gates" / shared-`pytest`-name limitation text.
- **`cw doctor` no longer crashes on an unreadable `~/.claude/settings.json` (#2226):** `_check_bypass_disclaimer` caught only `FileNotFoundError` and `json.JSONDecodeError`, so a settings file with invalid UTF-8 (`UnicodeDecodeError`), an unreadable path such as a directory or a permission error (`OSError`), or valid JSON that is not an object (`AttributeError` on `.get`) raised out of `run_doctor` before any later check — including `stop-hook-scope`, which exists to diagnose exactly a broken install — could print. The crash predates #2226 and was surfaced by it. Every doctor check that reads a user-level settings file (`bypass-disclaimer`, `stop-hook-scope`) now goes through one shared `cw.doctor._shared._read_settings`, which returns a `SettingsReadFailure` (a short failure-class label, never exception text) instead of raising; each check turns that into a WARN naming the file and the failure class, and `run_doctor` reports every other check. A missing file is unchanged: `bypass-disclaimer` notes it, `stop-hook-scope` stays silent. A top-level JSON value that is not an object is now a WARN for `stop-hook-scope` too (it previously read as "no Stop hook").

## [1.47.0] - 2026-09-20

### Added

- **A headless Stop hook that fires after a park comment but with no sentinel now parks the row instead of leaving it RUNNING until the liveness ladder finds it (#2135):** a worker that posted its park/blocker comment to the tracker and then stopped without emitting an `AUTO_DEV_RESULT` left its dev-queue row `RUNNING` — holding a lane slot — until the 45-minute staleness bucket paged. The evidence is now the worker's own **recorded claim**, not an inference: a new `cw signal-park` command, run once from the session worktree root after the park comment has posted, writes a `park_comment_marker` (ticket id, the RUNNING row's stage, cw session id, UTC `posted_at`) into `.claude/cw-context.json`. `cw signal-stop` parks the row when that marker covers the current session, ticket and row stage, the Stop fired with no pending background tasks and no sentinel, and no `AUTO_DEV_RESULT` framing text — not even an unpaired open marker — appears in the transcript at or after `posted_at`. On that conjunction the row moves `RUNNING → BLOCKED_ON_USER` with the `stopped_without_sentinel` disposition, charging no `unproductive_attempts`, and emits a `task.transition` plus a `session.needs_attention`. Evidence-driven, never a timer (ADR-0014): the session is left ACTIVE and the worker is not stopped, so a late sentinel still rescues the row (#918), `posted_at` is never compared to a wall clock (its only non-audit use is as an ordering pivot that can *suppress* a park, never cause one), and the disposition is deliberately outside concierge's false-park requeue set. The accompanying liveness change is signal-only — the sweep's recurring `session_unresponsive` distress fire is withheld for a row in exactly this state whose `session_id` matches the session, since it already paged once. cw never reads the tracker, so the evidence is tracker-agnostic (GitHub and Linear alike), and the +530-line transcript shell parser the first cut of this feature added is deleted. **The CONSUMER ships dark:** the park is gated by `park_on_abandoned_exit_enabled` in `orchestrator.yaml` (default `false`) plus a per-lane / per-ticket `park_on_abandoned_exit` map whose floor is `false`; with the master switch off a sentinel-less Stop defers exactly as before and the marker is not even read. **The PRODUCER is not gated:** `cw signal-park`, the plan-stage stamp step (step 3a of `.claude/commands/auto-dev-plan-appendix.md` and its pointer in `auto-dev-plan.md`) and the *Park-comment stamp rule* section of `auto-dev.md` are live for every dispatched plan-stage worker at merge, whatever the flag says. That is safe because the stamp is carved out of the Tool-Use Denial Exit: a denied, non-zero, unknown-command (version-skew) or `park marker NOT recorded` call is ignored and the worker emits its original sentinel unchanged. The plan stage's consolidated park is the one wired site — where the #2135 repro lives, covering `ambiguities_pending_resolution`, `premises_pending_verification` and `plan_pending_approval`; the impl and review stages are #2228. Arming the consumer is an operator action — see `config/CONFIG_REFERENCE.md`'s *Abandoned-Exit Park Enablement*. The gate is fail-closed and cheap: the Stop hook checks the `RUNNING` dev-queue row before touching config, memoizes the resolved config per `(client, lane)`, and treats a config load failure, a missing `orchestrator.yaml` (never created by the gate), an unreadable `dev_queue.json`, an unknown client, a lane the client never declares (checked *ahead* of the per-ticket override, which could otherwise open an unarmed lane) and an absent lane entry all as disabled — one WARNING with the names and error class, nothing raised out of the hook.
- **`task_already_terminal` is now consumed at all three reconcile-driven completion sites, not just the Stop-hook (#2140):** `_apply_sentinel_to_task`'s `task_already_terminal` outcome flag (from #1692) previously completed only the Stop-hook's own session when a dev-queue task was raced to a genuinely terminal status by a concurrent caller before the lookup ran; `cw.reconcile.idle._mutations`, `cw.reconcile.phantom._mutations`, and the LOCAL-DAEMON git-harvest reaper in `cw.reconcile.local` treated the same signal as an ordinary stage-mismatch refusal, silently orphaning the session (or, in the git-harvest case, re-synthesizing the same harvest sentinel via a real git/opencode subprocess call on every subsequent tick). All three now route the completion through `emit_result_on()` instead, mirroring the Stop-hook's carve-out so a foreign authority's already-door-written result is never clobbered.
...
- **`cw dev-queue tasks` now flags a RUNNING row whose session id no longer resolves to a live worker, via a new `TicketTask.advisory_note` (#1762):** `TicketTask.session_id` is cw's own `Session.id`, a different namespace from the daemon roster's short id (`Session.surface_ref`) and the transcript filename (`Session.claude_session_id`), so comparing them by eye reads as a mismatch even on a healthy ticket. Each reconcile tick now re-derives the mismatch through the same three-hop chain `cw dev-queue wait` already used (extracted to `resolve_session_liveness_for_task`), writes a `?session_mismatch` token to `advisory_note`, and renders it in the REASON column whenever `blocked_reason` is unset. The note is cleared by every `transition_task_status` call. Adds dev-queue schema v37 (`advisory_note` backfilled to `None`); `ARCHITECTURE.md` documents the three namespaces.

### Fixed

- **Dispatch now auto-advances a PLAN-stage ticket straight to IMPL when a signed-off plan is already on disk, instead of re-running the whole planning pass (#1286):** the only check for "does this ticket already have an approved plan?" (`_impl_bypass_plan_available`) sat behind the manual `cw dev-queue requeue --stage impl` command, so every automatic re-entry at `Stage.PLAN` (a fresh `cw dev-queue add`, or an ordinary same-stage requeue) re-verified the same plan from scratch and exited, over and over. `_spawn_claimed_task` now runs the check just before building the executor prompt, using a new local-only mode (`allow_tracker_fallback=False`) so dispatch never makes a tracker call, and advances the stored row PLAN to IMPL under `dev_queue_lock()` through the shared `_advance_stage` helper (also used by the manual requeue path), emitting `TASK_STAGE_CHANGED`. The bypass is skipped when the row was deliberately regressed into PLAN (`requeue --stage plan --regress`) or when the lane's resolved pipeline has no IMPL stage (new `resolve_pipeline_stages`, sharing `_lane_pipeline` with `resolve_executor_config`), and its locked re-find of the stored row is disambiguated by `created_at` so duplicate RUNNING rows for one ticket cannot advance the wrong one.
- **`cw statusline render` no longer reads "not hydrated yet" as "nothing needs you" (#1672):** a task that has shipped or parked with a PR URL but has not yet been observed by the `cw.pr_hydrate` pass (`pr_state` is `None`) contributed nothing to the segment, so a PR whose CI had just gone red rendered no `!N` until hydration caught up. The segment now appends a distinct `?N` count for such tasks (`client-a/impl 2▶ 1⧗ !1 ?2`), suppressed at zero exactly like `!N`. The R5 literals for `PAUSED`, `▶`, `⧗`, `!` are byte-identical for hydrated tasks. Tasks with no `pr_url` (nothing to hydrate) never show `?`. The predicate reuses `cw.pr_hydrate._is_candidate`, so it tracks whatever the hydrator would fetch. `cw dev-queue status`/`tasks` `NEEDS_ATTN` and the render path's R1 no-network guarantee are unchanged. Residuals: a stale but non-None `pr_state` still renders as of its last hydration, and a PR the hydrator can never fetch (deleted PR, unreachable repo, unparsable `pr_url`, dispatch loop not running) keeps showing `?N` until the row is pruned.
- **A phantom session whose `last_result` already carries a terminal sentinel is now completed instead of skipped forever (#1762):** `_detect_phantom_candidates` bypassed any phantom with a terminal-shaped `last_result` on every tick (#536), but `session.status` only flips to `COMPLETED` in the Stop hook, which a crashed daemon never reaches, so the owning dev-queue row stayed `RUNNING` with no recovery path under any `reap_policy`. The staged result is now reconstructed (`cw.result.reconstruct_staged_sentinel`, shared with the Stop hook) and routed through the existing `ROUTE_EMITTED_SENTINEL` action, which is constructive completion and so is not gated by `reap_policy`; a `last_result` that reconstructs into nothing falls through to the ordinary crash pipeline. The `routed_sentinel is None or salvage_csid is None` guard duplicated between the phantom and idle mutation helpers is now one shared `_resolve_routed_sentinel`, loosened once to tolerate a `None` csid.
- **The Codex review backend's `consolidate_verdict` call now stamps `AcceptedFinding.in_plan_scope` instead of leaving it permanently `None` (#2112):** `synthesize_codex_review_result` never derived `planned_files` for the Codex path, so every accepted finding's plan-scope flag stayed unset regardless of whether the finding actually fell inside or outside the ticket's plan. It now loads `.cw/plan.md` via the same `_load_ticket_context` reader prompt assembly already uses and parses its `## Files Modified` manifest with `parse_plan_files_modified`, threading the result into `consolidate_verdict`; `planned_files` stays `None` when `.cw/plan.md` is absent, matching pre-fix output byte-for-byte on that path.
...
- **Dispatch now backs off until the usage-limit reset time Claude names, per client, falling back to `usage_limit_backoff_seconds` only when that time is absent or unparseable (#1409):** the spawn-time `UsageLimitError` path discarded the `· resets <time>` fragment entirely, so every usage limit produced the same flat one-hour window — resuming too early against a weekly limit (re-hitting it, and charging `unproductive_attempts` on each re-hit) and too late against a limit that lifts in minutes. A new `parse_usage_limit_reset` in `cw.exceptions` resolves the fragment beside `USAGE_LIMIT_RE` (which is unchanged, so detection stays byte-identical), `native_daemon` parses it once from the raw subprocess text and carries it on `UsageLimitError.reset_at`, and the loop's new `_resolve_usage_limited_until` uses it only when it is aware, strictly future, and within 7 days. **The back-off window is now keyed by client name, not fleet-wide.** `dispatch_state.json`'s `usage_limited_until` changes from a single ISO string to a `{client: deadline}` mapping, and a tick skips only the clients whose own window is still open; the rest dispatch normally. That scope change is what makes a real parsed reset safe to honour — written into the old scalar, a weekly limit one client hit would have parked the entire fleet for hours instead of leaking one retry per hour. A pre-upgrade scalar found on disk is dropped rather than fanned out (at most one tick of early resumption, then the limit re-arms under the right client's name), and a transcript-derived (reconcile) limit, which names no client, still arms every known client on the flat window. **New event `dispatch.usage_limit_armed`** fires once per client whose window opens, carrying `client`, `until`, and whether the deadline came from a parsed reset or the flat fallback — the set-side counterpart to `dispatch.usage_limit_cleared`, which only ever fired on the way out. See `docs/events.md`. **The real `claude --bg` spawn-time wording is unverified against a captured message** — the recognised forms derive from interactive-transcript wording — so the raw spawn-time text is now logged once per raise at WARNING (secret-scrubbed and capped at 4000 characters via `executor_diagnostics.redact`/`_bounded`) to provide that sample, and any text that does not resolve keeps today's flat behavior exactly. A weekday form naming today at a time that has already passed also falls back to the flat window rather than producing a zero-length one, which would re-claim and re-revert the task every tick until the attempt ceiling parked it. The host timezone is resolved with its DST rules (`TZ`, then `/etc/localtime`), not flattened to today's fixed offset, so a reset on the far side of a transition is not an hour off; an ambiguous fall-back wall clock resolves to the later of its two instants.
- **The liveness sweep's operator distress signal now names an orphaned permission prompt instead of folding it into the generic `session_unresponsive` reason (#1482):** the signal could not previously distinguish a session legitimately waiting on something external from one stuck behind an unanswerable interactive tool prompt (e.g. `Bash`). It now scans the transcript tail for an unresolved, non-subagent tool_use — deliberately excluding `Agent`/`Task` per #1969 — and, when found, fires a `dangling_tool_use` `session.needs_attention` naming the tool and a redacted command snippet. Signal-only, per ADR-0014: nothing is disposed.

## [1.46.0] - 2026-09-14

### Added

- **The read-only-sandbox degraded carve-out now covers Code Quality Reviewer and SysAdmin Reviewer, not just Test Reviewer (#2174):** codex review's sandbox is unconditionally read-only for every reviewer role, so Code Quality Reviewer and SysAdmin Reviewer — whose rubrics require running the repo's configured lint/quality gates — self-reported `status="degraded"` on every ticket for a reason unrelated to the diff, same as Test Reviewer's inability to run pytest. `_TEST_REVIEWER_ROLE` becomes `_READ_ONLY_SANDBOX_EXEMPT_ROLES`, a `frozenset` of all three roles, so `_derive_health` mutes the structurally-forced "degraded" signal for all of them instead of only Test Reviewer.
- **Codex review stages can pin reasoning effort (#1711):** `StageExecutorConfig` gains `reasoning_effort` (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`), emitted as `-c model_reasoning_effort=<value>` on every codex reviewer, fix-cycle, and re-review invocation, so review depth no longer silently follows the operator's personal `~/.codex/config.toml`. It defaults to `high` (an explicit `null` unpins it), an unknown level fails config load, and setting a level on a non-codex backend is a config error. A parked fix-cycle failure's diagnostics bundle now records the effort pin that actually ran. **Operator-visible behavior change:** a `backend: codex` stage that sets no `reasoning_effort` now runs at `high` instead of inheriting codex's own configured level; set it explicitly on the lane (e.g. `max`) to go higher. This is the effort-pin slice of #1711 only; the lean reviewer profile and benchmark remain open there.
- **Reconcile's dirty-worktree breadcrumbs now carry the underlying reason, not just a bare path (#2118):** `_worktree_dirty_reason_by_path` (built on `worktree.unsaved_work_reason`) threads the specific reason through both dirty-worktree producers — the dev-queue revert backstop's breadcrumb now reads `"<worktree path>: <reason>"` instead of the bare path, and the phantom sweep's `ReapCandidate`/`session.phantom_reverted` payload carries `worktree_dirty_reason` alongside the existing boolean `worktree_dirty`. `_worktree_dirty_by_path` becomes a thin boolean view over the new reason-carrying function, mirroring the `unsaved_work_reason`/`worktree_has_unsaved_work` split #2114 established in `cw.worktree`.

### Changed

- **`cw pr-channel proxy --client-id <client>` now forwards only that client's repo's PR events, and fails closed when the repo cannot be resolved (#2146):** one `cw pr-channel serve` process serves every client, and the proxy's `--client-id` had only ever been a replay-cursor identity — `filter_by_client=False` was set explicitly on the pr channel because a PR event has no top-level `client` field to match against — so every subscribed session received every repo's events. `ChannelProxyConfig` gains a `filter_by_repo`/`resolve_repo` pair (a repo axis independent of the existing client-payload axis, following the `build_meta`/`always_relay` per-channel-callable shape), and the pr channel resolves a client to an `owner/repo` slug through the `load_clients` → `_client_cwd`/`_is_dangling_client` → `_resolve_repo_slug` composition `reconcile/stale_dispatch_watch.py` already uses. Scoping is by repo rather than ticket because `PREventRequest` carries no lane or ticket identity and adding one would be a server-side schema change. **Operator-visible behavior change:** because a filter whose purpose is cross-tenant isolation must not silently degrade to broadcast, resolution failure now forwards *nothing* for that client rather than everything — an existing `--client-id` caller whose client is missing from a populated `clients.yaml`, or whose workspace has no parseable GitHub `origin` remote, will see zero events instead of the previous unfiltered stream. The failure is surfaced twice, never silently: a `logger.error` naming the client and the specific resolution step that failed, and one `notifications/message` at level `error` (`event: repo_resolution_failed`) on the subscribed session. The new `--all-repos` flag skips repo resolution entirely and is the only supported path to an intentionally unfiltered stream; omitting `--client-id` remains unfiltered as before, since no filtering was requested there. Codified as ARCHITECTURE.md §7 principle 12 so the rule binds future isolation filters, not just this one.

### Fixed

- **The fix-dispatch loop no longer spawns an uncorrelated fix agent for a row that reverted out of RUNNING mid-handoff (#2142):** the review→fix-agent async handoff writes `task.pending_fix_dispatch` and leaves the row `RUNNING` by design, but an unrelated non-sentinel `RUNNING→PENDING` revert (crash/phantom/stall/salvage) could fire on that row before the handoff was consumed. The next reconcile tick then dispatched the fix agent unconditionally, with no `task.status` check — and since `dispatch_fix_agent` passes no `task=` kwarg, the resulting session correlated to no row at all: it inflated `dispatch/tick.py`'s session-based `running_count` (feeding `cap_full`) while no task-row occupancy metric ever counted it, so a client could sit at `running=2/2 cap_full` beside a single RUNNING row and admit nothing. `_build_dispatch_jobs` now guards on `task.status != RUNNING` before building a dispatch job and drops the stale handoff instead of spawning, clearing `pending_fix_dispatch` so `claim.py`'s ordinary reclaim can pick the row back up. The guard is `!= RUNNING` rather than `== PENDING`: a row parked `BLOCKED_ON_USER` under an unconsumed handoff is the same hazard. Dropping a handoff is never silent — it emits a `STAGE_ERRORED` + `SESSION_NEEDS_ATTENTION` pair (`error_kind`/`paused_status` `fix_dispatch_stale_row`, distinct from `fix_dispatch_failed` because nothing was attempted and nothing failed). The drop is two-phase: the lock only identifies a stale handoff, the audit pair is emitted per-candidate after the lock releases (no event I/O held under `dev_queue_lock()`), and a second, separate lock acquisition clears the handoff only if the row still carries that exact, unchanged handoff. A candidate whose event emission fails is logged and left in place for the next tick to re-detect and re-page — it does not abort any other candidate's drop or dispatch in the same tick — and a row reclaimed or re-armed with a new handoff while the lock was released wins the race: its handoff is never clawed back. `cw dev-queue status` additionally annotates a per-client divergence between the tick's session-based `running` count and the task-row `RUNNING` count (`[ORPHAN? — …]`, reporting only, never changes admission), so this shape is visible without running `cw doctor` directly. The two `cw doctor` wedge classes originally drafted alongside this fix — `wedge/orphan-active-pending-row` and `wedge/fix-dispatch-running-stale` — are deliberately **not** in this change; they feed the destructive `cw doctor --reap` path and their design moved to #2173. A row already built into a dispatch job in the same tick could still revert `RUNNING→PENDING` during the stale-handoff drop's own unlocked emission window, since the built jobs themselves were never re-checked after that window closed — `_act_on_pending_fix_dispatches` now re-validates every built job against the same RUNNING/`fix_dispatch_session_id` predicate (extracted as `_job_still_valid`, shared with `_build_dispatch_jobs`) under one fresh lock acquisition immediately before dispatch, dropping any job whose row no longer qualifies rather than spawning the same uncorrelated-session hazard a tick later than the original guard closed it. That re-validation only checked `status`/`fix_dispatch_session_id`, though: a row that left and re-entered `RUNNING` at a *different* pipeline stage during the same unlocked window (reverted, then re-claimed for a later stage) still passed, so a fix agent built for its old stage could still spawn against it — `_job_still_valid` now also takes the job's `stage`, captured on the row at build time, and requires the row's current `stage` to still match; `_build_dispatch_jobs` captures it, `_revalidate_dispatch_jobs` compares against it.

- **A resumed plan draft's stale round cap now resets on a genuine tracker-state change, gated to operator activity only, and the fingerprint no longer competes with the settlement-marker line for the same bookkeeping slot (#2154, closes #2160):** the plan-stage round cap (`<!-- plan-stage-scan-round: N -->`, cap 2) persisted across dispatches with no way to tell "still stuck" from "operator answered, round N should reset" apart, so a ticket that converged could still hard-park `ambiguity_scan_unconverged` on the next round purely from stale bookkeeping. The cap check now persists a `<!-- plan-stage-last-evaluated: ... -->` fingerprint — exactly two components, `operator_comment` (the id of the newest live-fetched comment that survives the *Comment provenance rule*, or `none`) and `body_sha` (a SHA-256 of the live-fetched issue body, over the raw string) — alongside the round counter, and resets `N` to 0 whenever either component diverges from what was last persisted: a new (non-agent-authored) operator comment, or a body edit. The fingerprint deliberately never derives from the issue's raw `updated_at`/`updatedAt`, which changes on every comment including this pipeline's own park comment and would reset the cap every round. `operator_comment` is gated by the existing *Comment provenance rule* (#2097), cited by name rather than restated: an agent-authored comment never becomes `operator_comment`, however recently it was posted, so the pipeline's own prior park comment can never read as a tracker-state delta. The bookkeeping-line order is explicit and non-competing across every reader: round counter first, the fingerprint line second when present, settlement markers after both. The cap check's `blocker.details` now names the fingerprint evaluated — the `operator_comment` id and the `body_sha` — closing #2160's ask. `resolution_consumed`/`resolution_evidence` (#1896) remain scoped to exactly the three Step 4c EXIT bullets, unchanged — the Pre-branch integrity checks' cap-check and stub-check hard-exits are never eligible carriers for these two keys, since widening that scope would let a round that merely reset its own cap read as productive to `cw.dispatch.productivity`'s crashloop ceiling (#1750). Every `.cw/plan-draft.md` rewrite site now carries forward its bookkeeping lines per one shared draft-rewrite rule.
- **`/cw-smoke-test`'s preflight checker now resolves the repo/tracker from `--client` instead of hardwiring `mattwwarren/claude-workspace` (#2158):** `preflight.py` defaulted `--repo` to this repo's slug and always climbed to this repo's own root for `_resolve_tracker`/`_check_agents`, regardless of `--client` — so for any other client, 4 of the 5 checks silently evaluated the wrong repo. `--client` now resolves a filesystem repo root via `clients.yaml` (mirroring `cw doctor`'s per-client resolution), derives the tracker and GitHub `owner/repo` slug from that root, and `--repo` becomes an explicit override rather than a default. A `--client` naming a client absent from a populated `clients.yaml`, or a `clients.yaml` that fails to load or validate, now fails loudly with a `client_repo_resolved` hard check instead of silently falling back to the default repo or crashing with a traceback. `cw.config.load_clients()` itself now validates `clients.yaml`'s shape before using it, rather than crashing with `AttributeError`/`TypeError` on a malformed-but-syntactically-valid document: a non-mapping top-level document, a non-mapping `clients:` value, a non-mapping client entry, or a non-string client key now all raise `ConfigValidationError` naming the offending key/type, closing the root cause behind three rounds of preflight-side exception handling for this ticket. **Operator-visible behavior change:** a top-level YAML list in `clients.yaml`, previously read silently as "no clients configured," now raises `ConfigValidationError` instead.
- **The fix-dispatch loop no longer fails to resolve its parent session for a same-cycle REVIEW handoff (#2149):** `spawn_create_impl` and the fix-agent's live-session guard both resolved `parent=` via `find_by_name_or_id`, which only matches against the hot `sessions.json` snapshot — so any `parent=` that was a `claude_session_id`, or one that had aged into the archive, raised "Parent session not found," a case a same-cycle REVIEW session's `requested_by_session_id` hits routinely. Both call sites, plus `dispatch_fix_agent`, now resolve through `session_retention.find_session_by_id` (hot-then-archived, id-or-claude-id); `dispatch_fix_agent` additionally treats a fully unresolvable parent as non-fatal, spawning anyway with `parent=None` and a friction note prepended to the prompt rather than dropping the fix-loop handoff entirely.
...
- **A DAEMON session raced to terminal by a concurrent caller no longer leaks until the wall-clock reaper notices (#1692):** `_apply_sentinel_to_task`'s lookup-miss branch reported `routed=False` for a same-ticket/session task already landed terminal by a concurrent caller (most plausibly a reconcile sweep salvaging the same emitted sentinel), and the Stop-hook call site treated every `routed=False` the same as a #1031 stage-mismatch refusal — bailing out and leaving the session's `SESSION_COMPLETED` transition (and its DAEMON worker) orphaned. `SentinelRouteOutcome` and `_HeadlessResolution` now carry a `task_already_terminal` flag distinguishing this race from a stage mismatch, wired up at the Stop-hook call site only (the identical race at the three reconcile-driven call sites is tracked separately in #2140): the task row stays untouched, but the session now completes and its worker is stopped. A new `sentinel.race_miss` event records the race for forensic traceability, modeled on the existing `sentinel.stage_mismatch` event.
