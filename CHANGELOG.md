...
- **`cw agent-spawn-pre` now refuses an explicitly-forked subagent in a headless worker, and a `Read Only Helper` agent that cannot write at all (#2211, following #2017):** an impl worker forked a harness subagent for a read-only tracker lookup; the fork inherited the parent's tools *and* its implementation mandate, edited a source file and a test, committed, and pushed to the live feature branch before the parent's stop message won the race — and because a harness subagent takes no `cw` roster entry, cw could not see it start, observe it, or stop it. The existing PreToolUse hook on the `^(Agent|Task)$` matcher (no new hook, no extra interpreter start, and already-provisioned worktrees pick it up on upgrade) now also applies a spawn-shape policy from the new leaf module `cw.cli._subagent_policy`: in a worker whose `.claude/cw-context.json` reports `headless: true`, a `subagent_type` of `fork` (any case) or blank exits 2 with the reason on stderr and writes no stamp, while a spawn that names **no** `subagent_type` at all is allowed with a `WARN` line — record-only, not enforcement, because the spawn-site inventory needed to make that denial safe is incomplete (`review-sweep.md` names six roles that are not registered agent types, so a refused caller would have nothing correct to retry with). Interactive sessions are never affected, and any cwd with no ancestor context file is structurally exempt. New `.claude/agents/read-only-helper.md` carries `tools: [Read, Grep, Glob]` — no `Bash`, `Edit` or `Write` — so an extraction helper is incapable of editing, committing or pushing by capability rather than by prompt; it cannot fetch its own input either, so the parent writes the material to a file and hands over the path. The refusal ships behind `subagent_spawn_guard_enabled`, a default-on global in `orchestrator.yaml` with a bidirectional per-lane override in `clients.yaml`, mirroring `busy_wait_guard_enabled` (#1946). Every previously-untyped spawn site in `auto-dev.md`, `auto-dev-impl.md`, `auto-dev-finalize.md`, `prep-pr.md` and `review-monitor.md` now names `subagent_type: "general-purpose"` explicitly. **Not addressed here:** nothing refuses or records a *push* from an unrostered subagent — `classify_git_mutation`, its enforcement in `cw guard-cwd`, and the durable `guard.subagent_action` event are deferred in full to #2248, which needs its own empirical capture and toggle first. The Stage 2 impl agent and the Stage 4 finalize agents also remain unrostered harness subagents (residual gap R1).
### Fixed
