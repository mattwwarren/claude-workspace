...
- **A resumed plan draft's stale round cap now resets on a genuine tracker-state change, gated to operator activity only, and the fingerprint no longer competes with the settlement-marker line for the same bookkeeping slot (#2154, closes #2160):** the plan-stage round cap (`<!-- plan-stage-scan-round: N -->`, cap 2) persisted across dispatches with no way to tell "still stuck" from "operator answered, round N should reset" apart, so a ticket that converged could still hard-park `ambiguity_scan_unconverged` on the next round purely from stale bookkeeping. The cap check now persists a `<!-- plan-stage-last-evaluated: ... -->` fingerprint (`updated_at`, the newest comment's id/timestamp, `queue_metadata.plan_approved_at`) alongside the round counter and resets `N` to 0 whenever a fresh live-fetch of those values diverges from what was last persisted — a new operator comment, a body edit, or an approval reply, per the existing #1653 human-facing vocabulary in `orchestrate-sprint/SKILL.md`. The "newest comment" half of that fingerprint is gated by the existing *Comment provenance rule* (#2097): only a comment that rule does not mark agent-authored can move the fingerprint, so the pipeline's own prior park comment can never read as a tracker-state delta on the very next round. The bookkeeping-line order is now explicit and non-competing across every reader: round counter first, the fingerprint line second when present, settlement markers after both. The cap check's `blocker.details` also now names the comment id/timestamp the round's fingerprint evaluated, closing #2160's ask. `resolution_consumed`/`resolution_evidence` (#1896) remain scoped to exactly the three Step 4c EXIT bullets, unchanged — the Pre-branch integrity checks' cap-check and stub-check hard-exits are never eligible carriers for these two keys, since widening that scope would let a round that merely reset its own cap read as productive to `cw.dispatch.productivity`'s crashloop ceiling (#1750). A resumed draft also now runs a capped, independent-axis "body-consistency fold" (Step 1c.0) that re-spawns the Plan agent to mechanically rewrite `## Files Modified`/Phase 1/Phase 2/`## Touch-point Contract` for consistency with the round's settled decisions, so a resumed plan's own file/phase sections no longer drift out of sync with what was actually settled.
