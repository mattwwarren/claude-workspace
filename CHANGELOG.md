...
- **`create_worktree` can now fetch and fast-forward a behind, unoccupied worktree on the reuse path, closing the reuse-path fetch asymmetry (#2213):** the first-time path fetched `origin/<branch>` before resolving the branch, but reuse returned the existing worktree untouched, so a per-ticket worktree reused across pipeline stages could sit on a stale HEAD while `origin/<branch>` had moved on. The refresh is opt-in via a new keyword-only `refresh_on_reuse` (default `False`, today's path-only behavior); the dispatch claim path and `dispatch_fix_agent` pass `True`, interactive `cw start` does not. With it set, an occupancy check with local reads only runs first: unsaved work (uncommitted, untracked or unpushed, per `unsaved_work_reason`), a live session homed on the path (the shared `live_session_worktree_paths`, extracted from the worktree GC), or unreadable session state all mean the worktree is used as-is, with a DEBUG log naming the path and reason and no network call. Otherwise it best-effort fetches `origin/<branch>` (a network call that can be slow; a failure degrades to as-is) and runs `git merge --ff-only` to `refs/remotes/origin/<branch>` only when HEAD is a strict ancestor. Diverged and `--ff-only`-refused worktrees are left as they were with a WARNING, and submodules are re-synced after a successful fast-forward. It never resets, force-checks-out, deletes or raises. A branch that is not on origin yet is now logged at DEBUG instead of WARNING (`fetch_feature_branch`'s `quiet_missing_ref`), which also quiets the first-time path. This closes the asymmetry only and does not claim to fix the byte-identical review verdicts that prompted the ticket.
### Changed
