...
| S1 ambiguity/premise scan, 2 consecutive rounds without converging | EXIT `blocked` with `blocker.reason: "ambiguity_scan_unconverged"` |
| S1 pre-branch stub check, unresolved `## Deferred Premises` entry | EXIT `blocked` with `blocker.reason: "deferred_stub_unresolved"` |
| S1 tier re-verification (Step 1g.0), stamped tier stale vs. current classification | EXIT `blocked` with `blocker.reason: "scope_tier_stale"` |
...
| `review.downgraded_disposition_count` | int \| null | **#2009** — count of `"fixed"` dispositions the Claude-native Step 3c `cw review verify-fixes` pass walked back to `"dropped"` because the fix-cycle diff never touched the finding's cited location; the same quantity as `ReviewVerdict.downgraded_disposition_count`. Captured at Step 3c after the fix loop settles — **not** part of the Checkpoint-3a frozen set. Each downgrade also emits one `review.fixed_disposition_downgraded` event (`docs/events.md`). Purely advisory — nothing in cw gates on it. Optional field; `null` means the producer did not report it (pre-#2009 payloads, and executors with no verify-fixes step, e.g. Codex) — **not** "zero downgrades"; `0` means a confirmed zero (#2098 convention). No `schema_version` bump required — see Note (A15, §8). |
| `health` | object | See §5. |
...
The same object shape MAY also appear on a `merge_gate_blocked` result to surface the gate reason (e.g. `prior_pipeline_pr_open`, issue #777) — `blocker = null` remains accepted for that status. Since #1870 the same applies to `empty_diff_blocked` (typically `empty_diff_no_commits`), on identical terms.
...
| `plan_unreviewable` | Plan Reviewer (spec station) returned MUST_FIX both before and after a single Step 1f.4 revision cycle — needs human triage, not another auto-revision. No branch created. `blocker.stage` is `"stage1_plan"`. Also posts the persisting blocking findings as a tracker comment (#1815) |
| `plan_unsound` | Plan Soundness Reviewer returned a MUST_FIX (direction contradicts a codified `ARCHITECTURE.md` §7/§8 rule) in a headless run, or it persisted after a Step 1f.4 revision cycle. No branch created. `blocker.stage` is `"stage1_plan"`. Also posts the persisting blocking findings as a tracker comment (#1815) |
| `ambiguity_scan_unconverged` | Step 1c's ambiguity/premise scan parked for 2 consecutive rounds without converging (round cap reached). No branch created. `blocker.stage` is `"stage1_plan"`; `retry_eligible: true`; the still-open item(s) and round count are named verbatim in `blocker.details` (#1683) |
| `deferred_stub_unresolved` | Step 1c's pre-branch stub check found a `## Deferred Premises` entry still marked `PENDING` after the scan required to classify it. No branch created. `blocker.stage` is `"stage1_plan"`; `retry_eligible: true`; the unresolved stub(s) are named verbatim in `blocker.details` (#1683) |
| `scope_tier_stale` | Step 1g.0's tier re-verification found the tier last computed this invocation differs from the tier freshly recomputed from the plan's current state, which could otherwise silently skip the Large-tier operator-approval gate. No branch created. `blocker.stage` is `"stage1_plan"`; `retry_eligible: true`; `blocker.details` names both tiers with their `(files, lines, forbidden_touched)` tuples (#1815, #1897) |
...
| `destructive_directive_requires_operator` | A directive sourced from a tracker comment would delete a remote branch, force-push or rewrite history on a shared branch, discard uncommitted/committed work, or close/reopen a ticket. Never actioned headlessly whatever the comment's provenance — an automated session posts under the operator's own account, so an agent-written comment and an operator's are indistinguishable without the `<!-- cw-agent-authored -->` marker (#2097). `details` quotes the directive verbatim and names the comment (author, created timestamp, marked/unmarked); `retry_eligible: false`. See `.claude/commands/auto-dev.md`'s "Comment provenance rule". |
...
| `dependency_unmerged` | This ticket was split from a dependency chain (#2233/#2213 precedent) and cannot proceed until an upstream ticket's PR merges — not a broken implementation leg. cw classifies this via `OPERATOR_UNAVAILABLE_BLOCKER_REASONS` and tags the park distinctly (RFC 0011 A1); `retry_eligible: true` (#2260). |
| `impl_comments_unreadable_after_regress` | Orientation's live tracker-comment fetch failed after this IMPL entry was reached via `_stage_regress` (`.claude/commands/auto-dev-impl.md`) — continuing on stale cached comments would defeat the regress. cw classifies this via `OPERATOR_UNAVAILABLE_BLOCKER_REASONS` and tags the park distinctly (RFC 0011 A1); `retry_eligible: true` (#2415). |
...
**Warn-only known-reason registry (#2097).** `cw.auto_dev_result.schema` carries `KNOWN_BLOCKER_REASONS`, the set of reasons cw and the producer skills are known to emit today. It is **advisory, never a gate**: a reason outside it still parses, is still stored, and is still surfaced verbatim exactly as the open-enum contract requires — cw additionally logs `blocker_reason_unknown reason=<value>` at WARNING, appends ` (unrecognized)` to the `session.needs_attention` breadcrumb, and prefixes the value with `?` in `cw dev-queue tasks`' REASON column. This exists because an *invented* reason reads exactly like a documented routing code to an operator, which is how #2097 escalated. A producer that genuinely wants a reason outside the registry declares it by prefixing the value with **`x_`** — the explicit freeform namespace, which suppresses the warning and the flags. Adding a registered reason requires no `schema_version` bump (the enum is still open); it is an edit to `KNOWN_BLOCKER_REASONS` plus the table above. `tests/test_agent_comment_provenance.py`'s `test_every_documented_reason_is_registered` enforces only one direction — every reason documented in `.claude/commands/auto-dev.md`'s table must be a member of `KNOWN_BLOCKER_REASONS` — not the reverse: registering a reason here does not require a matching row in that table (several registered reasons, including `push_auth_failed`/`operator_unavailable`/`dependency_unmerged`, currently have none).
...
**Note (A15, #2009):** Adding the optional `review.downgraded_disposition_count` int-or-null (§3.3) — the same shape of change as A13/`rejected_count`, not A14's fail-closed one: a purely advisory field that nothing in cw gates on and that does not feed `health.recommendation`, defaulting to `null` so every pre-#2009 payload parses unchanged and a missing value reads as not-reported rather than as a confirmed zero (the #2098 convention). The Claude-native producer sources it from Step 3c's `cw review verify-fixes` output, after the fix loop settles, so it is explicitly outside the Checkpoint-3a Freeze set; executors with no verify-fixes step leave it `null`. **No version bump** required — same precedent as A9/A11/A13.
...
- `reason` (str) — `s1_ambiguity_scan_skipped` only (#2376). Names why Step 1a.0b's fast path skipped the ambiguity/premise re-scan on a resumed round; its current sole value is `approved_fingerprint_match` (Checkpoint 1's row-path evidence — `queue_metadata.plan_approved_at` non-null AND `queue_metadata.plan_approved_fingerprint` equal to the freshly-computed `draft_fp` — already matched before Step 1c ran). Open enum — consumers MUST tolerate unknown future values, mirroring `error_kind` below.
