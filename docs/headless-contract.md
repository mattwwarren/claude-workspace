...
- `reason` (str) — `s1_ambiguity_scan_skipped` only (#2376). Names why Step 1a.0b's fast path skipped the ambiguity/premise re-scan on a resumed round; its values are `approved_fingerprint_match` (Checkpoint 1's row-path fingerprint equality already matched before Step 1c ran) and `operator_approval_no_delta` (the row has a durable approval timestamp, no newer operator-authority comment, and a matching persisted `body_sha` against the live-fetched body — all three required — so the Operator-authority delta alternate applied despite fingerprint drift). Open enum — consumers MUST tolerate unknown future values, mirroring `error_kind` below.
