...
- `reason` (str) — `s1_ambiguity_scan_skipped` only (#2376). Names why Step 1a.0b's fast path skipped the ambiguity/premise re-scan on a resumed round; its values are `approved_fingerprint_match` (Checkpoint 1's row-path evidence — `queue_metadata.plan_approved_at` non-null AND `queue_metadata.plan_approved_fingerprint` equal to the freshly-computed `draft_fp` — already matched before Step 1c ran) and, per #2433, `operator_approval_no_delta` (Checkpoint 1's row-path alternate condition — `queue_metadata.plan_approved_at` non-null AND no operator-authority comment postdates it, per the *Operator-authority delta* rule in `.claude/commands/auto-dev.md` — already matched before Step 1c ran, independent of fingerprint equality). Open enum — consumers MUST tolerate unknown future values, mirroring `error_kind` below.
