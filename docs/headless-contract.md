...
| `review.had_real_commit` | bool | **#1723** — true iff at least one fix cycle in the run actually committed a change (OR'd across cycles); distinguishes a fix loop that converged (no MUST_FIX survivors) because a real fix landed from one that converged purely because every cycle's fix invocation was a tolerated no-op. Purely advisory — prose-only consumption in `render_verdict_comment`'s non-blocking headline. Optional field; defaults to `True` on payloads from producers that predate this field (backward compatible — no prior payload could have reported a flaked convergence). No `schema_version` bump required — see Note (A11, §8). |
| `health` | object | See §5. |
...
**Note (A11, #1723):** Adding the optional `review.had_real_commit` bool (§3.3) — same shape of change as A9/`agents_run`: a purely advisory field, prose-only consumption (it only changes wording inside `render_verdict_comment`'s non-blocking headline), defaulting `True` so every pre-#1723 payload parses unchanged and renders exactly as it always did. **No version bump** required — same precedent as the v5 `agents_run` introduction (§8, version-history row 5).
