...
- `[OK/WARN] stop-hook-scope` — looks for a `cw signal-stop` Stop hook in `~/.claude/settings.json` or `~/.claude/settings.local.json`; `[WARN]` names the file, the `hooks.Stop` coordinates and the exact line to delete.
