...
- `[OK/WARN] stop-hook-scope` — looks for a `cw signal-stop` Stop hook in `~/.claude/settings.json` or `~/.claude/settings.local.json`; `[WARN]` names the file, the `hooks.Stop` coordinates and the exact line to delete, or names a settings file it could not read or parse (invalid UTF-8, malformed JSON).
...
The installer also prints an immediate `WARNING:` to stderr, at the moment it makes that backup, naming the replaced file and the backup path — not just at the end-of-run summary — so machine-local edits don't go unnoticed. Machine-local `review_monitor.py` config (its `CANONICAL_REPO_PATHS` dict) can be restored after such a replacement via the `CW_CANONICAL_REPO_PATHS` JSON env var (e.g. `CW_CANONICAL_REPO_PATHS='{"owner/repo": "/path/to/canonical/clone"}'`) instead of hand-editing this repo's tracked copy.
