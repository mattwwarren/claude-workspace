...
**#952 — zero-comment context.** Root-cause the debt-lane intake path (`local_runner.build_task_message`, suspect at `local_runner.py:232–240`) producing `comments: []` when the issue has comments. Fix: both lanes must read comments at the same call site; add regression test covering both lanes for a ticket with comments; emit a WARN event (or `session.needs_attention`) when the comments fetch returns empty for a ticket with a non-zero comment count.
