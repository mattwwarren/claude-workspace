...
| `blocked_result_requeue_enabled` | bool | `true` | Per-client rollout/escape hatch for #2401 deterministic-parse `BlockedResult` requeues. Set `false` to retain terminal handling for an affected client; cw logs each shadowed requeue so the operator can reconcile the rollout before re-enabling it. |
