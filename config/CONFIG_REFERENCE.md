...
| `blocked_result_requeue_enabled` | bool | `false` | Per-client opt-in for #2401 deterministic-parse `BlockedResult` requeues. Set `true` only after reconciling any already-requeued rows/events for the client; missing or unusable client configuration fails closed to terminal handling. |
