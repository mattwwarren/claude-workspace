...
| `sentinel_mismatch_veto_enabled` | bool | `false` | Per-client opt-in for #2405's cap-only phantom stage-mismatch veto. `false` retains the pre-#2405 transcript-liveness fallback and logs the cap-only decision as shadow telemetry; set `true` one client at a time to enable the evidence-based `sentinel_mismatch_veto_cap` policy. |
